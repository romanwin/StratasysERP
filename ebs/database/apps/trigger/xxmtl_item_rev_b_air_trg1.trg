create or replace trigger APPS.xxmtl_item_rev_b_air_trg1
  after insert on MTL_ITEM_REVISIONS_B
  for each row


when ((NEW.organization_id = 91) and (NEW.last_updated_by <> 4290))
DECLARE
...
BEGIN
  --------------------------------------------------------------------
  --  name:            XXMTL_ITEM_REV_B_AIR_TRG1
