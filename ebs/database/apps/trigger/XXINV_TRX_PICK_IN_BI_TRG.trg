CREATE OR REPLACE TRIGGER xxinv_trx_pick_in_bi_trg
  BEFORE INSERT ON xxinv_trx_pick_in
  FOR EACH ROW
--------------------------------------------------------------------
  --  customization code: CHG0041294
  --  name:               XXINV_TRX_SHIP_CONF_IN_BI_TRG  
  --  Revision:           1.0
  --------------------------------------------------------------------
...
BEGIN
  ...
  BEGIN
    ...
    FROM   xxobjt.xxinv_trx_ship_out tso
