create or replace view xxasn_pb_direct_support_v as
select
--------------------------------------------------------------------
--  name:            XXASN_PB_DIRECT_SUPPORT_V
...
where  flv.lookup_type               = 'XX_PB_ALARIS_SUPPORT'
...
from   fnd_lookup_values             flv,
       fnd_lookup_types_tl           flvt,
       fnd_lookup_types              flt
where  flv.lookup_type               = 'XX_PB_EDENCONNEX_SUPPORT'
and    flv.lookup_type               = flt.lookup_type
and    flv.language                  = 'US'
and    flv.enabled_flag              = 'Y'
and    trunc(sysdate )               between nvl(flv.start_date_active, sysdate - 1)  and  nvl(flv.end_date_active, sysdate)
