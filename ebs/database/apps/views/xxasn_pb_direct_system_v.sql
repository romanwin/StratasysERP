create or replace view xxasn_pb_direct_system_v as
select
--------------------------------------------------------------------
--  name:            XXASN_PB_DIRECT_SYSTEM_V
...
--  1.1  03/01/2011  Dalit A. Raviv    correct field end_user_price
--------------------------------------------------------------------
        decode(flv.tag,
               'CN','OBJET CN (OU)',
               'DE','OBJET DE (OU)',
        ...
        XXASN_PRICE_BOOK_PKG.get_transfer_price (XXCS_SESSION_PARAM.get_session_param_number(1),
                                                 ...
                                                 null)                                           end_user_price,
        XXASN_PRICE_BOOK_PKG.get_demo_unit_price(XXCS_SESSION_PARAM.get_session_param_number(1),
                                                 substr(flv.meaning,1,length(flv.meaning)-3) )   demo_unit_price,
        flv.attribute4                price,
        to_number(flv.attribute2)     order_by
from    fnd_lookup_values             flv,
        fnd_lookup_types              flt
where   flv.lookup_type               = 'XX_PB_SYSTEMS'
...
and     flv.attribute5                =  'XX_PB_ALARIS_BASIC'
union all
...
and     flv.enabled_flag              = 'Y'
and     trunc(sysdate )               between nvl(flv.start_date_active, sysdate - 1)  and  nvl(flv.end_date_active, sysdate)
and     flv.attribute5                =  'XX_PB_EDENCONNEX_BASIC';
