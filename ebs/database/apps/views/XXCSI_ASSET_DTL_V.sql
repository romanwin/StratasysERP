CREATE OR REPLACE VIEW XXCSI_ASSET_DTL_V
  AS
select
-----------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------
      ...
      sub_tbl.gmn_flag,                                                                    -- 13
      sub_tbl.status,                                                                      -- 14
      --sub_tbl.sf_end_customer,                                                           -- 15 #CHG0045159 Commented
      --sub_tbl.sf_current_location,                                                       -- 16 #CHG0045159 Commented
      --sub_tbl.sf_install_location,                                                       -- 17 #CHG0045159 Commented
...
from (SELECT msi.description item_description ,
   ...
   (SELECT DECODE(count(*),0,'False','True')
         ...
         WHERE WND.DELIVERY_ID = WDA.DELIVERY_ID
           ...
           AND OOLA.LINE_TYPE_ID IN (SELECT OTL.TRANSACTION_TYPE_ID
                                     FROM OE_TRANSACTION_TYPES_TL OTL
                                     WHERE OTL.LANGUAGE = 'US'
                                    AND OTL.NAME LIKE '%GMN%')
           AND HCA.Cust_Account_Id = hcao.cust_account_id
           and PR.INVENTORY_ITEM_ID = cii.inventory_item_id)  gmn_flag,                                -- 13
