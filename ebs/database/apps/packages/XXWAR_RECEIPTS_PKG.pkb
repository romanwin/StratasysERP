...
AS
  ...
  *                3.1      CR (TBD) - Process Lockboxes concurrent request has a new parameter  *
  *                         after patching for JPMC                                              *
  *****************************************************************************************
  */
       ...
       lc_request_id := FND_REQUEST.SUBMIT_REQUEST (application  => 'AR'
                          ...
                          , argument18   => NULL
                                                  ...
                                                  , argument20   => lc_profile_val   -- Issue-63484 -Added ny Mahidhar for fetching unearned discount profile value
                          						  -- Adding new parameter after JPMC patching
												  --, argument21   => 'L'      --Pass L argument. Don't change this
                                                  --, argument22   => NULL
												  , argument21   => '1'      --Pass 1 argument. Don't change this
												  , argument22   => 'L'      --Pass L argument. Don't change this
                                                  , argument23   => NULL
                                                ) ;
                                           -- COMMIT;
    ELSE
...
FUNCTION get_set_batch_id(
  p_line_type VARCHAR2,
  p_batch_id  VARCHAR2 DEFAULT NULL
)
RETURN VARCHAR2
IS
BEGIN
  IF UPPER(p_line_type) = 'BH' THEN 
    g_batch_id := p_batch_id;
  END IF; 
  RETURN g_batch_id;
END get_set_batch_id;
