CREATE OR REPLACE PACKAGE BODY xxssys_strataforce_events_pkg AS
  ...
  --  2.3  17/01/2019    Lingaraj                   CHG0044879 - exclude Trade In lines (quantity <0) from calculation
  --                                                Order Line Event Generation logic modification(handle_order_line_event)
  --  2.4  04/03/2019    Lingaraj                     INC0148774 - Add sync_daily_rate from xxobjt_oa2sf_interface_pkg
  --  2.5  07-Mar-2019   Diptasurjya                INC0149408 - Fix cursor query to fix invalid number error from
  --                                                procedure generate_related_price_events
  --  2.6  18/06/2019    Roman W.                   INC0160471-INC0160411
  --  2.7  07-Nov-2019   Adi Safin                 CHG0046812 - Fix in is_asset_valid2sync
  --  2.8  27/03/2020    Roman W.                  CHG0047450 - Account, Contact interface - ReDo
  --  2.85 15/10/2020    yuval tal                 CHG0048741 - modify push_so_line_fsl_events proc name and logic  to push_order_line_events
  --  2.9  7/7/2020      yuval tal                 CHG0048217 - b2b ecomm modify  CUSTOMER_EVENT_PROCESS /compare_old_new_items
  -- 3.0   4.4.2021      yual tal                  CHG0049656 - modify process_system_setup
  -- 3.1    5/11/2021    Yuval tal                 CHG0049822 - add new events modify  handle_order_line_event
  ----------------------------------------------------------------------------------------------------------------------------------------------
  g_user_id     NUMBER := 0;
  g_resp_id     NUMBER := 0;
  g_appl_id     NUMBER := 0;
  ...
  -- 1.1 7/7/2020      yuval tal                        CHG0048217 - b2b ecomm add new fields
  -- --------------------------------------------------------------------------------------------
  ...
  BEGIN
    IF p_old_item_rec.segment1 = p_new_item_rec.segment1 AND
      ...
      -- CHG0048217
       nvl(p_old_item_rec.unit_length, '-1') =
       nvl(p_new_item_rec.unit_length, '-1')
      
       AND nvl(p_old_item_rec.unit_width, '-1') =
       nvl(p_new_item_rec.unit_width, '-1')
      
       AND nvl(p_old_item_rec.dimension_uom_code, '-1') =
       nvl(p_new_item_rec.dimension_uom_code, '-1')
      
       AND nvl(p_old_item_rec.unit_height, '-1') =
       nvl(p_new_item_rec.unit_height, '-1')
      
       AND nvl(p_old_item_rec.attribute26, '-1') =
       nvl(p_new_item_rec.attribute26, '-1')
      
       AND nvl(p_old_item_rec.orderable_on_web_flag, '-1') =
       nvl(p_new_item_rec.orderable_on_web_flag, '-1')
      
       AND nvl(p_old_item_rec.hazardous_material_flag, '-1') =
       nvl(p_new_item_rec.hazardous_material_flag, '-1')
      
       AND nvl(p_old_item_rec.default_shipping_org, '-1') =
       nvl(p_new_item_rec.default_shipping_org, '-1')
  ...
  -------------------
  -- handle_acct_relationship
  ------------------
  --------------------------------------------------------------------
  --  purpose :      generate event to sfdc/sfdc_AccountRelationShipCmp
  ---------------------------------------------------------------------
  --  ver  date          name             desc
  -- 1.0   9/7/2020      yuval tal        CHG0048217 - initial
  -------------------------------------------------------------
  PROCEDURE handle_acct_relationship(p_cust_account_id         NUMBER,
			 p_related_cust_account_id NUMBER,
			 p_org_id                  NUMBER) IS
    l_xxssys_event_rec      xxssys_events%ROWTYPE;
    l_account_number        VARCHAR2(50);
    l_account_relate_number VARCHAR2(50);
  
    CURSOR c IS
      SELECT cust_acct_relate_id,
	 acc.account_number related_account_number,
	 acc2.account_number
      /* INTO   l_account_number,
      l_account_relate_number*/
      FROM   hz_cust_acct_relate_all hcara,
	 hz_cust_accounts        acc,
	 hz_cust_accounts        acc2
      WHERE  acc.cust_account_id = hcara.cust_account_id
      AND    acc2.cust_account_id = hcara.related_cust_account_id
	-- AND    hcara.cust_acct_relate_id = p_cust_acct_relate_id
	
      AND    hcara.cust_account_id = p_cust_account_id
      AND    related_cust_account_id = p_related_cust_account_id
  ...
  BEGIN
    ...
    LOOP
      l_xxssys_event_rec.target_name     := g_target_name;
      l_xxssys_event_rec.entity_name     := g_account_relate_name;
      l_xxssys_event_rec.entity_id       := i.cust_acct_relate_id;
      l_xxssys_event_rec.attribute1      := i.account_number;
      l_xxssys_event_rec.attribute2      := i.related_account_number; --l_cust_account_number;
    ...
    END LOOP;
    /* write_log('Event Generated for :' || g_account_relate_name ||
    ' ,cust_acct_relate_id : ' || p_cust_acct_relate_id);*/
  END;
  ...
  -- ----  ----------  -----------  ------------------------------------------------------------
  -- 1.0   04/01/2020  Roman W.     CHG0047450 - Re-Do
  ----------------------------------------------------------------------------------------------
  PROCEDURE handle_account_profile(p_cust_account_profile_id IN NUMBER) IS
    -----------------------------
    --    Local Definition
    -----------------------------
  ...
  BEGIN
    ...
    WHERE  hcp.cust_account_profile_id = p_cust_account_profile_id;
  
    handle_account(p_cust_account_id => l_cust_account_id);
  
  END handle_account_profile;
  ...
  -- 1.5  17/06/2020  Roman W.           CHG0047450 - adjustments acording to new logic
  -- --------------------------------------------------------------------------------------------
  ...
  BEGIN
    l_event_name := 'xxssys_strataforce_events_pkg.handle_account_site' ||
	        (CASE
	          WHEN g_eventname IS NOT NULL THEN
	           ('|' || g_eventname)
	          ELSE
	           ''
	        END); --INC0143702
    ...
    LOOP
      ...
      -- Added By Roman W. 31/03/2020 CHG0047450
      /* xxssys_strataforce_events_pkg.insert_account_addr_event(p_cust_acct_site_id => p_cust_acct_site_id,
      p_party_site_number => i.party_site_number,
      p_last_updated_by   => fnd_global.user_id,
      p_created_by        => fnd_global.user_id,
      p_trigger_name      => l_event_name,
      p_trigger_action    => 'UPDATE');*/
    
      --Generate Account EVENT to sync the Site and Bill to Address
      --Added on 22.Jun.2018
      IF 'Y' = is_account_addr(p_cust_acct_site_id => p_cust_acct_site_id) THEN
        l_xxssys_event_rec.entity_name := g_account_entity_name;
  ...
  -- 1.3  31/03/20202 Roman W.           CHG0047450 - Account, Contact interface - ReDo
  -- --------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    LOOP
      ...
      -- Added By Roman W. 31/03/2020 CHG0047450
      xxssys_strataforce_events_pkg.insert_account_addr_event(p_cust_acct_site_id => i.cust_acct_site_id,
					  p_party_site_number => i.party_site_number,
  ...
  -- Ver  Date        Name                               Description
  -- 1.0  11/20/2017  Diptasurjya Chatterjee             CHG0041829 : Initial build
  -- 1.1 08/07/2020   yuval tal                          CHG0048217 - b2b ecomm modify add p_trigger_mode parameter
  -- --------------------------------------------------------------------------------------------
  PROCEDURE handle_contact(p_contact_id   IN NUMBER,
		   p_trigger_mode IN VARCHAR2 DEFAULT 'N') IS
    l_xxssys_event_rec xxssys_events%ROWTYPE;
    l_is_valid         VARCHAR2(1) := 'N';
  BEGIN
    ...
    IF l_is_valid <> 'N' THEN
      ...
      xxssys_event_pkg.insert_event(l_xxssys_event_rec, p_trigger_mode);
    END IF;
  END handle_contact;

  -- -- --------------------------------------------------------------------------------------------
  -- Purpose: CHANGE - handle_role_resp
  -- --------------------------------------------------------------------------------------------
  -- Ver  Date        Name                               Description
  -- 1.1 08/07/2020   yuval tal                          CHG0048217 - b2b ecomm modify add p_trigger_mode parameter
  -- --------------------------------------------------------------------------------------------

  PROCEDURE handle_role_resp(p_responsibility_id IN NUMBER,
		     p_trigger_mode      VARCHAR2 DEFAULT 'N') IS
  ...
  BEGIN
    FOR i IN cur_role_resp
    LOOP
      --handle_contact(i.cust_account_role_id); -- CHG0042626 commented
      handle_contact(i.cust_account_role_id, p_trigger_mode); -- CHG0042626
    END LOOP;
  ...
  BEGIN
    ...
    IF l_is_valid <> 'N' THEN
      FOR rec_contact IN cur_contact_of
      LOOP
        handle_contact(rec_contact.contact_id);
      END LOOP;
    
      FOR rec_contact IN cur_employee_of
      LOOP
        handle_contact(rec_contact.contact_id);
      END LOOP;
    END IF;
  ...
  BEGIN
    ...
    IF l_is_valid <> 'N' THEN
      FOR rec_relationship IN cur_relationship
      LOOP
  ...
  BEGIN
    ...
    IF l_is_valid <> 'N' THEN
      FOR rec_code_assignment IN cur_code_assignment
      LOOP
  ...
  BEGIN
    ...
    IF l_is_valid <> 'N' THEN
      FOR rec_org_contact IN cur_org_contact
      LOOP
  ...
  BEGIN
    ...
    IF l_is_valid <> 'N' THEN
      FOR rec_account IN cur_account
      LOOP
        handle_account(rec_account.cust_account_id);
      END LOOP;
    
      FOR rec_cont_point IN cur_cont_point
      LOOP
  ...
  BEGIN
    ...
    IF l_is_valid <> 'N' THEN
      FOR rec_contact IN cur_contact
      LOOP
        handle_contact(rec_contact.contact_id);
      END LOOP;
    END IF;
    -- enter site events
    FOR rec_site IN cur_site
    LOOP
  ...
  -- 1.2  09/01/2019  Lingaraj    INC0143702 - Business Event - subscription (Handling Site create/update)
  -- 1.3  7/7/2020    yuval tal   CHG0048217 - b2b ecomm modify ADD responsibility/RELATIONSHIP event
  -- 1.4  04/01/2020  Roman W.    CHG0047450 - Re-Do
  --                                 added handling to BE :
  ...
  FUNCTION customer_event_process(p_subscription_guid IN RAW,
    ...
    l_related_cust_account_id NUMBER;
    l_cust_account_id         NUMBER;
    l_org_id                  NUMBER;
  BEGIN
    ...
    ELSIF g_eventname IN
          ('oracle.apps.ar.hz.CustAcctRelate.create',
           'oracle.apps.ar.hz.CustAcctRelate.update') THEN
    
      ---
    
      l_related_cust_account_id := p_event.getvalueforparameter('RELATED_CUST_ACCOUNT_ID');
      l_cust_account_id         := p_event.getvalueforparameter('CUST_ACCOUNT_ID');
      l_org_id                  := p_event.getvalueforparameter('ORG_ID');
    
      ---
      handle_acct_relationship(l_cust_account_id,
		       l_related_cust_account_id,
		       l_org_id);
    
      -- END CHG0048217 relationship
    ELSIF g_eventname IN
      ...
      ---CHG0048217 responsibility
    
    ELSIF g_eventname = 'oracle.apps.ar.hz.RoleResponsibility.create' OR
          g_eventname = 'oracle.apps.ar.hz.RoleResponsibility.update' THEN
    ...
    ELSIF g_eventname = 'oracle.apps.ar.hz.PartySite.create' OR
      ...
      --INC0143702 End
    ELSIF g_eventname = 'oracle.apps.ar.hz.CustomerProfile.create' OR -- Added By Roman W. CHG0047450
          g_eventname = 'oracle.apps.ar.hz.CustomerProfile.update' THEN
    
      l_parameter_name  := 'CUST_ACCOUNT_PROFILE_ID';
      l_parameter_value := p_event.getvalueforparameter(l_parameter_name);
    
      handle_account_profile(p_cust_account_profile_id => l_parameter_value);
  ...
  BEGIN
    ...
    FOR rec IN c_price_entry(nvl(p_old_rec.inventory_item_id, 0),
		     ...
		     nvl(p_new_rec.related_item_id, 0))
    LOOP
  ...
  -- 1.1  07-Mar-2019    Diptasurjya                        INC0149408 - Fix cursor query to fix invalid number error from
  --                                                        trigger xxqp_list_lines_biudr_trg1
  -- ----------------------------------------------------------------------------------------
  PROCEDURE generate_related_price_events(p_list_header_id   NUMBER,
    ...
    CURSOR cur_related_price IS
	 ...
	 nvl(to_char(p_inv_item_id), qpa.product_attr_value)
      ...
      AND    qll.list_line_id = qpa.list_line_id
      AND    msib.organization_id = 91
      AND    to_char(msib.inventory_item_id) = qpa.product_attr_value -- INC0149408 swith to_numer to to_char
	 ...
	 mtl_related_items     mri
      ...
      AND    qll.list_line_id = qpa.list_line_id
      AND    msib.organization_id = 91
	--AND    msib.inventory_item_id = to_number(qpa.product_attr_value)  -- INC0149408 commented
      AND    to_char(msib.inventory_item_id) = qpa.product_attr_value -- INC0149408 added
      AND    qpa.product_attribute_context = 'ITEM'
      AND    nvl(qpa.pricing_attribute_context, 'x') = 'x'
      AND    qpa.product_attr_value <> 'ALL'
      AND    mri.relationship_type_id = 5 -- Service Type
	--  AND    mri.inventory_item_id = to_number(qpa.product_attr_value)  -- INC0149408 commented
  ...
  --  1.4  17/01/2019  Lingaraj     CHG0044879 - exclude Trade In lines (quantity <0) from calculation
  --  1.5  5/11/2021   Yuval tal    CHG0049822 - add new 2 events GC account feature, GC printer feature
  ...
  PROCEDURE handle_order_line_event(errbuf        OUT VARCHAR2,
    ...
    CURSOR c_ord IS
	 ...
	 l.ordered_quantity -- v1.4 CHG0044879
      FROM   oe_order_headers_all h,
	 oe_order_lines_all   l
      WHERE  h.header_id = l.header_id
      AND    l.line_id = p_line_id;
    l_activity_analysis VARCHAR2(100);
  ...
  BEGIN
    ...
    LOOP
      ...
      -- CHG0049822
      IF i.flow_status_code = 'CLOSED' THEN
      
        l_activity_analysis := xxssys_oa2sf_util_pkg.get_category_value('Activity Analysis',
						i.inventory_item_id);
      
        CASE l_activity_analysis
          WHEN 'SW-Printer' THEN
          
	-- check GC_PRINTER_FEATURE
	l_xxssys_event_rec             := NULL;
	l_xxssys_event_rec.target_name := g_target_name;
	l_xxssys_event_rec.entity_name := 'GC_PRINTER_FEATURE';
	l_xxssys_event_rec.entity_id   := p_line_id;
	l_xxssys_event_rec.event_name  := p_event_name;
	l_xxssys_event_rec.entity_code := i.entity_code;
	--Insert Event
	xxssys_event_pkg.insert_event(l_xxssys_event_rec, 'N');
          WHEN 'SW-Account' THEN
          
	l_xxssys_event_rec             := NULL;
	l_xxssys_event_rec.target_name := g_target_name;
	l_xxssys_event_rec.entity_name := 'GC_ACC_FEATURE';
	l_xxssys_event_rec.entity_id   := p_line_id;
	l_xxssys_event_rec.event_name  := p_event_name;
	l_xxssys_event_rec.entity_code := i.entity_code;
	--Insert  Event
	...
	NULL;
      ...
      IF i.shipping_status = 'SHIPPED' AND
         ...
         ('Systems (net)', 'Systems-Used', 'BDL-Systems') AND
         i.ordered_quantity > 0 -- v1.4 CHG0044879
       THEN
    ...
    IF l_is_valid = 'Y' THEN
      ...
      CASE
        ...
        WHEN p_event_name = 'MACHINE_SCRAP' THEN
          FOR i IN c_scrap(p_instance_id)
          LOOP
	l_xxssys_event_rec             := NULL;
	l_xxssys_event_rec.target_name := g_target_name;
	l_xxssys_event_rec.entity_name := 'ASSET';
  ...
  BEGIN
    ...
    ELSE
      ...
      FOR component_rec IN sf_bom_options_c1
      LOOP
        fnd_file.put_line(fnd_file.log,
      ...
      --Sync Children
      FOR act_bom_option_rec IN bom_active_components_c
      LOOP
  ...
  BEGIN
    ...
    IF l_valid_structure = 'Y' AND l_category_id IS NOT NULL THEN
      ...
      FOR rec IN c_cpq_category_chg(l_category_id)
      LOOP
  ...
  -- 1.2   4.4.2021      yual tal            CHG0049656 -   include xml entries for  configuration rule generation for  features  "Service Contracts &
  -- --------------------------------------------------------------------------------------------
  ...
  BEGIN
	...
	WHEN OTHERS THEN
	  ...
	  RAISE my_exception;
          ...
          FOR i IN 1 .. l_feature_tab.count()
          LOOP
	...
	--CHG0042203 - CTASK0036202 -- CHG0049656
	--   IF l_feature_tab(i)
	...
	--  END IF; --CHG0042203 - CTASK0036202 -- CHG0049656
  ...
  --  1.2  18/06/2019  Roman W.          INC0160471 - INC0160411 - Machine status for SN T2231 & Sn 570333  didn't update in SFDC
  --  1.3  07-Nov-2019 Adi Safin         CHG0046812 - remove cii.active_end_date IS NULL in cursor query to support machine scrap event
  --------------------------------------------------------------------
  FUNCTION is_asset_valid2sync(p_instance_id NUMBER) RETURN VARCHAR2 IS
    ...
    CURSOR c_valid IS
	  ...
	  -- Added By Roman 18/06/2019 INC0160471
	  UNION ALL
	  SELECT cii.instance_id,
	         cii.serial_number,
	         msib.segment1,
	  ...
	  FROM   csi_item_instances    cii,
	         mtl_system_items_b    msib,
	         xxcs_items_printers_v print
	  WHERE  msib.organization_id = 91
	  AND    cii.inventory_item_id = msib.inventory_item_id
	  AND    cii.inventory_item_id = print.inventory_item_id
	        --  AND cii.active_end_date IS NULL -- CHG0046812
	  AND    cii.serial_number IS NOT NULL
	  AND    instance_id = p_instance_id
	  -- End Added By Roman 18/06/2019 INC0160471
	  )
  ...
  --  1.2  04/03/2019  Lingaraj     INC0148774 - Add sync_daily_rate from xxobjt_oa2sf_interface_pkg
  --------------------------------------------------------------------
  ...
  BEGIN
    ...
    LOOP
      ...
      l_xxssys_event_rec.event_name := 'XXSSYS_STRATAFORCE_EVENTS_PKG.SYNC_DAILY_RATE';
  ...
  -- Ver    When         Who          Description
  -- -----  ----------   -----------  ---------------------------------------------------------------
  -- 1.0    13/06/2019   Roman W.     INC0160471-INC0160411 - workaround, pushing missing SO_LINE_FSL events
  -- 1.1    15/10/2020   yuval tal    CHG0048741 - modify push_so_line_fsl_events proc name and logic  to push_order_line_events
  ---------------------------------------------------------------------------------------------------
  PROCEDURE push_order_line_events(errbuf            OUT VARCHAR2,
		           retcode           OUT VARCHAR2,
		           p_days_back       NUMBER,
		           p_order_source_id NUMBER) IS
    ----------------------------
    --    Local Definition
    ----------------------------
    CURSOR data_cur IS
	 ...
	 oola.line_id,
	 sol.status,
	 lv.meaning oa_status,
	 oola.order_source_id,
	 oola.line_number,
	 h.order_number
      FROM   apps.oe_order_lines_all           oola,
	 apps.oe_order_headers_all         h,
	 productrequestlineitem@source_sf2 sol,
	 apps.fnd_lookup_values            lv
      WHERE  h.header_id = oola.header_id
      AND    oola.line_id = sol.external_key__c
	
      AND    oe_line_status_pub.get_line_status(oola.line_id,
				oola.flow_status_code) !=
	 sol.status
      AND    lookup_type = 'LINE_FLOW_STATUS'
      AND    lookup_code = oola.flow_status_code
      AND    LANGUAGE = 'US'
      AND    oola.creation_date < SYSDATE - 1 / 24
      AND    h.order_source_id = 1081
      AND    NOT EXISTS (SELECT 1
	  FROM   apps.xxssys_events xe
	  WHERE  1 = 1
	  AND    entity_name = 'SO_LINE_FSL'
	  AND    target_name = 'STRATAFORCE'
	  AND    xe.status IN ('ERR', 'NEW') -- add new CHG0048741
	  AND    xe.entity_id = oola.line_id)
      
      ;
  
    ---CHG0048741
    CURSOR c_miss_order_line IS
    
      SELECT SYSDATE cur_sysdate,
	 sol.id,
	 oola.last_update_date,
	 oola.line_id,
	 sol.oracle_status__c,
	 lv.meaning oa_status,
	 oe_line_status_pub.get_line_status(oola.line_id,
				oola.flow_status_code) oa_status2,
	 h.order_source_id,
	 oola.line_number,
	 h.order_number
      FROM   apps.oe_order_lines_all   oola,
	 apps.oe_order_headers_all h,
	 orderitem@source_sf2      sol,
	 apps.fnd_lookup_values    lv
      WHERE  h.header_id = oola.header_id
      AND    oola.line_id = to_number(sol.external_key__c)
      AND    oe_line_status_pub.get_line_status(oola.line_id,
				oola.flow_status_code) !=
	 sol.oracle_status__c
      AND    lookup_type = 'LINE_FLOW_STATUS'
      AND    lookup_code = oola.flow_status_code
      AND    LANGUAGE = 'US'
      AND    oola.last_update_date BETWEEN SYSDATE - nvl(p_days_back, 14) AND
	 SYSDATE - (0.75 / 24)
      AND    h.order_source_id = nvl(p_order_source_id, h.order_source_id) --1061 --strataforce 1041 -- ecomm -- 1081 fsl
      AND    NOT EXISTS
       (SELECT 1
	  FROM   apps.xxssys_events xe
	  WHERE  1 = 1
	  AND    entity_name = 'SO_LINE'
	  AND    target_name = 'STRATAFORCE'
	  AND    xe.status IN ('NEW', 'ERR')
	  AND    xe.entity_id = oola.line_id
	  AND    xe.event_id =
	         (SELECT MAX(x.event_id)
	           FROM   apps.xxssys_events x
	           WHERE  x.entity_name = 'SO_LINE'
	           AND    x.target_name = 'STRATAFORCE'
	           AND    x.entity_id = oola.line_id));
  ...
  BEGIN
    ...
    retcode := '0';
    errbuf  := NULL;
    fnd_file.put_line(fnd_file.log, 'Looking for SO_LINE_FSL.....');
    FOR i IN data_cur
    LOOP
      l_xxssys_event_rec := NULL;
    
      l_xxssys_event_rec.entity_name := 'SO_LINE_FSL';
      -- l_xxssys_event_rec.event_name  := 'SO_LINE_UPDATE';
      l_xxssys_event_rec.entity_id   := i.line_id;
      l_xxssys_event_rec.entity_code := i.order_number || '-' ||
			    i.line_number;
      l_xxssys_event_rec.target_name := 'STRATAFORCE';
      l_xxssys_event_rec.attribute2  := 'PRODUCT_REQUEST_LINE';
      l_xxssys_event_rec.event_name  := 'xxssys_strataforce_events_pkg.push_order_line_events';
    ...
    fnd_file.put_line(fnd_file.log, 'SO_LINE_FSL :Count =' || l_cnt);
    COMMIT;
  
    -----CHG0048741
    fnd_file.put_line(fnd_file.log, 'Looking for SO_LINE.....');
    l_cnt := 0;
    FOR i IN c_miss_order_line
    LOOP
    ...
    fnd_file.put_line(fnd_file.log, 'SO_LINE :Count =' || l_cnt);
  
    ----- end CHG0048741
  
  EXCEPTION
    WHEN OTHERS THEN
      errbuf  := 'EXCEPTION xxssys_event_pkg.push_order_line_events ' ||
	     SQLERRM;
      retcode := '2';
  ...
  -- 2.8  27/03/2020  Roman W.    CHG0047450 - Account, Contact interface - ReDo
  ---------------------------------------------------------------------------------------
  PROCEDURE insert_account_addr_event(p_cust_acct_site_id IN NUMBER,
			  p_party_site_number IN VARCHAR2 DEFAULT NULL,
			  p_last_updated_by   IN NUMBER /* USER_ID */,
			  p_created_by        IN NUMBER /* USER_ID */,
			  p_trigger_name      IN VARCHAR2,
			  p_trigger_action    IN VARCHAR2 /* INSERT / UPDATE / DELETE */) IS
    l_xxssys_event_rec xxssys_events%ROWTYPE;
    l_entity_code      VARCHAR2(240);
  BEGIN
    IF 'Y' = is_account_addr(p_cust_acct_site_id) THEN
      IF p_party_site_number IS NULL THEN
      
        BEGIN
        
          SELECT xsd.oracle_site_number
          INTO   l_entity_code
          FROM   xxhz_site_dtl_v xsd
          WHERE  xsd.oracle_site_id = p_cust_acct_site_id;
	...
	l_entity_code := '';
      ...
      ELSE
        l_entity_code := p_party_site_number;
      END IF;
    
      l_xxssys_event_rec                 := NULL;
      l_xxssys_event_rec.target_name     := g_target_name;
      l_xxssys_event_rec.entity_name     := g_account_addr_entity_name;
      l_xxssys_event_rec.entity_id       := p_cust_acct_site_id;
      l_xxssys_event_rec.entity_code     := l_entity_code;
      l_xxssys_event_rec.last_updated_by := p_last_updated_by;
      l_xxssys_event_rec.created_by      := p_created_by;
      l_xxssys_event_rec.event_name      := p_trigger_name || '(' ||
			        p_trigger_action || ')';
      ...
      -- write_log('ACCOUNT_ADDR trigger event inserted');
    END IF;
  
  END insert_account_addr_event;
  ---------------------------------------------------------------------------------
  -- Ver   When        Who         Descr
  -- ----  ----------  ----------  ------------------------------------------------
  -- 1.0   31/03/2020  Roman W.    CHG0047450 - Account, Contact interface - ReDo
  --                                 DFF Title       : JG_HZ_CUST_ACCT_SITES
  --                                     Application : Regional Localizations
  ---------------------------------------------------------------------------------
  FUNCTION is_account_addr(p_cust_acct_site_id NUMBER) RETURN VARCHAR2 IS
    ---------------------------
    --    Local Definition
    ---------------------------
  ...
  BEGIN
    ...
    SELECT nvl(xsd.account_address_flag, 'N')
    INTO   l_ret_value
    FROM   xxhz_site_dtl_v xsd
    WHERE  xsd.oracle_site_id = p_cust_acct_site_id;
