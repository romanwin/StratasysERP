CREATE OR REPLACE PACKAGE BODY xxssys_oa2sf_util_pkg IS
  ...
  -- 1.7 29-Jan-19            Lingaraj            CHG0044879 - Change in SO header interface
  --                                              Function Modifiec - get_order_complete_ship_date() & get_systems_ship_date()
  -- 1.8 25-Mar-2019         Roman Winer          CHG0045353 - New function created to get sf_okc_header_ID
  -- 1.9 10/06/2019          Roman W              INC0159273-INC0159948 - Failed to debrief due to interface issue / VD
  -- 2.0 17.7.19             yuval tal            INC0163675 - modify get_sf_fsl_header_id - performence issues
  -- 2.1 01/12/2019          Roman W.             CHG0046929 ORACLE-SF Add Oracle Order Creator field
  -- 2.2 11/08/2020          yuval tal            INC0201353 modify get_sf_account_id :  - create sfdc  Account table  in xxcstorm.account
  -- 2.3 22/2/2021           yuval tal            CHG0048217 - modify get_state_name b2b ecomm modify 
  --------------------------------------------------------------------

  --------------------------------------------------------------------
  --  name:            get_sf_okc_header_id
  --  create by:       Roman Winer
  --  Revision:        1.0
  --  creation date:   25-Mar-2019
  --------------------------------------------------------------------
  --  purpose :        CHG0045353
  --                   New function created to get sf_okc_header_ID
  --------------------------------------------------------------------
  --  ver  date           name                  desc
  --  1.0  25-Mar-2019    Roman Winer      initial version
  --------------------------------------------------------------------
  FUNCTION get_sf_okc_header_id(p_okc_header_id IN NUMBER) RETURN VARCHAR2 IS
    l_sf_id VARCHAR2(18);
  BEGIN
    ...
    AND    xe.entity_name = 'SO_HEADER_OKC'
    AND    xe.status = 'SUCCESS'
    AND    xe.entity_id = p_okc_header_id
  ...
  EXCEPTION
    WHEN no_data_found THEN
      ...
      BEGIN
        ...
        INTO   l_sf_id
        FROM   xxsf2_order
        WHERE  external_key__c = to_char(p_okc_header_id);
  ...
  BEGIN
    ...
    IF p_operatingunit_id IS NOT NULL THEN
      ...
      WHERE  sf_ou.external_key__c = to_char(p_operatingunit_id); --external_key__c is NUMBER in SF[Table : OPERATING_UNIT__C]
    END IF;
  ...
  EXCEPTION
    WHEN no_data_found THEN
      ...
      BEGIN
        ...
        INTO   l_sf_pricebook_id
        FROM   xxsf2_pricebook2 pricebook
        WHERE  pricebook.external_key__c =
	   to_char(p_pricebook_listheader_id);
  ...
  --  1.1  24/07/2019     Roman Winer           CHG0045254(CTASK0042890)-To reduce performance issue
  --------------------------------------------------------------------
  FUNCTION get_concatenated_segments(p_inventory_item_id IN NUMBER)
    RETURN VARCHAR2 IS
    CURSOR c_category_segments(p_org_id NUMBER) IS
	 ...
	 fnd_flex_values           ffv_fmly
      ...
      AND    mic.organization_id = p_org_id --CHG0045254
	--xxinv_utils_pkg.get_master_organization_id            --CHG0045254
  ...
  BEGIN
    ...
    l_org_id                 := xxinv_utils_pkg.get_master_organization_id; --CHG0045254
    FOR rec IN c_category_segments(l_org_id)
    LOOP
      --CHG0045254
  ...
  --  ver  date           name                  desc
  --  1.0  15-Nov-2017    Lingaraj Sarangi      initial version
  -- 2.2  /08/2020         yuval tal            INC0201353 fetch from  sfdc  Account table in xxobjt which sync from copyStorm
  --------------------------------------------------------------------
  ...
  BEGIN
    ...
    SELECT id
    INTO   l_account_id
    FROM   xxsf2_account_local --xxobjt.account/xxcstorm.account in prod 
  ...
  BEGIN
    ...
    END IF;
    --End INC0130190
    FOR i IN c(l_inventory_item_id)
    LOOP
  ...
  -- 1.1    10/06/2019   Roman W       INC0159273 - Failed to debrief due to interface issue / VD
  ----------------------------------------------------------------------------------------------
  FUNCTION get_sf_fsl_so_header_id(p_so_header_id    IN NUMBER,
		           p_order_source_id IN NUMBER)
    RETURN VARCHAR2 IS
    l_sf_id            VARCHAR2(18);
    l_fsl_order_source VARCHAR2(300);
    l_fsl_attribute2   VARCHAR2(300);
  BEGIN
    ...
    AND    rownum = 1;
    -- added by roman w. 10/06/2019 inc0159273
  
    RETURN l_sf_id;
  EXCEPTION
    WHEN no_data_found THEN
      l_fsl_order_source := xxssys_strataforce_events_pkg.get_order_source_name(p_order_source_id);
      ...
      l_sf_id := get_sf_fsl_header_id(p_header_id             => p_so_header_id,
			  p_sync_destination_code => l_fsl_attribute2);
  ...
  --  1.1  22/2//2021     yuval tal             CHG0048217 - change logic , func will be used for all sfdc views 
  --------------------------------------------------------------------
  ...
  BEGIN
    ...
    BEGIN
      ...
      WHERE  p.lookup_type = 'XXSERVICE_COUNTRIES_SECURITY'
      AND    attribute4 = 'Y'
    ...
    CASE
      WHEN l_state_flag = 'Y' THEN
        ...
        AND    (geography_code = p_state_code OR
	  geography_name = p_state_code) -- chg0048217
        AND    rownum = 1;
      WHEN p_country_code = 'US' THEN
        l_state_name := xxobjt_general_utils_pkg.get_lookup_meaning('US_STATE',
					        p_state_code,
					        'N');
  ...
  --  1.1 17/07/2019       yuval tal            INC0163675           -- add to_char
  --------------------------------------------------------------------
  ...
  BEGIN
    IF p_sync_destination_code = 'PRODUCT_REQUEST_HEADER' THEN
      ...
      WHERE  external_key__c = to_char(p_header_id); --INC0163675
  ...
  --                                            Update "Complete Order Shipped Date" and "Systems Shipped Date"
  --  1.1  29-Jan-19      Lingaraj              CHG0044879 - Change in SO header interface
  --------------------------------------------------------------------
  FUNCTION get_order_complete_ship_date(p_header_id IN NUMBER) RETURN DATE IS
    --Cursor Code modified same as get_systems_ship_date #CHG0044879
    CURSOR ord_cur IS
	 ...
	 wsh_delivery_details wdd
      ...
      AND    nvl(ol.line_category_code, 'X') != 'RETURN';
  ...
  FUNCTION get_systems_ship_date(p_header_id IN NUMBER) RETURN DATE IS
    CURSOR ord_cur IS
	 ...
	 ('Systems (net)', 'Systems-Used', 'BDL-Systems')
      AND    nvl(ol.cancelled_flag, 'N') != 'Y'
      AND    nvl(ol.ordered_quantity, 0) > 0
      AND    nvl(ol.line_category_code, 'X') != 'RETURN'; --line_category_code Added #CHG0044879
  ...
  -- Ver    When         Who              Description
  -- -----  -----------  ---------------  ----------------------------------------
  -- 1.0    01/12/2019   Roman W.         CHG0046929 ORACLE-SF Add Oracle Order Creator field
  --------------------------------------------------------------------------------
  FUNCTION get_sf_user_id_by_email(p_user_name     VARCHAR2,
  ...
  BEGIN
    ...
    IF 'oradev@stratasys.com' = p_email_address THEN
      l_email_address := lower(p_user_name) || '@stratasys.com.invalid';
    ELSE
    ...
    IF l_email_address IS NOT NULL THEN
      SELECT id
      INTO   l_sf_id
      FROM   user2@source_sf2 sf_u
      WHERE  sf_u.email = lower(l_email_address);
