CREATE OR REPLACE PACKAGE BODY xxhz_soa_api_pkg IS
  ...
  * 1.6      13.3.19     yuval tal          INC0150448  upsertContact /get_usa_state_code_or_name change logic /create location
  * 1.7      25.3.19     yuval tal          INC0151620 - modify get_territory_org_info/get_territory_code logic
  * 1.8      25.06.19    Lingaraj           CHG0046009 - Apps Session initialize corrected to restrict event generation
  *                      yuval tal          CHG0046009 upsert contact  , incase adress is null replace with .
  * 1.9      21/01/2020  Roman W.           INC0180948
  * 1.4      31/03/2020  Roman W.           CHG0047450 - Account, Contact interface - ReDo
  * 1.5      05/04/2020  Roman W.           CHG0047450 - xxhz_contact_rec added NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
  * 1.6      21/04/2020  Roman W.           CHG0047450 - Check duplicate
  *                                                1) Upsert_account
  *                                                2) Upsert_contact
  *                                                3) Upsert_Site'
  * 1.7      24/11/2020  Roman W.           CHG0047450 update_account_role - added handling to ATTRIBUTE6 ( PARTNER_USER )
  *                                                    upsert_contact in update mode added handling to ATTRIBUTE6 ( PARTNER_USER )
  * 1.8      25/11/2020  Roman W.           CHG0047450 - added to upsert_site : cust_site_use_rec.location := substr(l_sites_out_rec.account_number || '-' || l_sites_out_rec.city, 1,40)
  * 2.2      19/01/2021  Roman W.           CHG0047450 - ReDo
  * 2.3      04/02/2021  Roman W.           CHG0047450 - ReDo ( added NULL handling to get_usa_state_code_or_name)
  *******************************************************************************************************************************************/
  --g_user_name          VARCHAR2(100) := 'SALESFORCE';
  -- g_resp_name          VARCHAR2(200) := 'AR Super User, SSYS'; Rem By Roman 21/01/2020 INC0180948 --'Oracle Customer Data Librarian Superuser'; -- CHG0040057 - By Adi Safin, No S3,No CDH
  g_resp_name          VARCHAR2(200) := 'XXAR_SUPER_USER_SSUS'; -- Added By Roman 21/01/2020 INC0180948
  ...
  --g_is_session_set     BOOLEAN := FALSE; --CHG0046009 Commented 25.JUN.19
  g_user_id         NUMBER; --CHG0046009 Added 25.JUN.19
  ...
  -- Ver   When         Who         Descr
  -- ----  -----------  ----------  ---------------------------------------
  -- 1.0   16/12/2020   Roman W.    CHG0047450
  -------------------------------------------------------------------------
  procedure message(p_str in VARCHAR2) is
  begin
  ...
  -- Ver   When         Who         Descr
  -- ----  -----------  ----------  ---------------------------------------
  -- 1.0   16/12/2020   Roman W.
  -------------------------------------------------------------------------
  procedure get_contact_name_local_lang(p_new_name_local_lang       IN VARCHAR2,
                                        p_new_first_name_local_lang OUT VARCHAR2,
                                        p_new_last_name_local_lang  OUT VARCHAR2,
                                        p_api_status                OUT VARCHAR2, --OUT Param
  ...
  begin
    ...
    l_str := trim(p_new_name_local_lang);
  
    p_new_first_name_local_lang := trim(nvl(REGEXP_SUBSTR(l_str, '[^ ]+'),
                                            fnd_api.g_null_char));
  
    p_new_last_name_local_lang := trim(nvl(substr(l_str,
                                                  length(p_new_first_name_local_lang) + 1),
                                           fnd_api.g_null_char));
  
  exception
    when others then
      p_api_status := fnd_api.G_RET_STS_ERROR;
      p_error_msg  := 'EXCEPTION_OTHERS xxhz_soa_api_pkg.get_contact_name_local_lang(' ||
                      p_new_name_local_lang || ') - ' || sqlerrm;
    
  end get_contact_name_local_lang;

  --------------------------------------------------------------------------------
  -- Ver   When         Who         Description
  -- ----  -----------  ----------  ----------------------------------------------
  -- 1.0   14/12/2020   Roman W.
  --------------------------------------------------------------------------------
  procedure get_job_title_code(p_in_title IN varchar2,
                               --p_out_title      OUT varchar,
                               p_out_title_code OUT varchar,
  ...
  begin
    ...
    if trim(p_in_title) is not null then
      select --meaning,
       lookup_code
        into --p_out_title,
             p_out_title_code
       ...
       where lookup_type = 'RESPONSIBILITY'
         ...
         and upper(meaning) = upper(trim(p_in_title));
    else
      p_out_title_code := null;
    end if;
  
    write_log_message(' p_out_title_code = ' || p_out_title_code);
  
  exception
    when no_data_found then
      p_error_code := 2;
      p_error_desc := 'EXCEPTION_NO_DATA_FOUND xxhz_soa_api_pkg.get_job_title_code(' ||
                      p_in_title || ') - JOB_TITLE Not found';
    
      write_log_message(p_error_desc);
    
    when OTHERS then
      p_error_code := 2;
      p_error_desc := 'EXCEPTION_OTHERS xxhz_soa_api_pkg.get_job_title_code(' ||
                      p_in_title || ') - ' || sqlerrm;
    
      write_log_message(p_error_desc);
    
  end get_job_title_code;
  ...
  * 1.00     26/02/2017  Lingaraj Sarangi   Initial version
   ...
   --1.2     25.3.19     yuval tal          INC0151620 - remove substr
    ...
    BEGIN
      SELECT territory_code
       ...
       WHERE (upper(territory_short_name) = upper(p_country) OR
             territory_code = upper(p_country))
         AND rownum = 1; -- CHG0042632;
    EXCEPTION
      WHEN no_data_found THEN
        x_error_msg := 'Invalid Country code :' || p_country ||
                       '. Please use the full Country name or 2 character abbreviation';
  ...
  * 1.2      25.06.19    Lingaraj           CHG0046009 - Apps Session initialize corrected to restrict event generation
  * 1.3      21/01/2019  Roman W.           INC0180948
  ******************************************************************************************************************************************/
  ...
  BEGIN
    ...
    /*(CASE
      ...
      ELSE
       p_source
    END);*/ -- CHG0046009 SALESFORCE System is absolute
    /* CHG0042044 - Dipta Start fetching details based on source */
    BEGIN
      SELECT attribute2, attribute6
        INTO l_request_source_user, g_created_by_module
    ...
    SELECT fuser.user_id, frv.application_id, frv.responsibility_id
      INTO l_user_id, l_resp_appl_id, l_resp_id
      FROM fnd_user              fuser,
     ...
     WHERE 1 = 1
       ...
       AND fuser.user_id = l_request_source_user -- CHG0042044 - Dipta added
          -- AND    frv.responsibility_name = g_resp_name -- INC0180948
       AND frv.RESPONSIBILITY_KEY = g_resp_name; -- INC0180948
  
    g_user_id := l_user_id; --CHG0046009 Added 25.JUN.19
    --Initialize Apps
    initialize_apps(l_user_id, l_resp_appl_id, l_resp_id);
  
    --g_is_session_set := TRUE;--CHG0046009 Commented 25.JUN.19
  ...
  --1.1  25.3.19     yuval tal                       INC0151620 - remove substr
  -- --------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    SELECT territory_code
     ...
     WHERE (upper(territory_short_name) = upper(p_country) OR
           territory_code = upper(p_country))
       AND rownum = 1;
    --upper(territory_short_name) = substr(upper(p_country), 1, 2); -- yuval
  ...
  PROCEDURE find_match_accounts(p_source           IN VARCHAR2,
    ...
    CURSOR party_cur(p_search_context_id IN NUMBER) IS
      SELECT hp.party_name,
        ...
        FROM hz_matched_parties_gt hmpg, hz_parties hp
    ...
    FOR r IN party_cur(l_return_id) LOOP
      ...
      BEGIN
        SELECT cust_account_id, account_name, account_number
          INTO l_cust_account_id, l_account_name, l_account_number
  ...
  ******************************************************************************************************************************************/
  ...
  PROCEDURE find_match_sites_old(p_source           IN VARCHAR2,
    ...
    CURSOR party_site_cur(p_ctx_id IN NUMBER, p_party_id IN NUMBER) IS
      SELECT *
  ...
  BEGIN
    ...
    IF p_sites_in.account_id IS NOT NULL THEN
      BEGIN
        SELECT account_number, party_id
          INTO l_acct_number, l_party_id
          FROM hz_cust_accounts_all
  ...
  BEGIN
    ...
    ELSE
      BEGIN
        SELECT cust_account_id, account_number, party_id
          INTO l_cust_account_id, l_acct_number, l_party_id
    ...
    if p_sites_in.site_address is null then
      p_status  := fnd_api.g_ret_sts_error;
      p_message := 'Field : { Address Name } can''t be empty';
      RAISE custom_err_exp;
    end if;
  ...
  * 1.00     11/05/2016  Somnath Dawn        initial version
  -- 1.1     16.1.19     yuval tal           CHG0042632 US state name logic change
     1.2     13.3.19     yuval tal           INC0150448    change logic
  ******************************************************************************************************************************************/
  PROCEDURE create_location(p_location_record IN OUT hz_location_v2pub.location_rec_type,
    ...
    l_state_code  VARCHAR2(20) := p_location_record.state; --INC0150448
  BEGIN
    ...
    -----------------------------------------
    -- dbms_output.put_line('country=' || p_location_record.country);
    IF p_location_record.country = 'US' THEN
      --  l_state_code := l_state_name; -- yuval 16.1.19  --INC0150448
      --  l_state_name := NULL; --- yuval  16.1.19  --INC0150448
  ...
  ******************************************************************************************************************************************/
  ...
  EXCEPTION
    WHEN OTHERS THEN
      p_api_status := 'E';
      p_error_msg  := 'EXCEPTION_OTHERS xxhz_soa_api_pkg.create_party_site() - ' ||
                      SQLERRM;
  END create_party_site;
  -----------------------------------------------------------------------------------------
  -- Ver   When         Who         Descr
  -- ----  -----------  ----------  -------------------------------------------------------
  -- 1.0   28/12/2020   Roman W.    CHG0047450 - Added to account
  -----------------------------------------------------------------------------------------
  PROCEDURE update_party_site(p_party_site_record IN hz_party_site_v2pub.party_site_rec_type,
                              p_api_status        OUT VARCHAR2,
                              p_error_msg         OUT VARCHAR2) IS
    ----------------------------
    --   Local Definitioin
  ...
  BEGIN
    ...
    select hps.object_version_number
     ...
     where hps.party_site_id = p_party_site_record.party_site_id;
    ...
    IF fnd_api.G_RET_STS_SUCCESS = l_return_status THEN
    
      FOR I IN 1 .. l_msg_count LOOP
        p_error_msg := p_error_msg || CHR(10) ||
                       SUBSTR(FND_MSG_PUB.Get(p_encoded => FND_API.G_FALSE),
                              1,
                              255);
      END LOOP;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      p_api_status := 'E';
      p_error_msg  := 'EXCEPTION_OTHERS xxhz_soa_api_pkg.update_party_site() - ' ||
                      SQLERRM;
  ...
  * 1.5      21/04/2020  Roman W.           CHG0047450 - check duplicate
  * 1.6      10/05/2020  Roman W.           CHG0047450 - added 4 extra null to initialization xxhz_accounts_rec
  * 1.7      11/05/202   Roman W.           CHG0047450 - added : DEFAULT_DIRECT_PRICELIST
  * 1.8      12/05/2020  Roman W            CHG0047450 - added check duplicate flag
  * 1.9      24/12/2020  Roman W.           CHG0047450 - Added to account "UPDATE"
  *                                                     l_cust_account_record.attribute15   := p_account_in(i).default_direct_pricelist; -- added by Roman W. 24/12/2020 CHG0047450 DEFAULT_DIRECT_PRICELIST
  *                                                     l_cust_account_record.price_list_id := p_account_in(i).default_pricelist;        -- added by Roman W. 24/12/2020 CHG0047450 DEFAULT_PRICELIST
  ******************************************************************************************************************************************/
  PROCEDURE upsert_account(p_source           IN VARCHAR2,
    ...
    l_soa_api_dup_chk_rec  xxhz_soa_api_dup_chk%ROWTYPE; -- CHG0047450
    l_check_duplicate_flag varchar2(10);
    l_hw_payment_terms_c   varchar2(30);
    l_sf_payment_term_id   varchar2(30);
  BEGIN
    ...
    FOR i IN 1 .. p_account_in.count LOOP
      ...
      l_cust_out_rec                 := xxhz_accounts_rec(NULL,
                                                          ...
                                                          NULL,
                                                          NULL,
                                                          NULL); --31 Variables  -- CHG0047450 added extra null
      ...
      */
      ...
      l_cust_out_rec.source_reference_id        := p_account_in(i)
                                                   .source_reference_id; -- CHG0047450 added by Roman W.
      ...
      l_org_record.organization_name_phonetic := p_account_in(i)
                                                 .account_name_local;
      ----------------------------------------------------------------------------------
      -- added by Roman W. CHG0047450 21/04/2020 check duplicate
      ----------------------------------------------------------------------------------
      if 'Y' = C_CHECK_DUPLICATE and p_account_in(i).account_id IS NULL then
        l_check_duplicate_flag := 'Y';
      end if;
    
      if 'Y' = l_check_duplicate_flag then
        l_soa_api_dup_chk_rec.source_name := p_source;
        l_soa_api_dup_chk_rec.object_name := 'ACCOUNT';
        l_soa_api_dup_chk_rec.source_id   := p_account_in(i)
                                             .source_reference_id;
      
        xxhz_soa_api_dup_chk_pkg.insert_trx(p_rec => l_soa_api_dup_chk_rec);
      
        if 'IN_PROCESS' = l_soa_api_dup_chk_rec.status then
          -- error handaling;
          l_main_status := fnd_api.g_ret_sts_error;
          l_main_err_msg := C_DUP_CHK_IN_PROCESS_MSG;
          l_cust_out_rec.error_code := fnd_api.g_ret_sts_error; -- If Return Value U, then Also the Value Returned in E
          l_cust_out_rec.error_msg := C_DUP_CHK_IN_PROCESS_MSG;
          l_cust_out_tab(l_cust_out_tab.count) := l_cust_out_rec;
          CONTINUE; --Skip to the Next Account Record
        
        elsif l_soa_api_dup_chk_rec.status in ('CREATED', 'ERROR') then
          NULL;
          p_account_in(i).account_id := l_soa_api_dup_chk_rec.cust_account_id;
          p_account_in(i).account_number := l_soa_api_dup_chk_rec.account_number;
          p_account_in(i).oe_email_contact_point_id := l_soa_api_dup_chk_rec.email_contact_point_id;
          p_account_in(i).oe_fax_contact_point_id := l_soa_api_dup_chk_rec.fax_contact_point_id;
          p_account_in(i).oe_mobile_contact_point_id := l_soa_api_dup_chk_rec.mobile_contact_point_id;
          p_account_in(i).oe_phone_contact_point_id := l_soa_api_dup_chk_rec.phone_contact_point_id;
          --p_rec.account_number,
        end if;
      end if;
      ----------------------------------------------------------------------------------
      -- end added by Roman W. CHG0047450 21/04/2020 check duplicate
      ----------------------------------------------------------------------------------
      ...
      IF p_account_in(i).account_id IS NULL THEN
        ...
        */
        ...
        ELSE
          ...
          EXCEPTION
            WHEN OTHERS THEN
              ...
              l_soa_api_dup_chk_rec.source_id   := l_cust_out_rec.source_reference_id; -- Added By Roman W. CHG0047450 10/12/2020
              l_soa_api_dup_chk_rec.source_name := 'STRATAFORCE'; -- Added By Roman W. CHG0047450 10/12/2020
              l_soa_api_dup_chk_rec.object_name := 'ACCOUNT'; -- Added By Roman W. CHG0047450 10/12/2020
            
              xxhz_soa_api_dup_chk_pkg.update_to_error_trx(p_rec => l_soa_api_dup_chk_rec);
        ...
        l_cust_account_record.attribute15   := p_account_in(i)
                                               .default_direct_pricelist; -- added by Roman W. 11/05/2020 CHG0047450 DEFAULT_DIRECT_PRICELIST
        l_cust_account_record.price_list_id := p_account_in(i)
                                               .default_pricelist; -- added by Roman W. 11/05/2020 CHG0047450 DEFAULT_PRICELIST
      
        get_sf_payment_terms(p_hw_payment_terms_c => p_account_in(i)
                                                     .hw_payment_terms_c,
                             p_ret_value          => l_hw_payment_terms_c,
                             p_error_code         => x_status,
                             p_error_desc         => x_message);
      
        if 'S' != x_status then
          l_main_status  := fnd_api.g_ret_sts_error;
          l_main_err_msg := x_message;
        ...
        l_cust_account_record.attribute7 := l_hw_payment_terms_c;
      
        get_sf_payment_term_id(p_sf_payment_term_id => p_account_in(i)
                                                       .SF_PAYMENT_TERM_ID,
                               p_ret_value          => l_sf_payment_term_id,
                               p_error_code         => x_status,
                               p_error_desc         => x_message);
        ...
        l_cust_prof_record.standard_terms := l_sf_payment_term_id; -- added by Roman W. 12/05/2020 CHG0047450 SF_PAYMENT_TERM_ID
        ...
        IF l_api_status != fnd_api.g_ret_sts_success THEN
          ...
          l_soa_api_dup_chk_rec.source_name := 'STRATAFORCE'; -- Added By Roman W. CHG0047450 10/12/2020
          l_soa_api_dup_chk_rec.object_name := 'ACCOUNT'; -- Added By Roman W. CHG0047450 10/12/2020
          xxhz_soa_api_dup_chk_pkg.update_to_error_trx(p_rec => l_soa_api_dup_chk_rec); -- Added By Roman W. CHG0047450 10/12/2020
        
          CONTINUE; --Skip to the Next Account Record
        ELSE
        
          l_cust_out_rec.account_id := l_cust_account_id; -- CHG0047450
        
          BEGIN
            SELECT account_number, status
              INTO l_account_number, l_account_status
              FROM hz_cust_accounts_all
        ...
        END IF;
        l_cust_out_rec.account_id     := l_cust_account_id;
        l_cust_out_rec.account_number := p_account_in(i).duns_number;
        -------------------------  update_trx  -------------------------------------------
        -- added by Roman W. CHG0047450 21/04/2020 check duplicate
        ----------------------------------------------------------------------------------
        if 'Y' = C_CHECK_DUPLICATE then
        
          l_soa_api_dup_chk_rec.source_id               := l_cust_out_rec.source_reference_id;
          l_soa_api_dup_chk_rec.source_name             := 'STRATAFORCE';
          l_soa_api_dup_chk_rec.object_name             := 'ACCOUNT';
          l_soa_api_dup_chk_rec.cust_account_id         := l_cust_out_rec.account_id;
          l_soa_api_dup_chk_rec.account_number          := l_cust_out_rec.account_number;
          l_soa_api_dup_chk_rec.email_contact_point_id  := l_cust_out_rec.oe_email_contact_point_id;
          l_soa_api_dup_chk_rec.fax_contact_point_id    := l_cust_out_rec.oe_fax_contact_point_id;
          l_soa_api_dup_chk_rec.mobile_contact_point_id := l_cust_out_rec.oe_mobile_contact_point_id;
          l_soa_api_dup_chk_rec.phone_contact_point_id  := l_cust_out_rec.oe_phone_contact_point_id;
        
          xxhz_soa_api_dup_chk_pkg.update_trx(p_rec => l_soa_api_dup_chk_rec);
      ...
      ELSE
        ...
        l_cust_account_record.attribute15   := p_account_in(i)
                                               .default_direct_pricelist; -- added by Roman W. 24/12/2020 CHG0047450 DEFAULT_DIRECT_PRICELIST
        l_cust_account_record.price_list_id := p_account_in(i)
                                               .default_pricelist; -- added by Roman W. 24/12/2020 CHG0047450 DEFAULT_PRICELIST
          ...
          BEGIN
            SELECT hp.object_version_number,
              ...
              INTO l_object_version_num,
                   ...
                   l_account_number,
                   l_account_status
              FROM hz_cust_accounts_all hcaa, hz_parties hp
  ...
  * Purpose             :  This procedure will be called through SOA integration to create/update the Contacts
  ...
  --  1.2     13.3.19     yuval tal          INC0150448  set oe_location_id
  --  1.3     14.07.19    yuval tal          CHG0046009 upsert contact  , incase adress is null replace with .
  --  1.4     05/04/2020  Roman W.           CHG0047450 - Account, Contact interface - ReDo
  --  1.5     21/04/2020  Roman W.           CHG0047450 - check duplicate
  --  1.6     12/05/2020  Roman W.           CHG0047450 - adding che3ck duplicate flag
  --  1.7     24/11/2020  Roman W.           CHG0047450 - added PARTNER_USER handling in update mode -> rec_cust_acc_role.attribute6 := l_contact_out_rec.partner_user;
  --  1.8     14/12/2020  Roman W.           CHG0047450 - xxhz_soa_api_dup_chk_pkg ignored status
  --  1.9     15/12/2020  Roman W.                        person_rec.attribute7 := nvl(p_contacts_in(i)
  --                                                                                  .CONTACT_NAME_LOCAL_LANGUAGE,
  --                                                                                   fnd_api.g_null_char); -- Added By Roman W 15/12/2020 CHG0047450
  --  2.0     16/12/2020  Roman W.                      - added procedure "get_contact_name_local_lang", called from "upsert_contact"
  --  2.1     28/12/2020  Roman W.           CHG0047450 -
  --  2.2     19/01/2021  Roman W.           CHG0047450 - ReDo
  ******************************************************************************************************************************************/
  PROCEDURE upsert_contact(p_source           IN VARCHAR2,
    ...
    -- user_not_valid_exp EXCEPTION;
    l_soa_api_dup_chk_rec  xxhz_soa_api_dup_chk%ROWTYPE; -- CHG0047450
    l_check_duplicate_flag varchar2(10); -- CHG0047450
  
    l_new_first_name_local_lang varchar2(120);
    l_new_last_name_local_lang  varchar2(120);
  ...
  BEGIN
    ...
    --set_global_variables(p_source);
    --Get the User details & initilize Apps for Processing the Record
    --IF g_is_session_set = FALSE THEN --CHG0046009 Commented 25.JUN.19
    set_my_session( --p_username       => NULL, --In Param -- CHG0042044 - Dipta commented
                   p_source         => p_source, -- CHG0042044 - Dipta added
                   p_status_out     => p_status, --Out Param
                   p_status_msg_out => p_message --Out Param
    ...
    FOR i IN 1 .. l_record_count LOOP
      l_check_duplicate_flag       := 'N'; -- CHG0047450
      l_main_status                := fnd_api.g_ret_sts_success;
      person_rec                   := NULL;
      location_rec                 := NULL;
      party_site_rec               := NULL;
      ...
      l_contact_out_rec            := xxhz_contact_rec(NULL,
                                                       ...
                                                       NULL, -- CHG0047450
                                                       NULL, -- CHG0047450
                                                       NULL -- CHG0047450 last_Deactivation_reason added 01/01/2020
                                                       ); -- 40 variables CHG0047450 added extra null
      l_contact_out_rec            := p_contacts_in(i);
      l_contact_out_tab.extend;
      /* Rem by Roman W. CHG0047450 28/12/2020
      \* CHG0042044 - Start Validate account_ID and account_number - Dipta*\
      ...
      */
      ...
      get_job_title_code(p_contacts_in(i).title,
                         --org_contact_rec.title,
                         org_contact_rec.job_title_code,
                         l_retcode,
                         l_errbuf); -- Added By Roman W. 14/12/2020
    
      if '0' != l_retcode then
        l_contact_out_rec.error_code := fnd_api.g_ret_sts_error; -- If Return Value U, then Also the Value Returned in E
        l_contact_out_rec.error_msg := l_errbuf;
        l_contact_out_tab(l_contact_out_tab.count) := l_contact_out_rec;
      ...
      ---------------------------------------------------------------------------
      -- Added By Roman W. 21-04-2020 CHG0047450 - check duplicate
      ---------------------------------------------------------------------------
      if 'Y' = C_CHECK_DUPLICATE and p_contacts_in(i).contact_id IS NULL then
        l_check_duplicate_flag := 'Y';
      end if;
    
      if 'Y' = l_check_duplicate_flag then
        l_soa_api_dup_chk_rec.source_id := p_contacts_in(i)
                                           .source_reference_id;
      
        l_soa_api_dup_chk_rec.source_name := 'STRATAFORCE';
        l_soa_api_dup_chk_rec.Object_Name := 'CONTACT';
        ...
        if 'IN_PROCESS' = l_soa_api_dup_chk_rec.status then
          ...
          l_main_err_msg := C_DUP_CHK_IN_PROCESS_MSG;
          l_contact_out_rec.error_code := fnd_api.g_ret_sts_error; -- If Return Value U, then Also the Value Returned in E
          l_contact_out_rec.error_msg := C_DUP_CHK_IN_PROCESS_MSG;
          l_contact_out_tab(l_contact_out_tab.count) := l_contact_out_rec;
        
          CONTINUE; --Skip to the Next Account Record
        
        elsif l_soa_api_dup_chk_rec.status in ('CREATED', 'ERROR') then
          -- Rem By Roman W. CHG0047450 10/12/2020
        
          -- Added By Roman W. -- Rem By Roman W. CHG0047450 10/12/2020
          p_contacts_in(i).account_id := l_soa_api_dup_chk_rec.cust_account_id;
          p_contacts_in(i).account_number := l_soa_api_dup_chk_rec.account_number;
          p_contacts_in(i).contact_id := l_soa_api_dup_chk_rec.contact_id;
          p_contacts_in(i).oe_location_id := l_soa_api_dup_chk_rec.location_id;
          p_contacts_in(i).oe_email_contact_point_id := l_soa_api_dup_chk_rec.email_contact_point_id;
          p_contacts_in(i).oe_fax_contact_point_id := l_soa_api_dup_chk_rec.fax_contact_point_id;
          p_contacts_in(i).oe_mobile_contact_point_id := l_soa_api_dup_chk_rec.mobile_contact_point_id;
          p_contacts_in(i).oe_phone_contact_point_id := l_soa_api_dup_chk_rec.phone_contact_point_id;
          -- p_contacts_in(i).oe_party_contact_id := l_soa_api_dup_chk_rec.party_contact_id; -- rem By Roman W 07/05/2020 CHG0047450
          p_contacts_in(i).oe_party_site_id := l_soa_api_dup_chk_rec.party_site_id; -- added By Roman W 07/05/2020 CHG0047450
        
          p_contacts_in(i).oe_person_party_id := l_soa_api_dup_chk_rec.person_party_id;
          p_contacts_in(i).oe_contact_party_id := l_soa_api_dup_chk_rec.contact_party_id;
        
        end if;
      end if;
      ---------------------------------------------------------------------------
      --   End Added By Roman W. 21-04-2020 CHG0047450
      ---------------------------------------------------------------------------
       ...
       .contact_id IS NULL AND l_main_status = fnd_api.g_ret_sts_success THEN
        ...
        IF p_contacts_in(i)
         .address_1 IS NOT NULL AND p_contacts_in(i).country IS NOT NULL THEN
          ...
          l_contact_out_rec.oe_location_id := l_location_id; --INC0150448
        ...
        org_contact_rec.attribute6               := l_contact_out_rec.decision_maker;
      
        get_contact_name_local_lang(p_new_name_local_lang       => l_contact_out_rec.contact_name_local_language,
                                    p_new_first_name_local_lang => org_contact_rec.attribute7,
                                    p_new_last_name_local_lang  => org_contact_rec.attribute8,
                                    p_api_status                => l_api_status,
                                    p_error_msg                 => l_error_msg); -- Added by Roman W. 16/12/2020
        ...
        org_contact_rec.attribute9 := l_contact_out_rec.last_deactivation_reason; -- Added by Roman W. 01/01/2021
        ...
        IF l_object_id IS NOT NULL AND
          ...
          IF l_api_status = fnd_api.g_ret_sts_success AND
            ...
            party_site_rec.attribute3 := nvl(p_contacts_in(i)
                                             .COUNTRY_LOCAL_LAGUAGE,
                                             fnd_api.g_null_char); -- Added By Roman W 12/05/2020 CHG0047450
            party_site_rec.attribute4 := nvl(p_contacts_in(i)
                                             .CITY_LOCAL_LANGUAGE,
                                             fnd_api.g_null_char); -- Added By Roman W 12/05/2020 CHG0047450
            party_site_rec.attribute5 := nvl(p_contacts_in(i)
            ...
            IF l_api_status = fnd_api.g_ret_sts_success THEN
              l_contact_out_rec.oe_party_site_id := l_party_site_id;
            end if;
        ...
        IF l_object_id IS NOT NULL AND
          ...
          rec_cust_acc_role.attribute6 := l_contact_out_rec.partner_user; -- Added by Roman 12/05/2020 CHG0047450
        ...
        ------------------------------------------------------------------------------
        -- Added By Roman W. CHG0047450 - check duplicate
        ------------------------------------------------------------------------------
        if 'Y' = l_check_duplicate_flag then
          l_soa_api_dup_chk_rec.source_id               := l_contact_out_rec.source_reference_id;
          l_soa_api_dup_chk_rec.source_name             := 'STRATAFORCE';
          l_soa_api_dup_chk_rec.object_name             := 'CONTACT';
          l_soa_api_dup_chk_rec.cust_account_id         := l_contact_out_rec.account_id;
          l_soa_api_dup_chk_rec.account_number          := l_contact_out_rec.account_number;
          l_soa_api_dup_chk_rec.contact_id              := l_contact_out_rec.contact_id;
          l_soa_api_dup_chk_rec.location_id             := l_contact_out_rec.oe_location_id;
          l_soa_api_dup_chk_rec.email_contact_point_id  := l_contact_out_rec.oe_email_contact_point_id;
          l_soa_api_dup_chk_rec.fax_contact_point_id    := l_contact_out_rec.oe_fax_contact_point_id;
          l_soa_api_dup_chk_rec.mobile_contact_point_id := l_contact_out_rec.oe_mobile_contact_point_id;
          l_soa_api_dup_chk_rec.phone_contact_point_id  := l_contact_out_rec.oe_phone_contact_point_id;
          -- l_soa_api_dup_chk_rec.party_contact_id     := l_contact_out_rec.oe_party_contact_id; -- added By Roman W. 07/05/2020 CHG0047450
          l_soa_api_dup_chk_rec.party_site_id    := l_contact_out_rec.oe_party_site_id; -- added By Roman W. 07/05/2020 CHG0047450
          l_soa_api_dup_chk_rec.person_party_id  := l_contact_out_rec.oe_person_party_id;
          l_soa_api_dup_chk_rec.contact_party_id := l_contact_out_rec.oe_contact_party_id;
        
          xxhz_soa_api_dup_chk_pkg.update_trx(p_rec => l_soa_api_dup_chk_rec);
        end if;
        ------------------------------------------------------------------------------
        -- End Added By Roman W. CHG0047450 - check duplicate
        ------------------------------------------------------------------------------
      ...
      ELSE
        ...
        BEGIN
          ...
          -----------------------------------------------------------------------------
          SELECT hr.party_id, subject_id, hr.relationship_id
            INTO l_rel_party_id, l_person_party_id, l_relationship_party_id
            FROM hz_relationships hr, hz_cust_account_roles hcar
          ...
          BEGIN
            SELECT org_contact_id, job_title, object_version_number
          ...
          BEGIN
            SELECT hps.location_id,
                   ...
                   hps.party_site_id
              INTO l_location_id, l_psrty_site_vrsn_num, l_party_site_id
              FROM hz_party_sites hps
        ...
        IF l_continuetonextstep = TRUE THEN
          IF upper(nvl(p_contacts_in(i).status, 'Active')) = 'INACTIVE' AND
          ...
          ELSE
            -- Added By Roman W 27/08/2020 CHG0047450
            rec_cust_acc_role.cust_account_role_id := p_contacts_in(i)
                                                      .contact_id;
            rec_cust_acc_role.status               := 'A';
          END IF;
          -- Added By Roman W. 24/11/2020
          rec_cust_acc_role.cust_account_id := l_contact_out_rec.account_id;
          rec_cust_acc_role.attribute6      := l_contact_out_rec.partner_user;
        ...
        IF l_continuetonextstep = TRUE THEN
          ...
          --Update Org Contact
          IF org_contact_rec.org_contact_id IS NOT NULL THEN
            /*
            AND
             (nvl(org_contact_rec.job_title, '~') !=
             nvl(l_org_contact_job_title, '~') or
             nvl(org_contact_rec.attribute7, '~') != p_contacts_in(i)
             .CONTACT_NAME_LOCAL_LANGUAGE or
             nvl(org_contact_rec.attribute8, '~') != p_contacts_in(i)
             .CONTACT_NAME_LOCAL_LANGUAGE) THEN
             */
          ...
          END IF;
          ---------------------------------------------
          ---- Update HZ_PARTY_SITES 28/12/2020 -------
          ---------------------------------------------
          if nvl(l_party_site_id, p_contacts_in(i).OE_PARTY_SITE_ID) is not null then
            begin
              party_site_rec                   := null;
              party_site_rec.created_by_module := g_created_by_module; --'TCA_V1_API';
              party_site_rec.party_site_id     := nvl(l_party_site_id,
                                                      p_contacts_in(i)
                                                      .OE_PARTY_SITE_ID);
            
              party_site_rec.location_id := nvl(l_location_id,
                                                fnd_api.G_NULL_NUM);
              ...
              update_party_site(p_party_site_record => party_site_rec, -- IN hz_party_site_v2pub.party_site_rec_type,
                                p_api_status        => l_api_status, -- OUT VARCHAR2,
                                p_error_msg         => l_error_msg -- OUT VARCHAR2
                                );
            ...
            exception
              when others then
                message('EXCEPTION_OTHERS before update_party_site() - ' ||
                        sqlerrm);
            end;
          
          else
            ------------------------------------------------------------------------------------------------------
            ------------------------------- CREATE PARTY SITE  Added By Roman W. 30/12/2020 ----------------------
            ------------------------------------------------------------------------------------------------------
            IF l_api_status = fnd_api.g_ret_sts_success AND
               l_location_id IS NOT NULL THEN
              ...
              party_site_rec.created_by_module := g_created_by_module; --'TCA_V1_API';
              party_site_rec.location_id       := l_location_id;
              party_site_rec.party_id          := p_contacts_in(i)
                                                  .oe_contact_party_id;
              -- p_contacts_in(i).oe_contact_party_id;
              party_site_rec.attribute3 := nvl(p_contacts_in(i)
                                               .COUNTRY_LOCAL_LAGUAGE,
                                               fnd_api.g_null_char); -- Added By Roman W 12/05/2020 CHG0047450
              party_site_rec.attribute4 := nvl(p_contacts_in(i)
              ...
              IF l_api_status = fnd_api.g_ret_sts_success THEN
                ...
                message('ERROR create_party_site() - ' || l_error_msg);
              end if;
            
              l_steps := l_steps || 'create_party_site Call Status :' ||
                         l_api_status || ';';
              write_log_message('Party Site  l_api_status=' ||
                                l_api_status);
              write_log_message('Party Site  l_error_msg=' || l_error_msg);
            END IF;
            ------------------------------------------------------------------------------------------------------
            ------------------------------- END CREATE PARTY SITE  Added By Roman W. 30/12/2020 ------------------
            ------------------------------------------------------------------------------------------------------
            IF l_api_status = fnd_api.g_ret_sts_success THEN
              upsert_contact_point(p_phone_num             => p_contacts_in(i)
                                                              .phone, --  IN VARCHAR2, -- Send Phone Number
        ...
        IF l_main_status = fnd_api.g_ret_sts_success AND
          ...
          l_contact_out_rec.contact_id          := p_contacts_in(i)
                                                   .contact_id; -- CHG0047450
          l_contact_out_rec.oe_person_party_id  := p_contacts_in(i)
                                                   .oe_person_party_id; -- CHG0047450
          l_contact_out_rec.oe_contact_party_id := p_contacts_in(i)
                                                   .oe_contact_party_id; -- CHG0047450
  ...
  * 1.4      31/03/2020  Roman W.           CHG0047450 - Account, Contact interface - ReDo
  *                                             added handling to new field ACCOUNT_ADDRESS_FLAG
  *                                             at call to CREATE_ACCT_SITE_USE
  * 1.5      21/04/2020  Roman W.           CHG0047450 - check duplication
  * 1.6      17/06/2020  Roman W.           CHG0047450 - insert SITE EVENT if ACCOUNT_ADDRESS_FLAG = 'Y'
  ******************************************************************************************************************************************/
  /*
    PROCEDURE upsert_sites(p_source           IN VARCHAR2,
      ...
      l_xxssys_event_rec xxssys_events%ROWTYPE; -- Added by Roman W. 17/06/2020 CHG0047450 - insert SITE EVENT if ACCOUNT_ADDRESS_FLAG = 'Y'
    BEGIN
      ...
      FOR i IN 1 .. l_record_count LOOP
        ...
        l_sites_out_rec     := xxhz_site_rec(NULL,
                                             ...
                                             NULL -- CHG0047450 - Account, Contact interface - ReDo
                                             );
        l_sites_out_rec     := p_sites_in(i);
        l_sites_out_tab.extend;
  
        -----------------------------------------------------------------------
        -- Added by Roman W. 26/04/2020 CHG0047450
        -----------------------------------------------------------------------
        if l_sites_out_rec.source_reference_id is null then
          l_sites_out_rec.error_code := fnd_api.g_ret_sts_error;
          l_sites_out_rec.error_msg := 'VALIDATION ERROR: SOURCE_REFERENCE_ID - can''t be NULL.';
        ...
        *\
        ...
        IF l_api_status = fnd_api.g_ret_sts_success THEN
          ...
          -- Added By Roman W. 21-04-2020 CHG0047450 - check duplicate
          ---------------------------------------------------------------------------
          if 'Y' = C_CHECK_DUPLICATE and p_sites_in(i).site_id IS NULL then
            l_check_duplicate_flag := 'Y';
          else
            l_check_duplicate_flag := 'N';
          end if;
          ...
          if 'Y' = l_check_duplicate_flag then
            l_soa_api_dup_chk_rec.source_id   := p_sites_in(i)
                                                 .source_reference_id;
            l_soa_api_dup_chk_rec.source_name := 'STRATAFORCE';
            l_soa_api_dup_chk_rec.object_name := 'SITE';
  
            xxhz_soa_api_dup_chk_pkg.insert_trx(p_rec => l_soa_api_dup_chk_rec);
  
            if 'IN_PROCESS' = l_soa_api_dup_chk_rec.status then
              -- error handaling;
              l_api_status := fnd_api.g_ret_sts_error;
              l_error_msg := C_DUP_CHK_IN_PROCESS_MSG;
              l_sites_out_rec.error_code := fnd_api.g_ret_sts_error; -- If Return Value U, then Also the Value Returned in E
              l_sites_out_rec.error_msg := C_DUP_CHK_IN_PROCESS_MSG;
              l_sites_out_tab(l_sites_out_tab.count) := l_sites_out_rec;
  
              CONTINUE; --Skip to the Next Account Record
  
            elsif 'CREATED' = l_soa_api_dup_chk_rec.status then
  
              p_sites_in(i).account_id := l_soa_api_dup_chk_rec.cust_account_id;
              p_sites_in(i).account_number := l_soa_api_dup_chk_rec.account_number;
              p_sites_in(i).site_id := l_soa_api_dup_chk_rec.site_id;
              p_sites_in(i).site_number := l_soa_api_dup_chk_rec.site_number;
              p_sites_in(i).oe_location_id := l_soa_api_dup_chk_rec.location_id;
              p_sites_in(i).oe_ship_site_use_id := l_soa_api_dup_chk_rec.ship_site_use_id;
              p_sites_in(i).oe_bill_site_use_id := l_soa_api_dup_chk_rec.bill_site_use_id;
  
            end if;
          end if;
          ---------------------------------------------------------------------------
          -- End Added By Roman W. 21-04-2020 CHG0047450 - check duplicate
          ---------------------------------------------------------------------------
            ...
            IF p_sites_in(i).site_id IS NULL THEN
              ...
              IF l_api_status = fnd_api.g_ret_sts_success THEN
                BEGIN
                  ...
                  cust_acct_site_rec.cust_account_id   := l_sites_out_rec.account_id; --p_sites_in(i) --CHG0042632
                  cust_acct_site_rec.party_site_id     := l_party_site_id;
                  cust_acct_site_rec.created_by_module := g_created_by_module; --'TCA_V1_API';
                  cust_acct_site_rec.attribute4        := p_sites_in(i)
                                                          .account_address_flag; -- Added By Roman W. 01/04/2020 CHG0047450
                  ...
                  IF p_source = g_old_sfdc_source THEN
                    ...
                    cust_acct_site_rec.attribute1 := p_sites_in(i)
                                                     .source_reference_id;
                  END IF; -- CHG0042044
                  *\ -- rem By Roman W. 01/04/2020 CHG0047450
              ...
              ----------------------------------------------------------------------------------------------
              -- Is Site SF_ACCOUNT_ADDRESS = 'Yes' then insert event
              ----------------------------------------------------------------------------------------------
              if 'Y' = p_sites_in(i).ACCOUNT_ADDRESS_FLAG then
                l_xxssys_event_rec.target_name     := 'STRATAFORCE';
                l_xxssys_event_rec.entity_name     := 'SITE';
                l_xxssys_event_rec.entity_id       := l_sites_out_rec.site_id;
                l_xxssys_event_rec.entity_code     := l_sites_out_rec.site_number; --CHG0042632
                l_xxssys_event_rec.event_name      := 'XXHZ_SOA_API_PKG.UPSERT_SITE'; --INC0143702
                l_xxssys_event_rec.last_updated_by := fnd_global.USER_ID;
                l_xxssys_event_rec.created_by      := fnd_global.USER_ID;
              ...
              ----------------------------------------------------------------------------------------------
              if 'Y' = l_check_duplicate_flag then
                l_soa_api_dup_chk_rec.source_id        := l_sites_out_rec.source_reference_id;
                l_soa_api_dup_chk_rec.source_name      := 'STRATAFORCE';
                l_soa_api_dup_chk_rec.object_name      := 'SITE';
                l_soa_api_dup_chk_rec.cust_account_id  := l_sites_out_rec.account_id;
                l_soa_api_dup_chk_rec.account_number   := l_sites_out_rec.account_number;
                l_soa_api_dup_chk_rec.site_id          := l_sites_out_rec.site_id;
                l_soa_api_dup_chk_rec.site_number      := l_sites_out_rec.site_number; --
                l_soa_api_dup_chk_rec.location_id      := l_sites_out_rec.oe_location_id;
                l_soa_api_dup_chk_rec.ship_site_use_id := l_sites_out_rec.oe_ship_site_use_id;
                l_soa_api_dup_chk_rec.bill_site_use_id := l_sites_out_rec.oe_bill_site_use_id;
              ...
              BEGIN
                ...
                IF p_sites_in(i)
                 .site_usage IS NOT NULL AND l_org_id IS NOT NULL THEN
                  ...
                  IF p_sites_in(i).country IS NOT NULL THEN
                    ...
                    IF l_api_status = fnd_api.g_ret_sts_success THEN
                      ...
                      IF l_api_status = fnd_api.g_ret_sts_success then
                        ...
                        l_sites_out_rec.oe_location_id := p_sites_in(i)
                                                          .oe_location_id;
                        ------------------------------------------------
                        -- Assign Corrected Address, If Country is US
                        --Address Corrected by Avalara, During Creation
                  ...
                  l_sites_out_rec.site_number := p_sites_in(i).site_number;
  ...
  */
  ...
  -- Ver   When        Who          Descr
  -- ----  ----------  ----------   -------------------------------------------------
  -- 1.0   22/06/2020  Roman W.     CHG0047450
  -----------------------------------------------------------------------------------
  procedure site_validation(p_sites_rec  IN OUT xxhz_site_rec,
                            p_error_code OUT VARCHAR2,
                            p_error_desc OUT VARCHAR2) is
    ------------------------
    --    Local Code
    ...
    ------------------------
  begin
    p_error_code := fnd_api.G_RET_STS_SUCCESS;
    p_error_desc := null;
  
    -----------------------------------------------------------------------
    -- Added by Roman W. 26/04/2020 CHG0047450
    -----------------------------------------------------------------------
    if p_sites_rec.source_reference_id is null then
      p_sites_rec.error_code := fnd_api.g_ret_sts_error;
      p_sites_rec.error_msg  := 'VALIDATION ERROR: SOURCE_REFERENCE_ID - can''t be NULL.';
    end if;
  
    IF p_sites_rec.account_id IS NULL AND
       p_sites_rec.account_number IS NOT NULL THEN
    
      BEGIN
        p_sites_rec.account_id := fetch_customer_account_id(NULL,
                                                            p_sites_rec.account_number);
      EXCEPTION
        WHEN OTHERS THEN
          -- If Return Value U, then Also the Value Returned in E
          p_sites_rec.error_code := fnd_api.g_ret_sts_error;
          p_sites_rec.error_msg  := 'VALIDATION ERROR: Customer account number: ' ||
                                    p_sites_rec.account_number ||
                                    ' is not valid.' || SQLERRM;
    ...
    IF p_sites_rec.account_id IS NOT NULL AND
       p_sites_rec.account_number IS NULL THEN
    
      BEGIN
        p_sites_rec.account_number := fetch_customer_account_number(p_sites_rec.account_id);
      EXCEPTION
        WHEN OTHERS THEN
          p_sites_rec.error_code := fnd_api.g_ret_sts_error;
          p_sites_rec.error_msg  := 'VALIDATION ERROR: Customer account id: ' ||
                                    p_sites_rec.account_id ||
                                    ' is not valid.' || SQLERRM;
      END;
    END IF;
    -----------------------------------------------------------------------
  ...
  EXCEPTION
    when OTHERS then
      ...
      p_error_desc := 'EXCEPTION_OTHERS xxhz_soa_api_pkg.validate_site() - ' ||
                      sqlerrm;
  ...
  -- Ver    When        Who          Descr
  -- -----  ---------   -----------  ---------------------------------------------------
  -- 1.0    23/06/2020  Roman W.     CHG0047450
  --------------------------------------------------------------------------------------
  function get_site_use_id(p_cust_acct_site_id NUMBER,
                           p_site_use_code     VARCHAR2) return number is
    ------------------------
    --    Local Definition
    ------------------------
    l_site_use_id number;
    ...
    ------------------------
  begin
    select hw.SITE_USE_ID
      into l_site_use_id
      from HZ_CUST_SITE_USES_ALL hw
     where hw.CUST_ACCT_SITE_ID = p_cust_acct_site_id --449781
       and hw.SITE_USE_CODE = p_site_use_code --'BILL_TO'
       and hw.object_version_number =
           (select max(hw_sub.object_version_number)
              from HZ_CUST_SITE_USES_ALL hw_sub
             where hw_sub.CUST_ACCT_SITE_ID = p_cust_acct_site_id --449781
               and hw_sub.SITE_USE_CODE = p_site_use_code --p_site_use_code --'BILL_TO'
            )
       and ROWNUM = 1;
  ...
  --                                          bill_site_use_id
  ------------------------------------------------------------------------------
  procedure get_sites_out_rec(p_sites_out_rec IN OUT xxhz_site_rec) is
    -----------------------------
    --     Local Definition
    -----------------------------
    ...
    -----------------------------
  begin
    p_sites_out_rec.error_code := fnd_api.G_RET_STS_SUCCESS;
  
    if p_sites_out_rec.account_id is null or
       p_sites_out_rec.account_number is null or
       p_sites_out_rec.site_number is null or
       p_sites_out_rec.oe_location_id is null then
    
      select hcas.cust_account_id,
             hca.account_number,
             hps.party_site_number,
             hps.location_id
        into p_sites_out_rec.account_id,
             p_sites_out_rec.account_number,
             p_sites_out_rec.site_number,
             p_sites_out_rec.oe_location_id
        from hz_cust_acct_sites_all hcas,
             hz_cust_accounts       hca,
             hz_party_sites         hps
       where hcas.CUST_ACCT_SITE_ID = p_sites_out_rec.site_id
    ...
    begin
      select hcsua.SITE_USE_ID
        into p_sites_out_rec.oe_bill_site_use_id
        from HZ_CUST_SITE_USES_ALL hcsua
       where hcsua.CUST_ACCT_SITE_ID = p_sites_out_rec.site_id --449781
         and hcsua.SITE_USE_CODE = 'BILL_TO'
         and hcsua.site_use_id =
             nvl(p_sites_out_rec.oe_bill_site_use_id, hcsua.site_use_id)
         and hcsua.status = 'A';
    exception
      when others then
        p_sites_out_rec.oe_bill_site_use_id := null;
    end;
  
    begin
      select hcsua.SITE_USE_ID
        into p_sites_out_rec.oe_ship_site_use_id
        from HZ_CUST_SITE_USES_ALL hcsua
       where hcsua.CUST_ACCT_SITE_ID = p_sites_out_rec.site_id --449781
         and hcsua.SITE_USE_CODE = 'SHIP_TO'
         and hcsua.site_use_id =
             nvl(p_sites_out_rec.oe_ship_site_use_id, hcsua.site_use_id)
         and hcsua.status = 'A';
    exception
      when others then
        p_sites_out_rec.oe_ship_site_use_id := null;
    end;
  
  exception
    when others then
      p_sites_out_rec.error_code := fnd_api.G_RET_STS_ERROR;
      p_sites_out_rec.error_msg  := 'EXCEPTION_OTHERS xxhz_soa_api_pkg.get_sites_out_rec : ' ||
                                    sqlerrm;
  end get_sites_out_rec;
  ------------------------------------------------------------------------------------------------------------------------------------
  -- Type                : Procedure
  -- Module Name         : AR_CUSTOMERS
  -- Name                : upsert_sites
  ...
  -- HISTORY
  -- =======
  -- Ver   When         AUTHOR(S)     DESCRIPTION
  -- ----  -----------  ----------    ------------------------------------
  -- 1.0   24/06/2020   Roman W.      CHG0047450 - insert SITE EVENT if ACCOUNT_ADDRESS_FLAG = 'Y'
  -- 1.1   1/07/2020    Roman W.      CHG0047450 - added initialization :l_sites_out_rec.error_code := fnd_api.G_RET_STS_SUCCESS;
  -- 1.2   25/11/2020   Roman W.      CHG0047450 - added : cust_site_use_rec.location := substr(l_sites_out_rec.account_number || '-' || l_sites_out_rec.city, 1,40)
  -- 1.3   10/12/2020   Roman W.      CHG0047450 - added error handling in XXHZ_SOA_APIDUP_CHK_PKG
  --------------------------------------------------------------------------------------------------------------------------------------
  PROCEDURE upsert_sites(p_source           IN VARCHAR2,
                         p_soa_reference_id IN NUMBER,
                         p_sites_in         IN OUT xxobjt.xxhz_site_tab,
    ...
    l_ship_to_delete_flag varchar2(10);
    l_ship_to_add_flag    varchar2(10);
    l_bill_to_delete_flag varchar2(10);
    l_bill_to_add_flag    varchar2(10);
  
    l_check_duplicate varchar2(10) := fnd_api.G_FALSE;
  ...
  BEGIN
    ...
    FOR i IN 1 .. l_record_count LOOP
      ...
      l_sites_out_rec.error_code := fnd_api.G_RET_STS_SUCCESS;
      l_sites_out_rec.error_msg  := null;
      ----------------- CHG0047450 --------------------------
      --         S I T E   V A L I D A T I O N
      -------------------------------------------------------
      site_validation(p_sites_rec  => l_sites_out_rec,
                      p_error_code => p_status,
                      p_error_desc => p_message);
    
      if fnd_api.G_RET_STS_ERROR = p_status then
        l_sites_out_rec.error_code := fnd_api.g_ret_sts_error;
        l_sites_out_rec.error_msg := p_message;
        l_sites_out_tab(l_sites_out_tab.count) := l_sites_out_rec;
        continue;
      end if;
    
      if fnd_api.G_RET_STS_ERROR = l_sites_out_rec.error_code then
        l_sites_out_tab(l_sites_out_tab.count) := l_sites_out_rec;
        continue;
      end if;
      --------------------------------------------------------
      --      E N D__S I T E__V A L I D A T I O N
      --------------------------------------------------------
      ...
      */
      ...
      -- Added By Roman W. 21-04-2020 CHG0047450 - check duplicate
      ---------------------------------------------------------------------------
      if 'Y' = C_CHECK_DUPLICATE and l_sites_out_rec.site_id IS NULL then
        l_check_duplicate_flag := 'Y';
      else
        l_check_duplicate_flag := 'N';
      end if;
      ...
      if 'IN_PROCESS' = l_soa_api_dup_chk_rec.status then
        ...
        --elsif 'CREATED' = l_soa_api_dup_chk_rec.status then -- Rem By Roman W. CHG0047450 10/12/2020
      elsif l_soa_api_dup_chk_rec.status in ('CREATED', 'ERROR') then
        l_sites_out_rec.account_id          := l_soa_api_dup_chk_rec.cust_account_id;
        l_sites_out_rec.account_number      := l_soa_api_dup_chk_rec.account_number;
        l_sites_out_rec.site_id             := l_soa_api_dup_chk_rec.site_id;
        l_sites_out_rec.site_number         := l_soa_api_dup_chk_rec.site_number;
        l_sites_out_rec.oe_location_id      := l_soa_api_dup_chk_rec.location_id;
        l_sites_out_rec.oe_ship_site_use_id := l_soa_api_dup_chk_rec.ship_site_use_id;
        l_sites_out_rec.oe_bill_site_use_id := l_soa_api_dup_chk_rec.bill_site_use_id;
        l_check_duplicate                   := fnd_api.G_TRUE;
      end if;
    
      ---------------------------------------------------------
      -- Calculate Party Name , Added by Roman W. 25/11/2020 --
      ---------------------------------------------------------
      begin
      ...
      exception
        when others then
          l_party_name := null;
          l_location   := substr(l_party_name || ',' ||
                                 nvl(l_sites_out_rec.city,
                                     p_sites_in(i).city),
                                 1,
                                 40);
      end;
    
      IF l_sites_out_rec.site_id IS NULL THEN
      
        --------------------------------------------------------------------
        --              C R E A T E
        --------------------------------------------------------------------
        IF l_sites_out_rec.country IS NOT NULL THEN
        
          get_territory_org_info(p_country        => l_sites_out_rec.country, -- IN Param
                                 x_territory_code => l_territory_code, --Out Param
                                 x_org_id         => l_org_id, --Out Param
                                 x_ou_unit_name   => l_ou_unit, --Out Param
                                 x_error_code     => l_api_status, --Out Param
          ...
          IF l_api_status = fnd_api.g_ret_sts_success THEN
            location_rec.country     := l_territory_code;
            location_rec.county      := l_sites_out_rec.county;
            location_rec.address1    := l_sites_out_rec.address;
            location_rec.city        := l_sites_out_rec.city;
            location_rec.postal_code := l_sites_out_rec.zipcode_or_postal_code;
            location_rec.state       := l_sites_out_rec.state_or_region;
            location_rec.location_id := l_sites_out_rec.oe_location_id;
          
            l_sites_out_rec.operating_unit := to_char(l_org_id);
          ELSE
            l_continue_flag := 0;
            l_sites_out_rec.error_code := fnd_api.g_ret_sts_error;
            l_sites_out_rec.error_msg := l_error_msg;
            ...
            l_sites_out_tab(l_sites_out_tab.count) := l_sites_out_rec; -- Added By Roman W. 22/06/2020 CHG0047450
            continue; -- Added By Roman W. 22/06/2020 CHG0047450
          
          END IF;
        END IF;
        ----------------------------------------------------------------------
        --                1) C R E A T E___L O C A T I O N
        ----------------------------------------------------------------------
        location_rec.created_by_module := g_created_by_module; --'TCA_V1_API';
        ...
        ELSE
          write_log_message('Create SHIP_TO Location API Status / Message :' ||
                            l_api_status || '/Error Msg:' || l_error_msg);
          l_sites_out_rec.error_code := fnd_api.G_RET_STS_ERROR;
          l_sites_out_rec.error_msg := substr(l_error_msg, 1, 1000);
          l_sites_out_tab(l_sites_out_tab.count) := l_sites_out_rec;
        
          p_status  := fnd_api.g_ret_sts_error;
          p_message := l_error_msg;
        
          l_soa_api_dup_chk_rec.source_id       := l_sites_out_rec.source_reference_id; -- Added By Roman W. 10/12/2020 CHG0047450
          l_soa_api_dup_chk_rec.source_name     := 'STRATAFORCE'; -- Added By Roman W. 10/12/2020 CHG0047450
          l_soa_api_dup_chk_rec.object_name     := 'SITE'; -- Added By Roman W. 10/12/2020 CHG0047450
          l_soa_api_dup_chk_rec.cust_account_id := l_sites_out_rec.account_id; -- Added By Roman W. 10/12/2020 CHG0047450
          l_soa_api_dup_chk_rec.account_number  := l_sites_out_rec.account_number; -- Added By Roman W. 10/12/2020 CHG0047450
          xxhz_soa_api_dup_chk_pkg.update_to_error_trx(l_soa_api_dup_chk_rec); -- Added By Roman W. 10/12/2020 CHG0047450
        
          continue;
        
        END IF;
        ----------------------------------------------------------------------
        --                1) E N D____C R E A T E___L O C A T I O N
        ----------------------------------------------------------------------
      
        BEGIN
          -----------------------------------------------------------
          --          2) C R E A T E____P A R T Y__S I T E
          -----------------------------------------------------------
          party_site_rec.party_site_name   := p_sites_in(i).site_name;
          party_site_rec.created_by_module := g_created_by_module; --'TCA_V1_API';
          SELECT party_id
            INTO party_site_rec.party_id
            FROM hz_cust_accounts_all
           WHERE cust_account_id = l_sites_out_rec.account_id; --CHG0042632
          ...
          if fnd_api.G_RET_STS_ERROR = l_api_status then
            write_log_message('(4698) Create create_party_site API :' ||
                              l_api_status || '/Error Msg:' || l_error_msg);
            l_sites_out_rec.error_code := fnd_api.G_RET_STS_ERROR;
            l_sites_out_rec.error_msg := substr(l_error_msg, 1, 1000);
            l_sites_out_tab(l_sites_out_tab.count) := l_sites_out_rec;
            ...
            l_soa_api_dup_chk_rec.cust_account_id := l_sites_out_rec.account_id; -- Added By Roman W. 10/12/2020 CHG0047450
            l_soa_api_dup_chk_rec.account_number  := l_sites_out_rec.account_number; -- Added By Roman W. 10/12/2020 CHG0047450
            xxhz_soa_api_dup_chk_pkg.update_to_error_trx(p_rec => l_soa_api_dup_chk_rec); -- Added By Roman W. 10/12/2020 CHG0047450
        ...
        EXCEPTION
          WHEN OTHERS THEN
            ...
            l_sites_out_rec.error_code := fnd_api.G_RET_STS_ERROR;
            l_sites_out_rec.error_msg := substr('EXCEPTION_OTHERS(4723) ' ||
                                                sqlerrm,
                                                1,
                                                1000);
            l_sites_out_tab(l_sites_out_tab.count) := l_sites_out_rec;
            p_status := fnd_api.g_ret_sts_error;
            p_message := l_sites_out_rec.error_msg;
        ...
        END;
        -----------------------------------------------------------
        --       2)  E N D____C R E A T E____P A R T Y____S I T E
        -----------------------------------------------------------
      
        ------------------------------------------------------------------
        --   3) C R E A T E____C U S T O M E R____A C C O U N T____SITE
        ------------------------------------------------------------------
        BEGIN
          IF l_org_id IS NOT NULL THEN
            cust_acct_site_rec.org_id := l_org_id;
          ...
          if fnd_api.G_RET_STS_ERROR = l_api_status then
            ...
            l_soa_api_dup_chk_rec.cust_account_id := l_sites_out_rec.account_id; -- Added By Roman W. 10/12/2020 CHG0047450
            l_soa_api_dup_chk_rec.account_number  := l_sites_out_rec.account_number; -- Added By Roman W. 10/12/2020 CHG0047450
            l_soa_api_dup_chk_rec.site_number     := l_sites_out_rec.site_number; -- Added By Roman W. 10/12/2020 CHG0047450
            xxhz_soa_api_dup_chk_pkg.update_to_error_trx(p_rec => l_soa_api_dup_chk_rec); -- Added By Roman W. 10/12/2020 CHG0047450
        ...
        END;
        ----------------------------------------------------------------------------
        --   3) E N D_____C R E A T E____C U S T O M E R____A C C O U N T____SITE
        ----------------------------------------------------------------------------
      
        ----------------------------------------------------------------------------
        --   4) C R E A T E____C U S T O M E R____S I T E____U S E
        ----------------------------------------------------------------------------
        IF l_sites_out_rec.site_usage IS NOT NULL THEN
        
          -- IF 0 < instr('Bill To/Ship To', 'Ship To') then
          IF 0 < instr(l_sites_out_rec.site_usage, 'Ship To') then
            l_ship_to_flag := fnd_api.G_TRUE;
          ELSE
            l_ship_to_flag := fnd_api.G_FALSE;
          END IF;
        
          --IF 0 < instr('Bill To/Ship To', 'Bill To') then
          IF 0 < instr(l_sites_out_rec.site_usage, 'Bill To') then
            l_bill_to_flag := fnd_api.G_TRUE;
          ELSE
            l_bill_to_flag := fnd_api.G_FALSE;
          END IF;
        
          cust_site_use_rec.cust_acct_site_id := l_cust_acct_site_id;
          cust_site_use_rec.created_by_module := g_created_by_module; --'TCA_V2_API';
          -- cust_site_use_rec.location          := x_location_id; -- Rem By Roman W. 25/11/2020 CHG0047450
          --------- S H I P___T O--------
          if fnd_api.G_TRUE = l_ship_to_flag then
          
            cust_site_use_rec.location      := l_location; -- Added by Roman W. 25/11/2020
            cust_site_use_rec.site_use_code := 'SHIP_TO';
            create_acct_site_use(p_cust_site_use_rec => cust_site_use_rec,
                                 p_site_use_id       => l_site_use_id,
                                 p_api_status        => l_api_status,
            ...
            if fnd_api.G_RET_STS_ERROR = l_api_status then
              ...
              l_soa_api_dup_chk_rec.account_number  := l_sites_out_rec.account_number; -- Added By Roman W. 10/12/2020 CHG0047450
              --create_location
              l_soa_api_dup_chk_rec.location_id := l_sites_out_rec.oe_location_id; -- Added By Roman W. 10/12/2020 CHG0047450
              --create_party_site
              l_soa_api_dup_chk_rec.site_number := l_sites_out_rec.site_number; -- Added By Roman W. 10/12/2020 CHG0047450
              --create_acct_site
              l_soa_api_dup_chk_rec.site_id := l_sites_out_rec.site_id; -- Added By Roman W. 10/12/2020 CHG0047450
              xxhz_soa_api_dup_chk_pkg.update_to_error_trx(p_rec => l_soa_api_dup_chk_rec); -- Added By Roman W. 10/12/2020 CHG0047450
          ...
          --------- B I L L___T O--------
          if fnd_api.G_TRUE = l_bill_to_flag then
          
            cust_site_use_rec.location      := substr(trim(l_party_name) || '-' ||
                                                      trim(l_sites_out_rec.city),
                                                      1,
                                                      40); -- Added by Roman W. 25/11/2020;
            cust_site_use_rec.site_use_code := 'BILL_TO';
            ...
            if fnd_api.G_RET_STS_ERROR = l_api_status then
              ...
              --create_acct_site
              l_soa_api_dup_chk_rec.site_id := l_sites_out_rec.site_id; -- Added By Roman W. 10/12/2020 CHG0047450
              -- create_acct_site_use
              l_soa_api_dup_chk_rec.ship_site_use_id := l_sites_out_rec.oe_ship_site_use_id; -- Added By Roman W. 10/12/2020 CHG0047450
              xxhz_soa_api_dup_chk_pkg.update_to_error_trx(p_rec => l_soa_api_dup_chk_rec); -- Added By Roman W. 10/12/2020 CHG0047450
        ...
        END IF;
        ----------------------------------------------------------------------------
        --   4) E N D___C R E A T E____C U S T O M E R____S I T E____U S E
        ----------------------------------------------------------------------------
        ...
        -- Is Site SF_ACCOUNT_ADDRESS = 'Yes' then insert event
        ----------------------------------------------------------------------------------------------
        if 'Y' = l_sites_out_rec.ACCOUNT_ADDRESS_FLAG then
          l_xxssys_event_rec.target_name     := 'STRATAFORCE';
          l_xxssys_event_rec.entity_name     := 'SITE';
          l_xxssys_event_rec.entity_id       := l_sites_out_rec.site_id;
          l_xxssys_event_rec.entity_code     := l_sites_out_rec.site_number; --CHG0042632
        ...
        -------------------------------------------------------------------------
        --                      U P D A T E___S I T E
        -------------------------------------------------------------------------
      ELSE
        get_sites_out_rec(l_sites_out_rec);
        ...
        END IF;
        --------------------------------------------------
        --    2)  Update CUSTOMER SITE USE
        --------------------------------------------------
        begin
          SELECT org_id
            INTO l_org_id
            FROM hz_cust_acct_sites_all
           WHERE cust_acct_site_id = l_sites_out_rec.site_id;
        ...
        END IF;
        -- SHIP_TO DELETE flag
        if fnd_api.G_FALSE = l_ship_to_flag and
           l_sites_out_rec.oe_ship_site_use_id is not null then
          l_ship_to_delete_flag := fnd_api.G_TRUE;
        else
          l_ship_to_delete_flag := fnd_api.G_FALSE;
        end if;
      
        -- SHIP_TO CREATE flag
        if fnd_api.G_TRUE = l_ship_to_flag and
           l_sites_out_rec.oe_ship_site_use_id is null then
          l_ship_to_add_flag := fnd_api.G_TRUE;
        else
          l_ship_to_add_flag := fnd_api.G_FALSE;
        end if;
        ...
        END IF;
        -- BILL_TO DELETE Flag
        IF fnd_api.G_FALSE = l_bill_to_flag and
           l_sites_out_rec.oe_bill_site_use_id is NOT NULL then
          l_bill_to_delete_flag := fnd_api.G_TRUE;
        else
          l_bill_to_delete_flag := fnd_api.G_FALSE;
        end if;
        -- BILL_TO CREATE Flag
        IF fnd_api.G_TRUE = l_bill_to_flag and
           l_sites_out_rec.oe_bill_site_use_id is NULL then
          l_bill_to_add_flag := fnd_api.G_TRUE;
        else
          l_bill_to_add_flag := fnd_api.G_FALSE;
        end if;
      
        -- DELETE SHIP_TO Site Use
        if fnd_api.G_TRUE = l_ship_to_delete_flag then
          cust_site_use_rec.site_use_id       := l_sites_out_rec.oe_ship_site_use_id;
          cust_site_use_rec.cust_acct_site_id := l_sites_out_rec.site_id;
        ...
        -- DELETE BILL_TO Site Use
        if fnd_api.G_TRUE = l_bill_to_delete_flag then
        
          cust_site_use_rec.site_use_id       := l_sites_out_rec.oe_bill_site_use_id;
          cust_site_use_rec.cust_acct_site_id := l_sites_out_rec.site_id;
          cust_site_use_rec.location          := l_location; -- Added by Roman W. 25/11/2020
          cust_site_use_rec.status            := 'I';
        ...
        -- ADD SHIP_TO Site Use
        if fnd_api.G_TRUE = l_ship_to_add_flag then
        
          l_site_use_id := get_site_use_id(l_sites_out_rec.site_id,
                                           'SHIP_TO');
        
          cust_site_use_rec.cust_acct_site_id := l_sites_out_rec.site_id;
          cust_site_use_rec.site_use_id       := l_site_use_id;
          cust_site_use_rec.status            := 'A';
        
          if cust_site_use_rec.site_use_id is null then
            --cust_site_use_rec.location          := l_sites_out_rec.oe_location_id; -- Rem by Roman W. 25/11/2020
            cust_site_use_rec.location := l_location; -- Added by Roman W. 25/11/2020
        ...
        -- ADD BILL_TO Site Use
        if fnd_api.G_TRUE = l_bill_to_add_flag then
          l_site_use_id := get_site_use_id(l_sites_out_rec.site_id,
                                           'BILL_TO');
        ...
        ----------------------------------------------------------------------------------------------
        -- Added By Roman W. 23/06/2020 CHG0047450 - check duplicate
        ----------------------------------------------------------------------------------------------
        if 'Y' = l_check_duplicate_flag then
          l_soa_api_dup_chk_rec.source_id        := l_sites_out_rec.source_reference_id;
          l_soa_api_dup_chk_rec.source_name      := 'STRATAFORCE';
  ...
  ******************************************************************************************************************************************/
  ...
  -- Ver   When         Who         Descr
  -- ----  -----------  ----------  ------------------------------------------------
  -- 1.0   27/08/2020   Roman W.    added handling to "A" status
  -- 1.1   24/11/2020   Roman W     added handling to ATTRIBUTE6 - PARTNER_USER
  ----------------------------------------------------------------------------------
  ...
  BEGIN
    write_log_message('Program Entered : xxhz_soa_api_pkg.Update_account_role Procedure',
                      'Y');
    ---------------------------------------------------------------
    --If Status is 'I/A', Get the Lastest Updated Record from Data Base
    ---------------------------------------------------------------
    IF p_cust_account_role_record.status in ('I', 'A') THEN
      ...
      IF p_api_status = fnd_api.g_ret_sts_success THEN
        -- l_cust_account_role_rec.status := 'I'; Rem By Roman W. 27/08/2020
        l_cust_account_role_rec.status     := p_cust_account_role_record.status;
        l_cust_account_role_rec.attribute6 := p_cust_account_role_record.attribute6;
        ...
        write_log_message(' Call to update_cust_account_role Status / Error :' ||
                          p_api_status || '/' || p_error_msg);
      END IF;
    END IF; -- Rem By Roman W. 27/08/2020
  ...
  * 1.00     11/05/2016  Somnath Dawn                Initial version
  ...
  --  ----   -----------  ---------------   --------------------------------------------------
  --  1.0    27/06/2017   Lingaraj Sarangi  Initial version
  --  1.1    13.3.19      yuval tal         INC0150448 add or condition
  --  1.2    04/02/2021   Roman W.          CHG0047450 - Account, Contact interface - ReDo
  -------------------------------------------------------------------------------------------
  ...
  BEGIN
    x_api_status := fnd_api.g_ret_sts_success;
    IF (p_state_name IS NOT NULL and p_state_name != fnd_api.g_null_char) OR
       (p_state_code IS NOT NULL and p_state_code != fnd_api.g_null_char) THEN
    
      SELECT lookup_code, meaning
        INTO p_state_code, p_state_name
        FROM fnd_common_lookups
       WHERE lookup_type = 'US_STATE'
         AND (upper(meaning) = upper(nvl(p_state_name, meaning)) OR --INC0150448
             lookup_code = nvl(p_state_code, lookup_code))
         AND rownum = 1;
  ...
  * 1.1     25.06.19    Lingaraj            CHG0046009 - Apps Session initialize corrected to restrict event generation
  * 1.2     21/01/2020  Roman W.            INC0180948 -  change responsibility name to responsibility key
  ******************************************************************************************************************************************/
  ...
  BEGIN
    ...
    --l_user_id
      FROM fnd_responsibility_tl frt,
           fnd_responsibility_vl frv, -- added by Roman W. 21/01/2020 INC0180948
           fnd_user              fu
     WHERE frv.RESPONSIBILITY_KEY = 'HZ_TCA_MANAGER' -- 'Trading Community Manager' added by Roman W. 21/01/2020 INC0180948
          --frt.responsibility_name = 'Trading Community Manager' -- Rem by Roman W. 21/01/2020 INC0180948
       AND frt.language = 'US'
       AND frv.RESPONSIBILITY_ID = frt.responsibility_id
       AND fu.user_id = g_user_id; ----CHG0046009 Added 25.JUN.19
    ...
    IF l_request_id = 0 THEN
      ...
      xxobjt_wf_mail.send_mail_text(p_to_role     => 'SYSADMIN',
                                    p_subject     => 'Error submit DQM Synchronization Program,Flow Id=' ||
                                                     p_soa_reference_id,
  ...
  -- Ver   When          Who             Descr
  -- ----  ------------  --------------  ------------------------------------------
  -- 1.0   12/05/2020    Roman W.
  ---------------------------------------------------------------------------------
  procedure get_sf_payment_terms(p_hw_payment_terms_c IN VARCHAR2,
                                 p_ret_value          OUT VARCHAR2,
                                 p_error_code         OUT VARCHAR2,
                                 p_error_desc         OUT VARCHAR2) is
  ...
  begin
    ...
    if p_hw_payment_terms_c is not null then
      SELECT xe.entity_code
        into p_ret_value
        FROM xxssys_events xe
       WHERE xe.target_name = 'STRATAFORCE'
         AND xe.entity_name = 'PAY_TERM'
         AND xe.status = 'SUCCESS'
         AND xe.entity_id = p_hw_payment_terms_c; -- Added By Roman W. 24/12/2020
      -- AND external_id = p_hw_payment_terms_c; -- Rem By Roman W. 24/12/2020
    else
      p_ret_value := null;
    end if;
  ...
  exception
    when NO_DATA_FOUND then
      p_error_code := 'E';
      p_error_desc := 'EXCEPTION_NO_DATA_FOUND xxhz_soa_api_pkg.get_sf_payment_terms(' ||
                      p_hw_payment_terms_c || ') - ' || sqlerrm;
    when OTHERS then
      p_error_code := 'E';
      p_error_desc := 'EXCEPTION_OTHERS xxhz_soa_api_pkg.get_sf_payment_terms(' ||
                      p_hw_payment_terms_c || ') - ' || sqlerrm;
  end get_sf_payment_terms;

  -------------------------------------------------------------------------
  -- Ver   When          Who        Descr
  -- ----  ------------  ---------  ---------------------------------------
  -- 1.0   12/05/2020    Roman W.   CHG0047450
  -------------------------------------------------------------------------
  procedure get_sf_payment_term_id(p_sf_payment_term_id IN VARCHAR2,
                                   p_ret_value          OUT VARCHAR2,
                                   p_error_code         OUT VARCHAR2,
                                   p_error_desc         OUT VARCHAR2) is
  ...
  begin
    ...
    if p_sf_payment_term_id is not null then
      SELECT xe.entity_id
        into p_ret_value
        FROM xxssys_events xe
       WHERE xe.target_name = 'STRATAFORCE'
         AND xe.entity_name = 'PAY_TERM'
         AND xe.status = 'SUCCESS'
         AND xe.entity_id = p_sf_payment_term_id; -- Added By Roman W. 24/12/2020
      -- AND external_id = p_hw_payment_terms_c; -- Rem By Roman W. 24/12/2020
    else
      p_ret_value := null;
    end if;
  ...
  exception
    when NO_DATA_FOUND then
      p_error_code := 'E';
      p_error_desc := 'EXCEPTION_NO_DATA_FOUND xxhz_soa_api_pkg.get_sf_payment_term_id(' ||
                      p_sf_payment_term_id ||
                      ') - PAYMENT TERM not exist in ORACLE';
    when OTHERS then
      p_error_code := 'E';
      p_error_desc := 'EXCEPTION_OTHERS xxhz_soa_api_pkg.get_sf_payment_term_id(' ||
                      p_sf_payment_term_id || ') - ' || sqlerrm;
    
  end get_sf_payment_term_id;
