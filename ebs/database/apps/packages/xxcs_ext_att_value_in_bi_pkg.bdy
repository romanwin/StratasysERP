CREATE OR REPLACE PACKAGE BODY xxcs_ext_att_value_in_bi_pkg IS
  ...
  BEGIN
    ...
    FOR get_refurbish_population_r IN get_refurbish_population_c LOOP
      ...
      ELSE
        fnd_file.put_line(fnd_file.log,
                          'Success Update Refurbished Flag' || ' Instance ' ||
                          to_char(get_refurbish_population_r.instance_id) ||
                          chr(10) || ' Atribute ' ||
                          get_refurbish_population_r.attribute_id ||
                          chr(10));
    ...
    --  1.1  11/11/2012  Adi Safin 
      SELECT ciab.customer_product_id instance_id,
       ...
       WHERE ciab.incident_status_id = cisb.incident_status_id
         ...
         AND cie1.attribute_name IN
             (ciab.external_attribute_5,
              ciab.external_attribute_12,
              ciab.external_attribute_13)
  ...
  BEGIN
    ...
    FOR get_foc_date_population_r IN get_foc_date_population_c LOOP
      ...
      IF NOT (x_return_status = fnd_api.g_ret_sts_success) THEN
        ...
        WHILE l_msg_count > 0 LOOP
          ...
          fnd_file.put_line(fnd_file.log,
                            ...
                            ' l_msg_index   ' || l_msg_index || chr(10) ||
                            ' Instance      ' ||
                            to_char(get_foc_date_population_r.instance_id) ||
                            chr(10) || ' Atribute      ' ||
                            get_foc_date_population_r.attribute_id ||
                            chr(10) || ' Error Message ' || x_msg_data ||
                            chr(10));
          l_msg_index := l_msg_index + 1;
          l_msg_count := l_msg_count - 1;
      ...
      ELSE
        fnd_file.put_line(fnd_file.log,
                          'Success Update FOC Date' || ' Instance ' ||
                          to_char(get_foc_date_population_r.instance_id) ||
                          chr(10) || ' Atribute ' ||
                          get_foc_date_population_r.attribute_id || chr(10));
        COMMIT;
      END IF;
    END LOOP;
