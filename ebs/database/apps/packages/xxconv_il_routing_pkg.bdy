CREATE OR REPLACE PACKAGE BODY xxconv_il_routing_pkg IS
  -------------------------------------------------------------------
  -- insert_interface_routing
  --------------------------------------------------------------------
  --  ver   date          name            desc
  --  1.1   18.08.2013    Vitaly          CR 812 - Copy BOMs and routings to the new organizations
  ...
  BEGIN
    ...
    FOR i IN cr_routing LOOP
      ...
      IF v_status = 'S' OR
        ...
        v_step := 'Step 400';
        UPDATE xxobjt_conv_il_routing a
           SET a.error_code = 'S'
