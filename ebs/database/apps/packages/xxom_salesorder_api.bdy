CREATE OR REPLACE PACKAGE BODY xxom_salesorder_api AS
  ...
  --  2.4    24-Jan-2019   Diptasurjya               INC0145173 - In case multiple PTO KIT systems are ordered on same quote,
  --                                                    update of orig_sys_line_ref will fail. Fix procedure prepare_line_delete_data
  --                                                    Also add a new validation for presence of parent_line_number in validate_order_line
  --  2.5    30-Jan-2019   LINGARAJ                  CHG0044825 - On Order book validation with source STRATAFORCE,
  --                                                    allow up to 10000 KRW difference between Order amount and Quote amount
  --  2.6    31-Jan-2019   Lingaraj                  CHG0044636 - Add new account to Get quote lines validation
  --  2.7    18-Mar-2019   Diptasurjya               INC0150858 - Assign different line numbers for lines coming without line numbers
  --  2.7.1  28.3.19       yuval tal                 INC0152058 - modify is_diff_amount_in_tolerance
  --  2.7.2  04-Mar-2019   Diptasurjya               CHG0044403 - Exclude some item-carrier combination from Dangerous goods string
  --  2.8    05-Feb-2019   Diptasurjya               CHG0044970 - Handle service contract items
  --  2.9    26-SEP-2019   Diptasurjya               CHG0046549 - Set calculate price flag for PTO child lines in prepare_line_data
  --  3.0    05-NOV-2019   Diptasurjya               CHG0046049 - PPU related changes
  --  3.1    04-NOV-2020   Diptasurjya               INC0211533 - Order book validation code fix
  --  3.2    17-DEC-2020   Diptasurjya               CHG0048964 - Change BOM explode code in prepare_line_date to exclude unimplemented lines
  --  4.0    12/01/2021    Roman W.                  CHG0048579 - OIC (JIRA : OIC-242)
  --  4.1    26/07/2021    Roman W.                  CHG0049588 - p_waller_loc=> p_wallet_loc bug fix
  ----------------------------------------------------------------------------------------------------------------------
  ...
  g_quote_lines        VARCHAR2(150) := 'QUOTE_LINES'; -- CHG0048579

  ----------------------------------------------------------------------------
  -- Ver   When         Who            Descr
  -- ----  -----------  ----------  ------------------------------------------
  -- 1.0   12/01/2021   Roman W.    CHG0048579 - OIC
  ----------------------------------------------------------------------------
  ...
  */
  ...
  -- 1.1  03/04/2019  Diptasurjya                     CHG0044403 - Remove DG message for excluded items
  --                                                  as per valueset XXOM_DG_SHIP_INSTR_EXCLUSION
  -- --------------------------------------------------------------------------------------------
  FUNCTION is_order_dg_eligible(p_so_lines_tab xxom_so_lines_tab_type,
                                p_header_id    NUMBER) -- CHG0044403 added new input parameter p_header_id
   RETURN VARCHAR2 IS
    ...
    SELECT 'Y'
     ...
     (SELECT 1
              FROM oe_order_headers_all oh,
                   ...
                   mtl_system_items_b   msib
             WHERE oh.header_id = p_header_id
               AND oh.shipping_method_code = wcs.ship_method_code
               ...
               AND msib.inventory_item_id = t1.inventory_item_id
               AND msib.organization_id = g_master_invorg_id
  ...
  --  1.3  31-Jan-2019  Lingaraj               CHG0044636 - Add new account to Get quote lines validation
  ----------------------------------------------------------------------------
  ...
  BEGIN
    ...
    IF p_request_source = g_strataforce_target THEN
      ...
      /* Start - CHG0044636 "SBQQ__Quote__c.X3rd_Party_Ship_To__c" Validate */
      IF l_header_rec.sf_3rd_party_ship_to IS NOT NULL OR
         l_header_rec.sf_3rd_party_ship_to_acc_id IS NOT NULL THEN
        BEGIN
          l_header_rec.sf_3rd_party_ship_to_acc_id := fetch_customer_account_id(l_header_rec.sf_3rd_party_ship_to_acc_id,
                                                                                l_header_rec.sf_3rd_party_ship_to);
        
        EXCEPTION
          WHEN no_data_found THEN
            x_status     := fnd_api.g_ret_sts_error;
            x_status_msg := x_status_msg ||
                            'ERROR: 3rd party Ship To account information not valid ' ||
                            chr(13);
        END;
      END IF;
      /* End - CHG0044636*/
             ...
             WHERE oh.header_id = l_header_rec.header_id
               AND xxhz_util.get_account_id_of_site_use_id(oh.ship_to_org_id) IN
                   (l_header_rec.sf_reseller_account_id,
                    l_header_rec.sf_3rd_party_account_id,
                    l_header_rec.sf_end_cust_account_id,
                    l_header_rec.sf_3rd_party_ship_to_acc_id --#CHG0044636
                    );
  ...
  --  1.2  24-Jan-2019 Diptasurjya             INC0145173 - Add validation for presence of parent_line_number
  --  1.3  18-Mar-2019 Diptasurjya             INC0150858 - Increment line number assignment variable
  --  1.4  15-Mar-2019 Diptasurjya             CHG0044970 - Assign different line number for lines without a number
  --                                                        Do not use liine number which were already sent back by pricing API
  ----------------------------------------------------------------------------
  ...
  PROCEDURE validate_order_line(p_header_rec     IN xxobjt.xxom_so_header_rec_type,
    ...
    l_max_line_num_papi NUMBER; -- CHG0044970 added
  ...
  BEGIN
    ...
    -- CHG0044970 start line number assignment change
    SELECT MAX(line_num)
      INTO l_max_line_num_papi
      FROM xx_qp_pricereq_lines
     WHERE request_number = p_header_rec.price_request_number
       AND request_source = g_strataforce_target;
  
    l_max_line_num := greatest(l_max_line_num, l_max_line_num_papi);
    -- CHG0044970 end
    ...
    FOR lr IN 1 .. l_line_tab.count LOOP
      ...
      IF l_line_tab(lr).line_number IS NULL THEN
        ...
        l_max_line_num := l_max_line_num + 1; -- INC0150858 increment counter so as to not assign same value to all lines
      END IF;
      ...
      */
      ...
      /*INC0145173 - Start - Add validation to check for presence of parent_line_number in received quote data*/
      IF l_line_tab(lr).parent_line_number IS NOT NULL THEN
        BEGIN
          SELECT 'Y'
            INTO l_is_valid
            FROM TABLE(CAST(l_line_tab AS xxom_so_lines_tab_type)) t1
           WHERE t1.external_ref_number = l_line_tab(lr).parent_line_number;
        EXCEPTION
          WHEN no_data_found THEN
            x_status     := fnd_api.g_ret_sts_error;
            x_status_msg := x_status_msg || 'ERROR: Parent line number: ' || l_line_tab(lr)
                           .parent_line_number || ' for : Item: ' || l_line_tab(lr).item ||
                            ' for line: ' || l_line_tab(lr).line_number ||
                            ' is not valid ' || chr(13);
        END;
      END IF;
      /* INC0145173 - End */
  ...
  --  1.2  10-APR-2019 Diptasurjya             CHG0044970 - Set direct/indirect deal DFF
  --  1.3  4-NOV-2019  Diptasurjya             CHG0046049 - Do not set deal type for PPU orders
  ----------------------------------------------------------------------------
  ...
  BEGIN
    ...
    IF p_request_source = g_strataforce_target THEN
      ...
      IF p_header_rec.operation <> 25 THEN
        -- CHG0046049 exclude PPU order for this setting
      ...
      -- CHG0044970 set Direct/Indirect deal DFF
      IF p_header_rec.attribute7 IS NOT NULL THEN
        SELECT decode(upper(p_header_rec.attribute7),
                      'DIRECT',
                      'Direct deal',
                      'INDIRECT',
                      'Indirect deal')
          INTO l_header_rec.attribute7
          FROM dual;
      END IF;
      -- CHG0044970 end
  ...
  --  2.2  05-Feb-2019 Diptasurjya             CHG0044970 - Service item handling
  --  2.3  26-SEP-2019 Diptasurjya             CHG0046549 - Set calculate price flag for PTO child lines
  --  2.4  04-NOV-2019 Diptasurjya             CHG0046049 - set return reason code for PPU orders
  ----------------------------------------------------------------------------
      ...
      SELECT bic.component_quantity,
       ...
       WHERE bbo.bill_sequence_id = bic.bill_sequence_id
         ...
         AND msib_c.inventory_item_id = bic.component_item_id
            --AND SYSDATE BETWEEN nvl(bic.implementation_date, SYSDATE - 1) AND nvl(bic.disable_date, SYSDATE + 1)  -- CHG0048964 comment
         AND SYSDATE BETWEEN bic.implementation_date AND
             nvl(bic.disable_date, SYSDATE + 1) -- CHG0048964 add
  ...
  BEGIN
    ...
    IF p_request_source = g_strataforce_target THEN
      ...
      FOR lr IN 1 .. p_line_tab.count LOOP
        ...
        l_line_tbl(lr).header_id := p_header_rec.header_id; -- CHG0044970 added
        ...
        -- CHG0044970 start new field assignment
        IF p_line_tab(lr).payment_term_id IS NOT NULL THEN
          l_line_tbl(lr).payment_term_id := p_line_tab(lr).payment_term_id;
        END IF;
        -- CHG0044970 end
        ...
        ELSIF p_header_rec.operation = 20 AND p_line_tab(lr)
             .ordered_quantity < 0 THEN
          --CHG0043970
          l_line_tbl(lr).return_reason_code := fnd_profile.value('XXOM_LINE_RETURN_REASON_CODE'); --CHG0043970
        ELSIF p_header_rec.operation = 25 AND p_line_tab(lr)
             .ordered_quantity < 0 THEN
          -- CHG0046049 add return reason setup for PPU returns
          l_line_tbl(lr).return_reason_code := 'PPU'; -- CHG0046049 added
        END IF;
        ...
        IF upper(p_line_tab(lr).sf_rel_syst_required) = 'TRUE' THEN
          IF p_line_tab(lr).parent_line_number IS NULL AND p_line_tab(lr)
             .oracle_required_by IS NULL THEN
          
            IF p_line_tab(lr).sf_activity_analysis = 'Contracts' THEN
              l_line_tbl(lr).service_reference_type_code := 'CUSTOMER_PRODUCT';
              l_line_tbl(lr).service_reference_line_id := p_line_tab(lr)
                                                          .sf_serial_external_key;
              l_line_tbl(lr).service_start_date := p_line_tab(lr)
                                                   .sbqq_start_date;
              l_line_tbl(lr).service_end_date := p_line_tab(lr)
                                                 .sbqq_end_date;
              l_line_tbl(lr).accounting_rule_id := 1000;
              --l_line_tbl(lr).item_type_code := 'SERVICE'; -- CHG0044970 commented as Oracle sets this automatically
              l_line_tbl(lr).unit_list_price := p_line_tab(lr)
                                                .unit_list_price * p_line_tab(lr)
                                                .ordered_quantity; -- INC0144895 added
              l_line_tbl(lr).unit_selling_price := (p_line_tab(lr)
                                                   .unit_selling_price -
                                                    nvl(p_line_tab(lr)
                                                        .manual_adjustment_amt,
                                                        0)) -- 23jan2019 manual adj added
                                                   * p_line_tab(lr)
                                                  .ordered_quantity; -- INC0144895 added
              l_line_tbl(lr).ordered_quantity := 1;
              l_line_tbl(lr).calculate_price_flag := 'N'; -- INC0144895 added
            END IF;
            -- CHG0044970 - handle assignment of Service item assignment to order lines
          ELSE
            IF p_line_tab(lr).sf_activity_analysis = 'Contracts' THEN
              BEGIN
                SELECT ol_child.line_id
                  INTO l_line_tbl(lr).service_reference_line_id
                  FROM oe_order_lines_all ol, oe_order_lines_all ol_child
                 WHERE ol.header_id = p_header_rec.header_id
                   AND ol.orig_sys_line_ref =
                       nvl(p_line_tab(lr).parent_line_number,
                           p_line_tab(lr).oracle_required_by)
                   AND ol.header_id = ol_child.header_id
                   AND ol_child.top_model_line_id = ol.line_id
                   AND xxinv_utils_pkg.get_category_value(1100000222,
                                                          ol_child.inventory_item_id,
                                                          g_master_invorg_id) =
                       'Systems (net)'
                   AND xxssys_strataforce_events_pkg.is_bom_valid(ol_child.inventory_item_id,
                                                                  g_master_invorg_id) = 'N';
              EXCEPTION
                WHEN no_data_found THEN
                  BEGIN
                    SELECT ol.line_id
                  ...
                  EXCEPTION
                    WHEN no_data_found THEN
                      raise_application_error(-20001,
                                              'ERROR: While preparing service line input data');
                  END;
              END;
            
              l_line_tbl(lr).service_reference_type_code := 'ORDER';
              l_line_tbl(lr).service_duration := p_line_tab(lr)
                                                 .ordered_quantity;
              l_line_tbl(lr).service_period := p_line_tab(lr).item_uom;
              l_line_tbl(lr).accounting_rule_id := 1000;
              l_line_tbl(lr).ordered_quantity := 1;
              l_line_tbl(lr).unit_list_price := p_line_tab(lr)
                                                .unit_list_price * p_line_tab(lr)
              ...
              l_line_tbl(lr).unit_selling_price := (p_line_tab(lr)
                                                  ...
                                                  .ordered_quantity;
              l_line_tbl(lr).ordered_quantity := 1;
              l_line_tbl(lr).calculate_price_flag := 'N';
            END IF;
            -- CHG0044970 - end
          END IF;
        END IF;
    ...
    FOR i IN 1 .. l_line_tbl.count LOOP
      ...
      IF is_item_bom_type_valid(l_line_tbl(i).inventory_item_id) = 'Y' THEN
        ...
        FOR rec_model IN cur_model_items(l_line_tbl(i).inventory_item_id) LOOP
          ...
          l_line_tbl(l_last_line_index).ordered_quantity := rec_model.component_quantity * l_line_tbl(i)
                                                           .ordered_quantity;
          l_line_tbl(l_last_line_index).top_model_line_index := i;
          l_line_tbl(l_last_line_index).link_to_line_index := i;
          -- CHG0046549 start - added below section to set pricing mode for PTO included lines
          IF p_header_rec.calculate_price_flag <> 'Y' THEN
            l_line_tbl(l_last_line_index).unit_list_price := 0;
            l_line_tbl(l_last_line_index).unit_selling_price := 0;
          END IF;
          l_line_tbl(l_last_line_index).calculate_price_flag := p_header_rec.calculate_price_flag;
          -- CHG0046549 end
  ...
  --  1.1  24/01/2019  Diptasurjya             INC0145173 - Lines to be updated with orig_sys_line_ref
  --                                           should be placed within the line delete loop
  --                                           In case multiple PTO KIT systems are ordered on same quote, this procedure
  --                                           is updating the orig_sys_line_ref for SC item associated to only the last PTO KIT system
  --                                           To fix this the update record preparation for API needs to be performed within the same
  --                                           loop which deletes lines, so that for every line deleted we can check and update orig_sys_line_ref
  --                                           properly
  --                                           Also in case multiple SC items are associated to same PTO KIT system line, handling needs
  --                                           to be done for update, so that we do not update the same SC line multiple times
  ----------------------------------------------------------------------------
  ...
  FUNCTION prepare_line_delete_data(p_header_id      IN NUMBER,
    ...
    CURSOR cur_line_to_delete IS
      SELECT ol.line_id,
             ...
             ol_parent.line_id    top_model_line_id,
             oh.attribute4,
             ol.inventory_item_id, -- INC0145173 added
             ol.service_duration, -- INC0145173 added
             ol.service_period -- INC0145173 added
        FROM oe_order_lines_all   ol,
       ...
       WHERE oh.header_id = p_header_id
         ...
         AND (xxssys_strataforce_events_pkg.is_bom_valid(p_inventory_item_id => ol_serv.inventory_item_id,
                                                         p_organization_id   => g_master_invorg_id) = 'Y' OR
             (xxinv_utils_pkg.get_category_value(1100000222,
                                                  ol_serv.inventory_item_id,
                                                  g_master_invorg_id) <>
             'Systems (net)' AND
             ol_serv.top_model_line_id = ol_parent.line_id));
  
    l_line_tbl oe_order_pub.line_tbl_type;
  
    l_tbl_indx NUMBER := 0;
  
    l_line_id_for_upd     NUMBER; --INC0145173 added
    l_line_exists_for_upd VARCHAR2(1); --INC0145173 added
  ...
  BEGIN
    IF p_request_source = g_strataforce_target THEN
      FOR rec_line_to_delete IN cur_line_to_delete LOOP
        ...
        l_line_tbl(l_tbl_indx).header_id := p_header_id;
        l_line_tbl(l_tbl_indx).line_id := rec_line_to_delete.line_id;
        ...
        BEGIN
          SELECT 'Y'
           ...
           WHERE xqm.price_request_number = rec_line_to_delete.attribute4
             AND xqm.external_ref_number =
                 rec_line_to_delete.orig_sys_line_ref;
        EXCEPTION
          WHEN no_data_found THEN
        ...
        IF l_exists_in_quote_bkp = 'Y' THEN
          l_line_id_for_upd := NULL;
        
          -- INC0145173 Start - move update block from outside this for loop to inside
          -- Update orig sys ref from deleted line to the remaining line
          ------------------------------------------------------------------------------
          -- For Loop Logic - Below loop will find all SERVICE lines which are associated to non-PTO, system lines
          -- having parent PTO line same as the line which is being deleted
          FOR line_ids_rec IN (SELECT ol_serv.line_id
                                 FROM oe_order_lines_all ol,
                                      oe_order_lines_all ol_serv
                                WHERE ol.top_model_line_id =
                                      rec_line_to_delete.top_model_line_id
                                  AND ol.line_id =
                                      ol_serv.service_reference_line_id
                                  AND ol_serv.service_reference_type_code =
                                      'ORDER'
                                  AND ol_serv.item_type_code = 'SERVICE'
                                  AND ol_serv.inventory_item_id =
                                      rec_line_to_delete.inventory_item_id
                                  AND ol_serv.service_duration =
                                      rec_line_to_delete.service_duration
                                  AND ol_serv.service_period =
                                      rec_line_to_delete.service_period
                                  ...
                                  AND xxinv_utils_pkg.get_category_value(1100000222,
                                                                         ...
                                                                         g_master_invorg_id) =
                                      'Systems (net)') LOOP
            l_line_exists_for_upd := 'N';
          
            -- For Loop Logic - Below loop is to ensure that we are not updating the same line ID multiple times
            -- This is to take care of the scenario where multiple SC items are associated to same system PTO KIT line
            FOR i IN 1 .. l_line_tbl.count LOOP
              IF l_line_tbl(i).line_id = line_ids_rec.line_id AND l_line_tbl(i)
                 .operation = oe_globals.g_opr_update THEN
                l_line_exists_for_upd := 'Y';
                EXIT;
              END IF;
            END LOOP;
          
            IF l_line_exists_for_upd = 'N' THEN
              l_line_id_for_upd := line_ids_rec.line_id;
              EXIT;
            END IF;
          END LOOP;
        
          -- If line to be updated is no null, prepare table type for update of orig_sys_line_ref
          IF l_line_id_for_upd IS NOT NULL THEN
            l_tbl_indx := l_tbl_indx + 1;
            l_line_tbl(l_tbl_indx) := oe_order_pub.g_miss_line_rec;
            l_line_tbl(l_tbl_indx).operation := oe_globals.g_opr_update;
            l_line_tbl(l_tbl_indx).header_id := p_header_id;
            l_line_tbl(l_tbl_indx).line_id := l_line_id_for_upd;
            l_line_tbl(l_tbl_indx).orig_sys_line_ref := rec_line_to_delete.orig_sys_line_ref;
          
            SELECT xqm.unit_list_price * xqm.ordered_quantity,
                   xqm.unit_selling_price * xqm.ordered_quantity
              INTO l_line_tbl(l_tbl_indx).unit_list_price,
                   l_line_tbl(l_tbl_indx).unit_selling_price
              FROM xxcpq_quote_lines_mirr xqm
             WHERE xqm.price_request_number = rec_line_to_delete.attribute4
               AND xqm.external_ref_number =
                   rec_line_to_delete.orig_sys_line_ref;
          
            l_line_tbl(l_tbl_indx).calculate_price_flag := 'N';
          END IF;
          -- INC0145173 end
        END IF;
      END LOOP;
  ...
  --  1.2  05-Feb-2019 Diptasurjya             CHG0044970 - Handle service contract items
  ----------------------------------------------------------------------------
    ...
    CURSOR c_adjustment(p_request_number IN VARCHAR2) IS
      SELECT *
       ...
       (SELECT 1
               ...
               WHERE t1.line_number = adj.line_num) -- CHG0044970
       ORDER BY line_num, line_adj_num;
      ...
      FOR k IN 1 .. p_line_tab.count LOOP
        IF p_line_tab(k).manual_adjustment_amt IS NOT NULL AND p_line_tab(k)
          ...
          IF p_line_tab(k).sf_activity_analysis = 'Contracts' THEN
            l_manual_adj_amt := p_line_tab(k).manual_adjustment_amt * p_line_tab(k)
                                .ordered_quantity;
          END IF;
          -- INC0144895 end
        
          l_line_adj_tbl(l_adj_inx).adjusted_amount := -1 *
                                                       l_manual_adj_amt;
          l_line_adj_tbl(l_adj_inx).adjusted_amount_per_pqty := -1 *
                                                                l_manual_adj_amt;
  ...
  FUNCTION prepare_line_adj_att_data(p_header_rec     IN xxobjt.xxom_so_header_rec_type,
    ...
    CURSOR c_attributes(p_request_number IN VARCHAR2) IS
      SELECT *
       ...
       (SELECT 1
                FROM TABLE(CAST(p_line_tab AS xxom_so_lines_tab_type)) t1
               WHERE t1.line_number = attrib.line_num) -- CHG0044970
       ORDER BY line_num, line_adj_num;
  ...
  FUNCTION prepare_line_adj_assoc_data(p_header_rec     IN xxobjt.xxom_so_header_rec_type,
    ...
    CURSOR c_assoc(p_request_number IN VARCHAR2) IS
      SELECT *
       ...
       (SELECT 1
                FROM TABLE(CAST(p_line_tab AS xxom_so_lines_tab_type)) t1
               WHERE t1.line_number = assoc.related_line_num) -- CHG0044970
         AND EXISTS
       (SELECT 1
                FROM TABLE(CAST(p_line_tab AS xxom_so_lines_tab_type)) t2
               WHERE t2.line_number = assoc.line_num) -- CHG0044970
       ORDER BY line_num, line_adj_num;
  ...
  BEGIN
    ...
    FOR i IN 1 .. l_line_tbl.count LOOP
      ...
      IF upper(l_line_tbl(i).sf_rel_syst_required) = 'TRUE' THEN
        ...
        IF l_line_tbl(i).parent_line_number IS NULL AND l_line_tbl(i)
           .oracle_required_by IS NULL --29OCT18 for #CHG0044300
         THEN
          IF l_line_tbl(i).sf_activity_analysis <> 'Contracts' THEN
            -- CHG0044253 - DFF not updated for Contracts
            ...
            l_line_api_tbl(l_line_index).attribute14 := l_line_tbl(i)
                                                        .serial_number;
          
            IF l_line_tbl(i).sf_serial_external_key IS NOT NULL THEN
              l_line_api_tbl(l_line_index).attribute1 := l_line_tbl(i)
                                                         .sf_serial_external_key; -- INC0132115
            END IF;
            ...
            FOR pto_rec IN (SELECT line_id, inventory_item_id
              ...
              IF xxinv_utils_pkg.get_category_value(1100000222,
                ...
                IF l_line_tbl(i).sf_activity_analysis <> 'Contracts' THEN
                  ...
                  l_line_api_tbl(l_line_index) := oe_order_pub.g_miss_line_rec;
                  l_line_api_tbl(l_line_index).operation := oe_globals.g_opr_update;
                  l_line_api_tbl(l_line_index).line_id := l_line_tbl(i)
                                                          .line_id;
  ...
  --  1.1 20-Nov-2018    Lingaraj             CHG0044433 - Get Quote Lines
  --  1.2 31-Jan-2019    Lingaraj             CHG0044636 - Add new account to Get quote lines validation
  --  1.3 05-Feb-2019    Diptasurjya          CHG0044970 - Add extra fields in bkp
  ----------------------------------------------------------------------
  ...
  BEGIN
    ...
    INSERT INTO xxobjt.xxcpq_quote_header_mirr
      (source_name,
       ...
       sf_3rd_party_ship_to, --#CHG0044636
       sf_3rd_party_ship_to_acc_id, --#CHG0044636
       attribute1,
       attribute2,
       attribute4,
       attribute6,
    ...
    VALUES
      (p_request_source,
       ...
       p_header_rec.sf_3rd_party_ship_to, --#CHG0044636
       p_header_rec.sf_3rd_party_ship_to_acc_id, --#CHG0044636
    ...
    FOR i IN 1 .. p_line_tab.count() LOOP
      INSERT INTO xxobjt.xxcpq_quote_lines_mirr
        (source_name,
         ...
         attribute14,
         attribute15,
         oracle_required_by, -- CHG0044970
         sbqq_start_date, -- CHG0044970
         sbqq_end_date, -- CHG0044970
         payment_term_id, -- CHG0044970
         --
         creation_date,
         created_by,
         last_updated_by,
      ...
      VALUES
        (p_request_source,
         ...
         p_line_tab(i).oracle_required_by, -- CHG0044970
         p_line_tab(i).sbqq_start_date, -- CHG0044970
         p_line_tab(i).sbqq_end_date, -- CHG0044970
         p_line_tab(i).payment_term_id, -- CHG0044970
         --
         SYSDATE,
         to_number(fnd_profile.value('USER_ID')),
         to_number(fnd_profile.value('USER_ID')),
  ...
  ----------------------------------------------------------------------------
  --  name:          create_non_service_lines
  --  created by:    Diptasurjya Chatterjee
  --  Revision       1.0
  --  creation date: 05/02/2019
  ----------------------------------------------------------------------------
  --  purpose :      CHG0044970: Add non-service item lines to the order and update
  --                 order header with required details
  ----------------------------------------------------------------------------
  --  ver  date        name                    desc
  --  1.0  05/02/2019  Diptasurjya Chatterjee  CHG0044970 - initial build
  ----------------------------------------------------------------------------
  PROCEDURE create_non_service_lines(p_header_rec     IN xxobjt.xxom_so_header_rec_type,
                                     p_line_tab       IN xxobjt.xxom_so_lines_tab_type,
                                     p_request_source IN VARCHAR2,
                                     p_err_code       OUT VARCHAR2,
                                     p_err_message    OUT VARCHAR2,
                                     p_order_number   OUT NUMBER,
                                     p_header_id      OUT NUMBER) IS
    l_action_request_tbl oe_order_pub.request_tbl_type;
    ...
    l_cust_line_tab xxobjt.xxom_so_lines_tab_type := xxom_so_lines_tab_type();
    l_line_eligible VARCHAR2(1) := 'N';
  ...
  BEGIN
    IF p_request_source = g_strataforce_target THEN
      ...
      oe_debug_pub.add('SSYS CUSTOM: Preparing input types for xxom_salesorder_api.create_non_service_lines for non-service line processing');
      BEGIN
        FOR l IN 1 .. p_line_tab.count LOOP
          l_line_eligible := 'N';
        
          IF nvl(p_line_tab(l).sf_activity_analysis, 'x') <> 'Contracts' THEN
            BEGIN
              SELECT 'N'
                INTO l_line_eligible
                FROM TABLE(CAST(p_line_tab AS xxom_so_lines_tab_type)) t1
               WHERE t1.external_ref_number =
                     nvl(p_line_tab(l).parent_line_number,
                         p_line_tab(l).oracle_required_by)
                 AND t1.sf_activity_analysis = 'Contracts';
            EXCEPTION
              WHEN no_data_found THEN
                l_line_eligible := 'Y';
            END;
            IF l_line_eligible = 'Y' THEN
              l_cust_line_tab.extend;
              l_cust_line_tab(l_cust_line_tab.count) := p_line_tab(l);
            END IF;
          END IF;
        END LOOP;
        ...
        l_is_order_dg_eligible := is_order_dg_eligible(p_line_tab,
                                                       p_header_rec.header_id); -- CHG0044403 added
        ...
        BEGIN
          l_pricing_maint_status := pricing_temp_housekeep(p_header_rec,
                                                           p_line_tab);
        EXCEPTION
          WHEN OTHERS THEN
            p_err_code    := fnd_api.g_ret_sts_error;
        ...
        l_header_rec := prepare_header_data(p_header_rec,
                                            p_request_source,
                                            oe_globals.g_opr_update,
                                            l_is_order_dg_eligible);
        l_line_tbl   := prepare_line_data(p_header_rec,
                                          l_cust_line_tab,
                                          p_request_source,
                                          oe_globals.g_opr_create);
      
        IF p_header_rec.calculate_price_flag <> 'Y' THEN
          l_header_adj_tbl        := prepare_head_adj_data(p_header_rec,
                                                           p_request_source,
                                                           oe_globals.g_opr_create);
          l_header_adj_attrib_tbl := prepare_head_adj_att_data(p_header_rec,
                                                               p_request_source,
                                                               oe_globals.g_opr_create);
          l_header_prc_att_tbl    := prepare_head_prc_att_data(p_header_rec,
                                                               p_request_source,
                                                               oe_globals.g_opr_create);
          l_line_adj_tbl          := prepare_line_adj_data(p_header_rec,
                                                           l_cust_line_tab,
                                                           p_request_source,
                                                           oe_globals.g_opr_create);
          l_line_adj_attrib_tbl   := prepare_line_adj_att_data(p_header_rec,
                                                               l_cust_line_tab,
                                                               p_request_source,
                                                               oe_globals.g_opr_create);
          l_line_adj_assoc_tbl    := prepare_line_adj_assoc_data(p_header_rec,
                                                                 l_cust_line_tab,
                                                                 p_request_source,
                                                                 oe_globals.g_opr_create);
        ELSE
      ...
      EXCEPTION
        WHEN OTHERS THEN
          p_err_code    := fnd_api.g_ret_sts_error;
          p_err_message := 'UNEXPECTED ERROR: While preparing non-service item input types. ' ||
                           SQLERRM;
          oe_debug_pub.add('SSYS CUSTOM: ' || p_err_message);
          oe_debug_pub.add('SSYS CUSTOM: Exiting program xxom_salesorder_api.create_non_service_lines');
          RETURN;
      END;
      ...
      IF l_api_status = fnd_api.g_ret_sts_success THEN
        ...
        p_header_id    := l_api_order_header_id;
      ELSE
        p_err_code    := l_api_status;
        p_err_message := l_api_message;
      END IF;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      p_err_code    := fnd_api.g_ret_sts_error;
      p_err_message := 'UNEXPECTED ERROR: In non-service line creation ' ||
                       SQLERRM;
  END create_non_service_lines;
  ...
  --  creation date: 05/02/2019
  ----------------------------------------------------------------------------
  --  purpose :      CHG0044970: Add service item lines to the order
  ----------------------------------------------------------------------------
  --  ver  date        name                    desc
  --  1.0  05/02/2019  Diptasurjya Chatterjee  CHG0044970 - initial build
  ----------------------------------------------------------------------------
  PROCEDURE create_service_lines(p_header_rec     IN xxobjt.xxom_so_header_rec_type,
                                 p_line_tab       IN xxobjt.xxom_so_lines_tab_type,
                                 p_request_source IN VARCHAR2,
                                 p_err_code       OUT VARCHAR2,
                                 p_err_message    OUT VARCHAR2) IS
  ...
  BEGIN
    IF p_request_source = g_strataforce_target THEN
      -- prepare all input table and record types to process_order API
      oe_debug_pub.add('SSYS CUSTOM: Preparing input types for xxom_salesorder_api.create_service_lines for service line processing');
      BEGIN
        FOR l IN 1 .. p_line_tab.count LOOP
          l_line_eligible := 'N';
        
          IF p_line_tab(l).sf_activity_analysis = 'Contracts' THEN
            l_line_eligible := 'Y';
          ELSE
            BEGIN
              SELECT 'Y'
      ...
      EXCEPTION
        WHEN OTHERS THEN
          p_err_code    := fnd_api.g_ret_sts_error;
          p_err_message := 'UNEXPECTED ERROR: While preparing service item input types. ' ||
                           SQLERRM;
          oe_debug_pub.add('SSYS CUSTOM: ' || p_err_message);
          oe_debug_pub.add('SSYS CUSTOM: Exiting program xxom_salesorder_api.create_service_lines');
          RETURN;
      END;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      p_err_code    := fnd_api.g_ret_sts_error;
      p_err_message := 'UNEXPECTED ERROR: In service line creation ' ||
                       SQLERRM;
  ...
  --  1.6  05-Feb-2019 Diptasurjya             CHG0044970 - Service contract line handling
  ----------------------------------------------------------------------------
  ...
  PROCEDURE process_order(p_header_rec      IN xxobjt.xxom_so_header_rec_type,
    ...
    l_serv_line_add_status    VARCHAR2(1); -- CHG0044970
    l_nonserv_line_add_status VARCHAR2(1); -- CHG0044970
    ...
    l_serv_line_add_status_msg    VARCHAR2(2000); -- CHG0044970
    l_nonserv_line_add_status_msg VARCHAR2(2000); -- CHG0044970
    ...
    -- CHG0044970 commented below variables
    --l_action_request_tbl oe_order_pub.request_tbl_type;
  ...
  BEGIN
    ...
    IF p_request_source = g_strataforce_target THEN
      -- CHG0044970 - Send the logic of creating non-service lines to seperate procedure
      create_non_service_lines(p_header_rec     => l_cust_header_rec,
                               p_line_tab       => l_cust_line_tab,
                               p_request_source => p_request_source,
                               p_err_code       => l_nonserv_line_add_status,
                               p_err_message    => l_nonserv_line_add_status_msg,
                               p_order_number   => l_api_order_number,
                               p_header_id      => l_api_order_header_id);
    
      IF l_nonserv_line_add_status = fnd_api.g_ret_sts_success THEN
        -- CHG0044970 change vaariable name for status check
        -- CHG0044970 - add below call to create service lines on the order
        create_service_lines(p_header_rec     => l_cust_header_rec,
                             p_line_tab       => l_cust_line_tab,
                             p_request_source => p_request_source,
                             p_err_code       => l_serv_line_add_status,
                             p_err_message    => l_serv_line_add_status_msg);
      
        IF l_serv_line_add_status = fnd_api.g_ret_sts_success THEN
          -- CHG0044970 end
          -- CHG0044253 - add below order line delete call
          -- Delete Order lines if required
          -- CHG0044970 - commented delete lines code
          ...
          split_line(p_header_id      => l_cust_header_rec.header_id, -- CHG0044970 change header rec variable
                     p_request_source => p_request_source,
          ...
          IF l_line_split_status = fnd_api.g_ret_sts_success THEN
            ...
            IF is_eligible_for_avg_disc(l_cust_header_rec.header_id) = 1 THEN
              -- CHG0044970 change header rec variable
              update_avg_discount_dff(p_header_id   => l_cust_header_rec.header_id, -- CHG0044970 change header rec variable
            ...
            IF l_avg_disc_upd_status = fnd_api.g_ret_sts_success THEN
              -- Update line DFFs
              update_line_dff(p_header_id   => l_cust_header_rec.header_id, -- CHG0044970 change header rec variable
          ...
          ELSE
            ...
            ROLLBACK;
          END IF;
          -- CHG0044970 - remove delete line handler
          ...
          -- CHG0044970 - Service line else condition
        ELSE
          p_err_code    := l_serv_line_add_status;
          p_err_message := 'ERROR: Operation: Service Line Add: ' ||
                           l_serv_line_add_status_msg;
          ROLLBACK;
        END IF;
        -- CHG0044970 end
      ELSE
        p_err_code    := l_nonserv_line_add_status; -- CHG0044970 change variable name
        p_err_message := 'ERROR: Operation: Line Import: ' ||
                         l_nonserv_line_add_status_msg; -- CHG0044970 change variable name
        ROLLBACK;
      END IF;
    END IF;
  ...
  --  1.1 20-NOV-2018    Lingaraj             CHG0044433 - Get Quote Lines
  --                                                reuse the same Quote number when Order lines are cancelled
  --  2.0 12/01/2021     Roman W.             CHG0048579 - OIC (JIRA : OIC-33 )
  ----------------------------------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    -- added by Roman W. CHG0048579 12/01/2021
    l_oic_enable_flag := xxssys_oic_util_pkg.get_service_oic_enable_flag(p_service => g_quote_lines);
  
    IF 'Y' = l_oic_enable_flag THEN
      process_quote_lines_oic(p_header_id    => p_order_header_id,
                              p_user_id      => l_user_id,
                              p_resp_id      => l_resp_id,
                              p_appl_id      => l_appl_id,
    ...
    <quot:source_reference_id>' ||
                             l_quote_number || '</quot:source_reference_id>
          </quot:HeaderInfo>
          <quot:Quote_Details>
  ...
  -- Ver   When         Who         Desc
  -- ----  -----------  ----------  ----------------------------------------------
  -- 1.0   13/01/2021   Roman W.    CHG0048579 - OIC (JIRA : OIC-242)
  --------------------------------------------------------------------------------
  PROCEDURE get_response_error(p_response_text       IN VARCHAR2,
                               p_response_errcode    OUT VARCHAR2,
                               p_response_errmessage OUT VARCHAR2,
  ...
  BEGIN
    ...
    l_xmltype := xmltype.createxml(p_response_text);
  
    SELECT TRIM(xt.errcode), TRIM(xt.errmessage)
      INTO p_response_errcode, p_response_errmessage
      FROM xmltable('processQuoteLineResponse/quoteDetails' passing
                    l_xmltype columns errcode VARCHAR2(300) path 'errCode',
                    errmessage VARCHAR2(2000) path 'errMessage') xt;
  
  EXCEPTION
    WHEN xml_parsing_failed THEN
      xxssys_oic_util_pkg.html_parser(p_in_text    => p_response_text,
                                      p_out_text   => l_text_out,
      ...
      ELSE
        p_error_code := 2;
        p_error_desc := l_text_out;
      END IF;
    
    WHEN OTHERS THEN
      message('sqlerrm - ' || SQLERRM);
      message('SQLCODE - ' || SQLCODE);
      message('------------------------------------');
      p_error_code := 2;
      p_error_desc := 'EXCEPTION_OTHERS xxom_salesorder_api.get_response_error(' ||
                      p_response_text || ') - ' || SQLERRM;
  ...
  -- Ver   When        Who        Descr
  -- ----  ----------  ---------  ----------------------------------------------------------
  -- 1.0   12/01/2021  Roman W.   CHG0048579 - OIC (JIRA : OIC-242)
  -- 1.1   26/07/2021  Roman W.   CHG0049588 - p_waller_loc=> p_wallet_loc bug fix
  ------------------------------------------------------------------------------------------
  PROCEDURE process_quote_lines_oic(p_header_id    IN VARCHAR2,
    ...
    l_url                 VARCHAR2(240);
    l_wallet_loc          VARCHAR2(240);
    l_wallet_pwd          VARCHAR2(240);
    l_auth_user           VARCHAR2(240);
    l_auth_pwd            VARCHAR2(240);
    l_http_request        utl_http.req;
    l_http_response       utl_http.resp;
    l_http_response_text  VARCHAR2(32766);
    l_response_errcode    VARCHAR2(10);
    l_response_errmessage VARCHAR2(2000);
    l_error_code          VARCHAR2(10);
    ------------------------------
    --      Code Section
    ------------------------------
  BEGIN
    ...
    xxssys_oic_util_pkg.get_service_details(p_service     => g_quote_lines,
    ...
    message('p_url         =>' || l_url);
    --   message('p_waller_loc  =>' || l_waller_loc);
    --   message('p_wallet_pwd  =>' || l_wallet_pwd);
    --   message('p_auth_user   =>' || l_auth_user);
    --   message('p_auth_pwd    =>' || l_auth_pwd);
  
    l_body := '<processQuoteLineRequest>
	      <headerInfo>
	        <sourceName>STRATAFORCE</sourceName>
	        <sourceReferenceId>' || p_quote_number ||
              '</sourceReferenceId>
	      </headerInfo>
	      <quoteDetails>
	        <soHeaderId>' || p_header_id || '</soHeaderId>
	        <userId>' || p_user_id || '</userId>
	        <respId>' || p_resp_id || '</respId>
	        <applId>' || p_appl_id || '</applId>
	        <quoteNumber>' || p_quote_number ||
              '</quoteNumber>
	        <orderNumber>' || p_order_number ||
              '</orderNumber>
	      </quoteDetails>
	    </processQuoteLineRequest>';
    ...
    message('------------------------');
    utl_http.read_text(l_http_response, l_http_response_text, 32766);
    message('RESPONSE : ' || chr(10) || l_http_response_text);
  
    get_response_error(p_response_text       => l_http_response_text,
                       p_response_errcode    => l_response_errcode,
                       p_response_errmessage => l_response_errmessage,
    ...
    p_error_desc := l_response_errmessage;
    IF l_response_errcode IS NULL THEN
      p_error_code := 2;
    
    ELSE
      IF l_response_errcode = fnd_api.g_ret_sts_error THEN
        p_error_code := 2;
      ELSE
        p_error_code := 0;
      END IF;
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      p_error_code := 2;
      p_error_desc := 'EXCEPTION_OTHERS xxom_salesorder_api.process_quote_lines_oic() - ' ||
                      SQLERRM;
  END process_quote_lines_oic;

  ----------------------------------------------------------------------------------------------
  --  name:          is_diff_amount_in_tolerance
  --  created by:    Lingaraj Sarangi
  --  Revision       1.0
  ...
  ----------------------------------------------------------------------
  --  ver  date          name              Desc
  --  1.0 30-Jan-2019    Lingaraj          CHG0044825 - On Order book validation with source STRATAFORCE,
  --                                       allow up to 10000 KRW difference between Order amount and Quote amount
  --  1.1 28.3.19         yuval tal        INC0152058 - change logic
  ----------------------------------------------------------------------------------------------
  PROCEDURE is_diff_amount_in_tolerance(p_order_header_id NUMBER,
                                        p_order_amount    VARCHAR2,
                                        p_quote_amount    VARCHAR2,
                                        x_error           OUT VARCHAR2,
                                        x_in_tolerance    OUT VARCHAR2) IS
    l_sfdc_amount_diff_allowed NUMBER;
    l_currency_code            VARCHAR2(5);
    l_order_number             NUMBER;
    l_diff_abs_value           NUMBER := abs(p_order_amount -
                                             p_quote_amount);
  BEGIN
    x_in_tolerance := 'N';
    SELECT nvl(sfdc_amount_diff_allowed, 0),
           fcv.currency_code,
           oh.order_number
      INTO l_sfdc_amount_diff_allowed, l_currency_code, l_order_number
      FROM fnd_currencies_vl    fcv,
           fnd_currencies_dfv   fcd,
           oe_order_headers_all oh
     WHERE fcv.row_id = fcd.row_id
       AND fcv.currency_code = oh.transactional_curr_code
       AND oh.header_id = p_order_header_id
       AND nvl(fcv.enabled_flag, 'N') = 'Y'
       AND trunc(SYSDATE) <= trunc(nvl(fcv.end_date_active, (SYSDATE + 1)));
  
    -- INC0152058
    IF l_diff_abs_value > l_sfdc_amount_diff_allowed THEN
      x_error        := 'Total Quote Amount of:' || p_quote_amount ||
                        ' and total Sales Order Amount of:' ||
                        p_order_amount ||
                        ',does not match. Tollerance allowed in difference of value is :' ||
                        l_sfdc_amount_diff_allowed || '.';
      x_in_tolerance := 'N';
    ELSE
      x_in_tolerance := 'Y';
      x_error        := NULL;
    ...
    /*IF l_sfdc_amount_diff_allowed = 0 and l_diff_abs_value = 0 Then
        x_in_tolerance := 'Y';
        x_error := '';
    ELSIF  l_sfdc_amount_diff_allowed = 0 and l_diff_abs_value != 0 Then
        x_in_tolerance := 'N';
    ELSIF  l_sfdc_amount_diff_allowed != 0 and l_diff_abs_value != 0 Then
           IF  l_diff_abs_value <=  l_sfdc_amount_diff_allowed THEN
                x_in_tolerance := 'Y';
  ...
  EXCEPTION
    WHEN no_data_found THEN
      x_error := 'xxom_salesorder_api.is_diff_amount_in_tolerance :' ||
                 'No Active record found in fnd_currencies_vl Table, for the Currency Code :' ||
                 l_currency_code || ' And Order Number :' || l_order_number;
    WHEN OTHERS THEN
      x_error := 'UNEXPECTED ERROR in xxom_salesorder_api.is_diff_amount_in_tolerance :' ||
                 SQLERRM;
  END is_diff_amount_in_tolerance;
  ...
  --  1.6 30-Jan-2019   LINGARAJ           CHG0044825 - On Order book validation with source STRATAFORCE,
  --                                       allow up to 10000 KRW difference between Order amount and Quote amount
  --  1.7 31-Jan-2019   LINGARAJ           CHG0044636-Add new account to Get quote lines validation
  --  1.8 04-NOV-2020   Diptasurjya        INC0211533 - Bug - If SHIP and RETURN line exist on order for same item, validation fails
  --                                                   due to item mismatch between quote and order.
  ----------------------------------------------------------------------
  FUNCTION validate_quote(p_header_id      NUMBER,
    ...
    CURSOR c_all_product_exits1 IS
      ...
      SELECT msib.segment1 item,
             SUM(decode(line_category_code,
                        ...
                        (decode(nvl(ol.service_reference_type_code, 'X'),
                                'ORDER',
                                ol.service_duration,
                                ol.ordered_quantity)) * -1, -- INC0145470 added
                        (decode(nvl(ol.service_reference_type_code, 'X'),
                                'ORDER',
                                ol.service_duration,
                                ol.ordered_quantity)) -- INC0145470 added
                        )) ordered_quantity
      ...
      ; -- INC0211533 comment ol.line_category_code
      ...
      SELECT msib.segment1 item,
       ...
       WHERE ol.header_id = p_header_id
         ...
         AND msib.item_type NOT IN
             (fnd_profile.value('XXAR PREPAYMENT ITEM TYPES'),
              fnd_profile.value('XXAR_FREIGHT_AR_ITEM'))
       GROUP BY msib.segment1 --, ol.line_category_code -- INC0211533 comment ol.line_category_code
      MINUS
      SELECT ql.item, SUM(ql.ordered_quantity) ordered_quantity
    ...
    l_is_ord_amt_tolerance VARCHAR2(1); --#CHG0044825
    ...
    */
    ...
    IF l_validation_error IS NULL THEN
      ...
      BEGIN
        ...
        --Added on 30JAN19 #CHG0044825
        l_errbuf := '';
        is_diff_amount_in_tolerance(p_order_header_id => p_header_id,
                                    p_order_amount    => nvl(l_tot_oh_amt, 0),
                                    p_quote_amount    => nvl(l_tot_qh_amt, 0),
                                    x_error           => l_errbuf,
                                    x_in_tolerance    => l_is_ord_amt_tolerance);
        ...
        */ --Commented #CHG0044825
      
        --Added #CHG0044825
        IF l_is_ord_amt_tolerance = 'N' THEN
          l_validation_error := l_validation_error || chr(10) || l_errbuf;
        END IF;
      EXCEPTION
        WHEN OTHERS THEN
    ...
    IF l_validation_error IS NULL THEN
      ...
      FOR rec IN c_qh_record LOOP
        ...
        l_header_rec.sf_3rd_party_ship_to        := rec.sf_3rd_party_ship_to; --#CHG0044636
        l_header_rec.sf_3rd_party_ship_to_acc_id := rec.sf_3rd_party_ship_to_acc_id; --#CHG0044636
