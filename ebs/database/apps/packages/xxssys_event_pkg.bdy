create or replace package body xxssys_event_pkg AS
  ...
  BEGIN
    ...
    ELSE
      ...
      SET    event_message    = p_xxssys_event_rec.event_message,
             ...
             err_message = nvl(p_xxssys_event_rec.err_message, err_message),
  ...
  BEGIN
    ...
    IF l_event_id = 0 THEN  -- CHG0042626 - No existing events so insert NEW event
      INSERT INTO xxssys_events
        (event_id,
         target_name,
         entity_name,
  ...
  WHERE  rn > 1);
    ...
    For rec in (select event_id,concatenated_key_cols
                 from xxssys_events xe
                 where xe.status  = 'ERR'
  ...
  BEGIN
    ...
    fnd_file.put_line(fnd_file.log,
	          'Purge All program completed successfully. Total records purged for entity: ' ||p_entity_name||' for target: '||p_target_name||' :: '||
	          l_records_updated);
  ...
  PROCEDURE retry(p_target_name      IN VARCHAR2,
    ...
    CURSOR cur_retry(p_entity_name      IN VARCHAR2,
      ...
      AND    nvl(a.bpel_instance_id, -1) =
   nvl(p_bpel_instance_id, nvl(a.bpel_instance_id, -1))
      AND    a.status = 'ERR';
  ...
  BEGIN
    purge(x_errbuf       => l_purge_errbuf,
          x_retcode      => l_purge_retcode,
          p_event_status => 'ERR',
          p_event_days   => NULL,
          p_target_name  => p_target_name);

    IF l_purge_retcode = 0 THEN
      /*      UPDATE XXSSYS_EVENTS  a
        SET a.status='NEW',
            a.REQUEST_MESSGAE=NULL,
            a.ERR_MESSAGE =NULL,
    ...
    ELSE
      x_status          := 'ERROR';
      x_records_updated := 0;
      x_status_message  := 'ERROR: While purging ERR status events';
    END IF;
  EXCEPTION
    WHEN OTHERS THEN
      ROLLBACK;
    ...
    -- CHG0042626 if event message is null only then process data as per message generation function
      ...
      IF l_varchar_pos IS NOT NULL THEN
        FOR i IN 1 .. l_varchar_pos.count LOOP
          ...
          l_var_type_tmp := REPLACE(l_varchar_pos(i), l_pos_tmp || ':', NULL);
  ...
  begin
    ...
    l_mail_to_list := xxobjt_general_utils_pkg.get_dist_mail_list(p_operating_unit     => p_operating_unit,
					      p_program_short_name => p_program_name_to);

    l_mail_cc_list := xxobjt_general_utils_pkg.get_dist_mail_list(p_operating_unit     => p_operating_unit,
					      p_program_short_name => p_program_name_cc);

    l_message_data := '<NOTIFICATION_MAIL_DATA>'||
                      '<SUBJECT>'||p_subject||'</SUBJECT>'||
                      '<MAIL_TO>'||l_mail_to_list||'</MAIL_TO>'||
                      '<MAIL_CC>'||l_mail_cc_list||'</MAIL_CC>'||
                      '<BODY>'||p_body||'</BODY>'||
                      '</NOTIFICATION_MAIL_DATA>';
  ...
  PROCEDURE send_mail_from_events(errbuf  OUT VARCHAR2,
    ...
    cursor cur_mail_events is
      select xmltype(xe.event_message).extract('//SUBJECT/text()').getStringVal() subject,
         xmltype(xe.event_message).extract('//MAIL_TO/text()').getStringVal() to_mail,
         xmltype(xe.event_message).extract('//MAIL_CC/text()').getStringVal() cc_mail,
         listagg('<TR><TD>'||xe.attribute1||'</TD><TD style="white-space:pre-wrap; word-wrap:break-word">'||xmltype(xe.event_message).extract('//BODY/text()').getStringVal()||'</TD></TR>','') within group (order by event_id) bodytxt
    from xxssys_events xe
   where entity_name='NOTIF_EMAIL'
     and status='IN_PROCESS'
     and xe.target_name = p_target_name
     group by xmltype(xe.event_message).extract('//SUBJECT/text()').getStringVal(),
              xmltype(xe.event_message).extract('//MAIL_TO/text()').getStringVal(),
              xmltype(xe.event_message).extract('//MAIL_CC/text()').getStringVal()
     order by 1 desc;
  ...
  BEGIN
    ...
    for rec in cur_mail_events loop
      l_body := xxobjt_wf_mail_support.get_header_html('INTERNAL')||
                ...
                rec.bodytxt||'</TABLE>'||xxobjt_wf_mail_support.get_footer_html;
      ...
      if l_err_code <> 0 then
        update xxssys_events xe
         ...
         where entity_name='NOTIF_EMAIL'
           and status='IN_PROCESS'
           and target_name = p_target_name
           and xmltype(xe.event_message).extract('//SUBJECT/text()').getStringVal() = rec.subject
           and xmltype(xe.event_message).extract('//MAIL_TO/text()').getStringVal() = rec.to_mail
           and xmltype(xe.event_message).extract('//MAIL_CC/text()').getStringVal() = rec.cc_mail;
      else
        update xxssys_events xe
