CREATE OR REPLACE PACKAGE BODY xx_bpel_fnd_request_wrapper2 IS
  --------------------------------------------------------------------
  ...
  --------------------------------------------------------------------
  --  purpose :          CHG0032349 - Creation of Host to Host connectivity between JPMC 
  --                      do not add any extra code !!!!!!!!
  --modification history
  --------------------------------------------------------------------
  --  ver   date               name         desc
  --  1.0   2.11.14            yuval tal    CHG0032349 - used in Host to Host load bank startement  JPMC 

  FUNCTION fnd_request$submit_request(application VARCHAR2,
			  program     VARCHAR2,
    ...
    RETURN NUMBER IS
    return_      NUMBER;
    sub_request_ BOOLEAN;
  BEGIN
    sub_request_ := sys.sqljutl.int2bool(sub_request);
    return_      := apps.fnd_request.submit_request(application,
				    program,
				    description,
				    start_time,
				    sub_request_,
				    ...
				    argument100);
    RETURN return_;
  END fnd_request$submit_request;

END xx_bpel_fnd_request_wrapper2;
/
