...
-- =========================================================================================
 IS
  ...
  g_err_email_body_H  VARCHAR2(1000):=
  '<HTML>
	<p>Hello,</p>
  ...
  PROCEDURE log_message(p_msg VARCHAR2)
   ...
   Exception
     When Others Then
       ...
       ELSE
            fnd_file.put_line(fnd_file.log, 'There is an Error in Printing log Messages.');
       END IF;
   END log_message;
  --------------------------------------------------------------------
  --  customization code: CHG0039163
  ...
  PROCEDURE wait_for_request_completion(p_request_id IN NUMBER,
                                        ...
                                        p_max_wait   IN  NUMBER DEFAULT 600
                                        )
  IS
    l_complete		 BOOLEAN;
  ...
  BEGIN
      ...
      END IF;
        log_message('          Request Id :'|| p_request_id ||' Completed with Status:'||p_dev_phase);
        log_message('       Program Exited  - xxcust_document_submitter_pkg.wait_for_request_completion Procedure');
  Exception
  When Others Then
   log_message('       Program Exited  - xxcust_document_submitter_pkg.wait_for_request_completion Procedure with Error :'||sqlerrm);
  ...
  l_err_email_body VARCHAR2(4000) := (g_err_email_body_H || g_err_email_body);
  l_err_code    NUMBER;
  l_err_message VARCHAR2(2000);
  l_subject     VARCHAR2(100) :=  'Error In Auto Submit Document Set :'|| p_set_code ||' ,For Key: '||p_key;
  BEGIN
    log_message('');
  ...
  Exception
  When Others Then
      log_message('       Program Exited  - xxcust_document_submitter_pkg.Send_error_log_to_Admin  Procudure with Error :'||SQLERRM);
  ...
  --  1.0   19-Feb-2017   Lingaraj(TCS)       Initial Build: CHG0039163
  ----------------------------------------------------------------------
  Function is_valid_user(p_user_name VARCHAR2) Return BOOLEAN
  IS
    l_user_id NUMBER;
  BEGIN
  ...
  BEGIN
      ...
      Begin
       Select ORGANIZATION into l_ou_name from XXHR_OPERATING_UNITS_V where org_id = p_org_id;
       select completion_text into l_completion_text  from fnd_concurrent_requests where request_id = p_request_id ;
      Exception
        When Others Then
        Null;
      End;
      ...
      g_err_email_body :=
            '<TR>
                        <TD>'||p_request_id||'</TD>
                        <TD>'||p_program_short_name||'</TD>
                        <TD>'||p_user_program_name||'</TD>
                        <TD>'||p_key||'</TD>
                        <TD>'||l_ou_name||'</TD>
                        <TD>'||p_org_id||'</TD>
                        <TD>'||p_set_code||'</TD>
                        <TD>'||p_appl_short_name||'</TD>
                        <TD>'||p_template_code||'</TD>
                        <TD>'||p_export_folder||'</TD>
                        <TD>'||p_export_file_name||'</TD>
                        <TD>'||p_output_type||'</TD>
                    </TR>
                </TABLE>
                <p>Please Review the Concurrent Log of Request Id :'||g_request_id||' for detail information.</p>'||
                '<P style="color:red;font-weight:bold;font-size:20">'||
                (CASE WHEN p_request_id IS NOT NULL THEN 'Program Completed with Completion Text :'||l_completion_text  ELSE '' END)
                 ||'</p>'
                ||'<p style="color:red;font-weight:bold;font-size:20">'
                || nvl(p_exp_error,'') ||'</p>'
                ||'<p>Good day,</p>
    ...
    CURSOR cur_rpt_submtr IS
			SELECT xcrs.set_id                  set_id,
                   ...
                   (select USER_CONCURRENT_PROGRAM_NAME from
                        ...
                        CONCURRENT_PROGRAM_NAME = xcrs.concurrent_program_name and rownum = 1 ) user_concurrent_program_name,
			 ...
			 WHERE   xcrs.set_code = p_set_code
			   AND   xcrs.org_id = nvl(p_org_id, xcrs.org_id)
    ...
    CURSOR cur_rpt_submtr_param(c_set_id NUMBER) IS
			SELECT xcrsp.set_id              set_id,
			 ...
			 WHERE  xcrsp.set_id = c_set_id
			   ...
			   AND  t.descriptive_flexfield_name = '$SRS$.' || xcrs.concurrent_program_name
  ...
  BEGIN
    ...
    x_retcode := 0;
    log_message('');
    log_message('  For Loop Started to run the Program for the Set Code :'||p_set_code);
    FOR i IN cur_rpt_submtr LOOP
      BEGIN
        log_message(' *****************************************************************************************');
        log_message('  Data Processing Started for Concurrent Program :'||i.user_concurrent_program_name);
      ...
      --1.1
        ...
        l_admin_user:= xxcust_document_submitter_pkg.get_dyn_sql_value(i.admin_user,p_key);
      ...
      log_message('  ');
      log_message('  Paremeter Values Set for Concurrent Program :'||i.user_concurrent_program_name);
      ...
      FOR j IN cur_rpt_submtr_param(i.set_id) LOOP
        ...
        IF j.default_value IS NOT NULL THEN
          l_value1 := NULL;
          l_value1 := xxcust_document_submitter_pkg.get_dyn_sql_value(j.default_value,p_key);
      ...
      END LOOP;
      log_message('  Paremeter Values Set for Concurrent Program :'||i.user_concurrent_program_name ||' Completed sucessfully.');
      log_message('');
      --
      ...
      IF i.template_type_code IS NOT NULL THEN
        ...
        log_message('   Template Code / Template Language  / Template Territory / Output Type:'||i.template_code || '/'|| i.default_language ||'/'||i.default_territory ||'/'||i.output_type);
      END IF;
      ...
      IF l_request > 0 THEN
		---- Wait for Request completion  
                    ...
                    wait_for_request_completion(p_request_id => l_request ,
                                                ...
                                                p_message    => l_message,
                                                p_max_wait   => nvl(fnd_profile.VALUE('XXCUST_DOCSUBMITTER_WAIT_SEC'),600) 
                                                );
					--COMMIT;
					...
					IF UPPER(l_dev_phase) = 'COMPLETE' AND
                       upper(l_dev_status) = 'NORMAL' THEN
						log_message('  The Program SUCCESSFULLY COMPLETED for Request ID : '||l_request);
						...
						BEGIN
							SELECT request_id
							 ...
							 WHERE has_sub_request = 'N'
							   ...
							   AND rownum = 1 ;
						    log_message('  Child Request found for request id '|| l_request);
						...
						EXCEPTION
							WHEN no_data_found THEN
                                ...
                                log_message(' No Child Request found for Parent Request Id:'||l_request);
						END;
						log_message(' ');
						IF 	i.export_folder	IS NOT NULL AND l_file_name IS NOT NULL THEN
                            -- Selection of output file name from dynamic function.

                            log_message('  File Transfer Process Started for Request Id :'||l_req_child);
                            --l_file_name := xxcust_document_submitter_pkg.get_dyn_sql_value(i.export_file_name,p_key);
                            --log_message(' Export File Name                  : '||l_file_name);
                           ...
                           IF l_request > 0 THEN
                             ...
                             l_dev_status := NULL;
                             l_message    := NULL;
                             wait_for_request_completion(l_file_trf_request , l_dev_phase,l_dev_status,l_message );
                             ...
                             IF --UPPER(l_dev_phase) != 'COMPLETE' OR
                                ...
                                log_message('  Dev_Phase/ Dev_Status / Message :'||l_dev_phase||'/'||l_dev_status||'/'||l_message);
                                build_err_email_body(p_request_id 	   => l_file_trf_request,
                                 ...
                                 END IF;
                             ELSE
                                 log_message('  File Transfer Process Completed Sucessfully for Request Id :'||l_req_child);
                             END IF;
                           ELSE
                       ...
                       ELSE
                          log_message('  File Transfer Process SKIPPED because of either Export folder Path or File Name missing for Request Id :'||l_req_child);
                       END IF;
                    ELSE
	 ...
	 EXCEPTION
       WHEN MAIN_CP_FAILED_TO_SUBMIT THEN
           l_cp_exception := 'Main Concurrent Program '||i.user_concurrent_program_name ||' Submission Failed.';
       WHEN FILE_COPY_CP_FAILED_TO_SUBMIT  THEN
           l_cp_exception := 'File Copy Concurrent Program '||i.user_concurrent_program_name ||' Submission Failed.';
       WHEN OTHERS THEN
 ...
 EXCEPTION
    WHEN OTHERS THEN
      log_message('Unexpected error occured in program, process abort. ' ||SQLERRM);
      x_errbuf  := sqlerrm;
      x_retcode := 2;
  ...
  BEGIN
    ...
    log_message('        Program Exited : xxcust_document_submitter_pkg.get_dyn_sql_value Procedure With value :'||l_return_val);
    Return l_return_val;
  /*Exception
  When Others Then
