CREATE OR REPLACE PACKAGE BODY XXQP_PL_FORMULA_SYNC_PKG AS
  ...
  --  1.1  08/12/2019  Roman W.                     INC0177120
  ----------------------------------------------------------------------------
  ...
  BEGIN
    ...
    l_xxssys_event_rec.entity_code     := p_list_header_id || '|' ||
                                          p_item_code || '|' ||
                                          p_currency_code;
    ...
    IF l_status = 'SP01' THEN
      ...
      IF i IS NOT NULL THEN
        LOOP
          ...
          IF l_line_details(i).unit_sales_price IS NOT NULL AND l_line_details(i)
             .line_num = 1 THEN
  ...
  BEGIN
    fnd_file.PUT_LINE(fnd_file.OUTPUT,
                      'INPUT: Price List ID: ' || p_list_header_id);
    fnd_file.PUT_LINE(fnd_file.OUTPUT,
                      'INPUT: Target Name: ' || p_target_name);
    ...
    begin
      ...
      select fcp.CONCURRENT_PROGRAM_NAME || '(CONC_REQ_ID:' ||
    ...
    exception
      when others then
        l_exception_message := 'UNEXPECTED ERROR: While deriving default values for event generation. ' ||
                               sqlerrm;
        raise e_exception;
    end;
    ...
    begin
      ...
      select xxinv_product_rec_type(null,
        ...
        bulk collect
        into l_pl_item_list
        from (select to_char(msib.inventory_item_id) inventory_item_id,
               ...
               where msib.organization_id =
                 ...
                 and trunc(sysdate) between
                     nvl(msib.start_date_active, sysdate - 1) and
                     nvl(msib.end_date_active, sysdate + 1)
                 and msib.primary_uom_code in
              ...
              union
              select qll.product_id            inventory_item_id,
                     qll.product_attr_val_disp item_code,
                     qll.product_uom_code      uom_code
                from qp_list_lines_v qll
               where qll.list_header_id = p_list_header_id
                 ...
                 and trunc(sysdate) between
                     nvl(qll.start_date_active, sysdate - 1) and
                     nvl(qll.end_date_active, sysdate + 1)) aa
      /*where rownum < 1000*/
      ;
    
    exception
      when others then
        l_exception_message := 'UNEXPECTED ERROR: While generating item list for pricing. ' ||
                               sqlerrm;
        raise e_exception;
    end;
  
    fnd_file.PUT_LINE(fnd_file.OUTPUT,
                      'Items count considered for price generation: ' ||
                      l_pl_item_list.count);
    ...
    for i in 1 .. l_pl_item_list.count loop
      begin
        ...
        call_price_api(p_item_id       => l_pl_item_list(i)
                                          .inventory_item_id,
        ...
        if l_price_status = 'E' then
          l_exception_message := 'ERROR: Exiting program. Exception while calculating price for item ID: ' || l_pl_item_list(i)
                                .inventory_item_id || ' and pricelist ' ||
                                 p_list_header_id || '. ' ||
                                 l_price_message;
        ...
        begin
          select xe.event_id, xe.active_flag, xe.attribute7
            ...
            from (SELECT xe1.event_id,
                         xe1.active_flag,
                         xe1.attribute7,
                         rank() over(PARTITION BY xe1.entity_id, xe1.attribute1, xe1.attribute5, xe1.attribute8, xe1.entity_name, xe1.target_name ORDER BY xe1.event_id DESC) rn
                   ...
                   WHERE xe1.entity_id = p_list_header_id
                     ...
                     AND xe1.attribute1 = l_pl_item_list(i)
                        .inventory_item_id
        ...
        elsif l_last_active_flag = 'Y' then
          -- Last interfaced event with Active flag
          /*if l_list_price = 0 then -- List price calculated as 0 so insert event as inactive
            l_active_flag := 'N';
          else  -- List price is non-0*/
          if l_last_price = l_list_price then
            -- New calculated price is same as last successfully interfaced price no event needed
            continue;
          else
            -- new price is different than last interfaced price so insert event as active
            l_active_flag := 'Y';
          end if;
          --end if;
        end if;
        ...
        insert_formula_pl_line_event(p_target_name       => p_target_name,
                                     ...
                                     p_inventory_item_id => l_pl_item_list(i)
                                                            .inventory_item_id,
                                     p_list_header_id    => p_list_header_id,
                                     p_list_price        => l_list_price,
                                     p_item_code         => l_pl_item_list(i)
                                                            .segment1,
                                     p_item_uom_code     => l_pl_item_list(i)
      ...
      exception
        when others then
          l_exception_message := 'UNEXPECTED ERROR: While generating event for item ID: ' || l_pl_item_list(i)
                                .inventory_item_id || chr(13) || chr(10) || ' ' ||
                                 sqlerrm;
          l_is_error          := 'Y';
    ...
    for rec_exist_items in (select xe.attribute1 inventory_item_id,
                                   xe.attribute4 segment1,
                                   xe.attribute8 uom_code
                              from (SELECT xe1.attribute1,
                                           xe1.attribute4,
                                           xe1.attribute8,
                             ...
                             where xe.rn = 1
                            minus
                            SELECT to_char(t1.inventory_item_id),
                                   t1.segment1,
                                   t1.PRIMARY_UNIT_OF_MEASURE uom_code
                              FROM TABLE(CAST(l_pl_item_list AS
                                              XXINV_PRODUCT_TAB_TYPE)) t1) loop
    
      begin
        fnd_file.PUT_LINE(fnd_file.OUTPUT,
                          'Existing item to be removed from pricelist: Item ID: ' ||
                          rec_exist_items.inventory_item_id);
      ...
      exception
        when others then
          l_exception_message := 'UNEXPECTED ERROR: While generating event for removal of item ID: ' ||
                                 rec_exist_items.inventory_item_id ||
