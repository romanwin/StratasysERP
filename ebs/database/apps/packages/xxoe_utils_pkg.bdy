create or replace package body xxoe_utils_pkg IS
  ...
  -- 1.71 10.01.2019   Diptasurjya     CHG0044828 - Add new function get_order_approver_info
  -- 1.7  23/12/2018   Roman W         CHG0044705 - Commercial Invoice - Bug Fix
  -- 1.8  03/07/2019   Offer S.        CHG0045219 - AYCP - need to exclude all project related orders from avarage discount calculation
  --                                         code corection in : get_order_average_discount
  -- 1.9  2019-08-13   Roman W.        CHG0045507 - For Order with discount at line level, the field "Modification Reason" needs to be set as mandatory
  -- 2.0  08/21/2019   Diptasurjya     CHG0045128 - Add function generate_ack_dist_list to Build email distribution list based on input parameters
  -- 2.1  10/24/2019   Diptasurjya     CHG0046640 - Add new funtion modifier_elibility_type for qualifier attribute
  -- 2.2  24/03/2020   Roman W.        CHG0047653 - Commercial Invoice - Change logic for Zero price for SP items
  -- 2.3  04/21/2020   Diptasurjya     CHG0047838 - Remove line level hold from hold string
  -- 2.4  19/05/2020   Roman W.        CHG0047959 - Commercial Invoice - Fix SP 50% price on zero price function
  -- 2.5  09-JUN-2020  Diptasurjya     CHG0046575 - change function generate_ack_dist_list to match changed valuset structures
  -- 1.5  04/02/2021   Roman W.        CHG0049362 - ECO country excluding from notifications
  --                                                      added function "get_ship_to_country_code"
  -- 2.7  01-JUN-2021  Diptasurjya      INC0233284 - Add new function get_order_totals
  ------------------------------------------------------------------------------------------------------------------------------
  ...
  -- ----  ----------  ----------  ---------------------------------------------------------------------------------------------------
  -- 1.0   03/07/2019  Ofer S.     CHG0045219 - AYCP - need to exclude all project related orders from avarage discount calculation
  ------------------------------------------------------------------------------------------------------------------------------------
      ...
      FROM (SELECT t.attribute4,
             ...
             WHERE t.header_id = p_header_id
               ...
               AND msi.organization_id =
                   xxinv_utils_pkg.get_master_organization_id
               and xxar_autoinvoice_pkg.exclude_from_avg_discount(t.line_id) = 'N'
  ...
  --  1.2  05/02/2019  Roman W          CHG0044705 - Bug Fix
  --------------------------------------------------------------------
    ...
    BEGIN
      ...
      SELECT 'Y'
        ...
        FROM oe_order_lines_all         oola,
             ServiceContract@source_sf2 sc, -- 1.2
             Asset@source_sf2           ast -- 1.2
       WHERE oola.unit_selling_price = 0
         AND oola.attribute1 = to_char(ast.External_Key__c)
         AND xxinv_utils_pkg.get_category_segment('SEGMENT1',
                                                  1100000221,
                                                  oola.INVENTORY_ITEM_ID) =
             'Customer Support' -- FROM Product Hierarchy
         AND oola.line_id = p_line_id
         AND sc.Asset__c = ast.id
         AND trunc(oola.pricing_date) BETWEEN sc.startdate AND sc.enddate
         AND ROWNUM = 1;
  ...
  /* Rem by Roman 05/02/2019 CHG0044705 - Bug Fix
  FUNCTION is_line_order_under_contract(p_line_id IN NUMBER) RETURN VARCHAR2 IS
    l_is_machine_under_contract VARCHAR2(5) := 'N';
    l_service_application       VARCHAR2(15) := nvl(fnd_profile.value('XXINV_TPL_SERVICE_APPLICATION'),
                                                    'Oracle');
  ...
  */
  ...
  --  1.1   04/21/2020    Diptasurjya          CHG0047838 - Exclude Line level holds from appearing on this note
  --------------------------------------------------------------------------------------------------
  function get_order_approver_info(p_header_id number) return varchar2 is
    pragma autonomous_transaction;
    l_approver_info varchar2(2000) := '';

    l_dp_hold_string     varchar2(500) := '';
    l_auto_hold_string   varchar2(500) := '';
    l_credit_hold_string varchar2(500) := '';
    ...
    l_orig_sys_ref    varchar2(200);
    l_order_source_id number;
    l_org_id          number;
  begin
    select oh1.orig_sys_document_ref, oh1.order_source_id, oh1.org_id
      into l_orig_sys_ref, l_order_source_id, l_org_id
      from oe_order_headers_all oh1
     where oh1.header_id = p_header_id;

    OE_MSG_PUB.Initialize;

    /* Delete existing hold messages for this order */
    oe_msg_pub.DELETE_OI_MESSAGE(p_request_id                 => -1,
                                 p_change_sequence            => null,
                                 p_orig_sys_document_line_ref => null,
                                 p_orig_sys_shipment_ref      => null,
                                 p_orig_sys_document_ref      => l_orig_sys_ref,
                                 p_org_id                     => l_org_id,
                                 p_entity_code                => 'HEADER',
                                 p_entity_ref                 => null,
                                 p_order_source_id            => l_order_source_id);

    OE_MSG_PUB.set_msg_context(p_entity_code                => 'HEADER',
                               p_entity_ref                 => null,
                               p_entity_id                  => NULL,
    ...
    begin
      SELECT listagg('HOLD: ' || ohd.name || ': ' || chr(10) ||
                     'Approver: ' || t.recipient_role || ' (Email: ' ||
                     xxhr_util_pkg.get_person_email(fu.employee_id) || ')' ||
                     chr(10) || 'Approval Status: ' || t.status || chr(10) ||
                     'Notified On: ' || t.begin_date || chr(10) || chr(10),
                     '') within group(order by t.notification_id desc)
        INTO l_dp_hold_string
       ...
       WHERE ta.item_type = t.message_type
         ...
         and ooh.header_id = ta.number_value
         and ohs.hold_source_id = ooh.hold_source_id
         and ohs.hold_id = ohd.hold_id
         and ooh.released_flag = 'N'
         and ooh.hold_release_id is null
         and ohd.name = 'Denied Parties Hold'
         and ooh.header_id = oh.header_id
         and t.recipient_role = fu.user_name
         and oh.header_id = p_header_id;

      l_dp_hold_string := trim(rtrim(l_dp_hold_string, chr(10)));

      if l_dp_hold_string is not null then
        OE_MSG_PUB.Add_text(l_dp_hold_string, 'NOTE');
      end if;
    exception
      when no_data_found then
        l_dp_hold_string := '';
    ...
    begin
      SELECT listagg('HOLD: ' || ohd.name || ': ' || chr(10) ||
       ...
       WHERE ah.doc_code = doc.doc_code
         ...
         AND t.message_type = 'XXWFDOC'
            --AND t.status = 'OPEN'
         AND t.message_name = 'NEED_APPR_MSG'
         AND oh.header_id = doc_ins.n_attribute2
         AND doc_ins.n_attribute1 = ooh.order_hold_id
         AND ooh.header_id = oh.header_id
         AND ooh.hold_source_id = ohs.hold_source_id
         AND ooh.released_flag = 'N'
         AND ooh.hold_release_id is null
         AND ohs.hold_id = ohd.hold_id
         and fu.user_name = t.recipient_role
         AND oh.header_id = p_header_id;

      l_credit_hold_string := trim(rtrim(l_credit_hold_string, chr(10)));

      if l_credit_hold_string is not null then
        OE_MSG_PUB.Add_text(l_credit_hold_string, 'NOTE');
      end if;
    exception
      when no_data_found then
        l_credit_hold_string := '';
    end;

    for om_hold_rec in (SELECT t.begin_date,
                               t.recipient_role,
                               ta1.text_value,
                               xxhr_util_pkg.get_person_email(fu.employee_id) email,
                               t.status
                          FROM wf_notifications         t,
                               wf_item_attribute_values ta,
                         ...
                         WHERE t.message_type = ta.item_type
                           AND t.item_key = ta.item_key
                           AND t.message_type = 'XXOMHLD'
                           AND t.message_name = 'AGG_APPROVER_NOTE'
                              --AND t.status = 'OPEN'
                           AND ta.name = 'SO_HEADER_ID'
                           AND ta1.item_type = t.message_type
                           AND ta1.item_key = t.item_key
                           AND ta1.name = 'AGG_AUTO_HOLD_IDS_LIST'
                           and ta.number_value = p_header_id
                           and fu.user_name = t.recipient_role
                         order by t.notification_id desc) loop
      for rec_hold_id in split_c(om_hold_rec.text_value) loop
        begin
          select 'HOLD: ' || ohd.name || ': ' || chr(10) || 'Approver: ' ||
                 om_hold_rec.recipient_role || ' (Email: ' ||
                 om_hold_rec.email || ')' || chr(10) || 'Approval Status: ' ||
                 om_hold_rec.status || chr(10) || 'Notified On: ' ||
                 om_hold_rec.begin_date || chr(10) || chr(10)
            INTO l_auto_hold_string
            from xxom_auto_hold      xah,
                 oe_hold_definitions ohd,
                 oe_hold_sources_all ohs,
                 oe_order_holds_all  ooh
           where xah.auto_hold_id = rec_hold_id.split_auto_hold_id
             and xah.hold_id = ohd.hold_id
             and ohd.hold_id = ohs.hold_id
             and ohs.hold_source_id = ooh.hold_source_id
             and ooh.header_id = p_header_id
             and ooh.released_flag = 'N'
             and ooh.line_id is null -- CHG0047838 add to exclude line level holds
             and ooh.hold_release_id is null;

          if rtrim(l_auto_hold_string, chr(10)) is not null then
            OE_MSG_PUB.Add_text(rtrim(l_auto_hold_string, chr(10)), 'NOTE');
          end if;

          l_approver_info := l_approver_info || l_auto_hold_string;
        exception
          when no_data_found then
            null;
        end;
    ...
    l_approver_info := trim(rtrim(l_approver_info, chr(10)));

    /* Save all hold messages with request ID -1, to identify these records for susequent calls */
    OE_MSG_PUB.save_messages(-1);
    commit;

    l_approver_info := ltrim(rtrim(l_credit_hold_string || chr(10) ||
                                   chr(10) || l_dp_hold_string || chr(10) ||
                                   chr(10) || l_approver_info,
                                   chr(10)),
                             chr(10));

    if l_approver_info is null then
      l_approver_info := 'No pending hold approval notifications for this order!' ||
                         chr(10) || chr(10) ||
                         'There might still be manual holds waiting to be released, please check from Actions --> Additional Order Information';
    end if;

    return l_approver_info;
  end get_order_approver_info;

  function get_signed_order_line_quant(p_line_type     varchar2,
                                       p_line_quantity number) return number is
    l_signed_quant number := 0;
  begin
    --insertlog('Enter function: '||p_line_type||' '||p_line_quantity);
    select decode(p_line_type,
                  'RETURN',
                  -1 * p_line_quantity,
                  p_line_quantity)
      into l_signed_quant
      from dual;
    --insertlog('Exit function: '||l_signed_quant);
    return l_signed_quant;
  end get_signed_order_line_quant;
  ...
  -----------------------------------------------------------------------------------------
  function get_modif_reason_msg(p_header_id number) return varchar2 is
    ------------------------------
    --    Local Definition
    ------------------------------
  ...
  begin
     ...
     where HEADER_ID = p_header_id
       ...
       and ottt.Name in
           (select ffv.FLEX_VALUE
             ...
             where ffvs.FLEX_VALUE_SET_NAME =
                   'XXOE_ORDER_TYPES_WITH_DISCOUNT'
               and ffv.FLEX_VALUE_SET_ID = ffvs.FLEX_VALUE_SET_ID)
       and ooha.org_id = hou.ORGANIZATION_ID
       and hou.name in
           (select ffv.FLEX_VALUE
              from fnd_flex_values ffv, fnd_flex_value_sets ffvs
             where ffvs.FLEX_VALUE_SET_NAME = 'XXOE_OU_WITH_DISCOUNT'
               and ffv.FLEX_VALUE_SET_ID = ffvs.FLEX_VALUE_SET_ID);

    If l_flag > 0 THEN
      for rec in (select (oola.line_number || '.' || oola.shipment_number) line_number
                    from oe_order_lines_all     oola,
                         OE_PRICE_ADJUSTMENTS_V opav
                   where oola.line_id = opav.LINE_ID
                     and oola.attribute6 is null
                     and opav.ADJUSTMENT_NAME = 'Manual Adjustment'
                     and opav.HEADER_ID = p_header_id) Loop

        l_rec_cnt := l_rec_cnt + 1;
        l_message := l_message || chr(10) || '(' || l_rec_cnt || ') ' ||
                     rec.line_number;
    ...
    If l_rec_cnt > 0 Then
      /*  l_message := 'Please fill up Modification Reason Line Level for Below Lines :' ||
      chr(10) || l_message;*/
      fnd_message.set_name('XXOBJT', 'XXOE_ERR_MSSG_WITH_DISCOUNT');
      fnd_message.set_token('LINE_NUMS', l_message);
  ...
  exception
    when others then
      l_message := 'Unexpected Error in GET_ERROR_MESSAGE,' || sqlerrm;
      return l_message;

  end get_modif_reason_msg;
  ...
  --  1.1   09-JUN-2020   Diptasurjya          CHG0046575 - Change based on new valueset structure
  --------------------------------------------------------------------------------------------------
  FUNCTION generate_ack_dist_list(p_order_type_id      NUMBER,
    ...
    l_distribution_list VARCHAR2(240) := NULL;
    l_mail_list_raw     varchar2(240);

    l_ship_contact_email varchar2(240) := null;
    l_bill_contact_email varchar2(240) := null;
    l_cust_contact_email varchar2(240) := null;

    l_delete varchar2(1) := 'N';

    l_bill_mail_suffix varchar2(10) := '(BILL)';
    l_ship_mail_suffix varchar2(10) := '(SHIP)';
    l_cust_mail_suffix varchar2(10) := '(CUST)';
  BEGIN
    begin
      select v.DESCRIPTION
        into l_mail_list_raw
        from FND_FLEX_VALUES_VL v, fnd_flex_value_sets ffvs
       where v.FLEX_VALUE_SET_ID = ffvs.flex_value_set_id
         and ffvs.flex_value_set_name = p_distribution_type
            --and v.FLEX_VALUE = to_char(p_order_type_id)  -- CHG0046575 comment
         and v.attribute1 = to_char(p_order_type_id) -- CHG0046575 add
         and v.ENABLED_FLAG = 'Y'
         and sysdate between nvl(v.START_DATE_ACTIVE, sysdate - 1) and
             nvl(v.END_DATE_ACTIVE, sysdate + 1);

      if l_mail_list_raw is null then
        return null;
      end if;
    exception
      when no_data_found then
    ...
    l_distribution_list := nvl(p_existing_dist_list, l_mail_list_raw);

    if p_ship_contact_id is not null then
      if instr(l_mail_list_raw, 'SHIP_TO') <> 0 then
        begin
          SELECT hcp.email_address
            INTO l_ship_contact_email
           ...
           WHERE hcp.owner_table_name = 'HZ_PARTIES'
             ...
             AND hcar.cust_account_role_id = p_ship_contact_id;
        exception
          when no_data_found then
            l_ship_contact_email := null;
        end;

        if l_ship_contact_email is not null then
          if instr(l_distribution_list, 'SHIP_TO') <> 0 then
            for rec in (select regexp_substr(l_distribution_list,
                                             '[^|]+',
                                             1,
                                             level) emails
                          from dual
                        connect by regexp_substr(l_distribution_list,
                                                 '[^|]+',
                                                 1,
                                                 level) is not null) loop
              if rec.emails like '%' || l_ship_mail_suffix || '%' then
                l_distribution_list := replace(l_distribution_list,
                                               rec.emails,
                                               '');
                exit;
              end if;
            ...
            if instr(l_distribution_list,
                     l_ship_contact_email || l_ship_mail_suffix) = 0 then
              l_distribution_list := replace(l_distribution_list,
                                             'SHIP_TO',
                                             l_ship_contact_email ||
                                             l_ship_mail_suffix);
            else
              l_distribution_list := replace(l_distribution_list,
                                             'SHIP_TO',
            ...
            if instr(l_distribution_list,
                     l_ship_contact_email || l_ship_mail_suffix) = 0 then
              l_distribution_list := l_ship_contact_email ||
                                     l_ship_mail_suffix || '|' ||
                                     l_distribution_list;
            end if;
          end if;
          ...
          l_delete            := 'Y';
          l_distribution_list := replace(l_distribution_list, 'SHIP_TO', '');
        end if;
      else
        l_delete := 'Y';
      end if;
    ...
    l_delete := 'N';
    if p_bill_contact_id is not null then
      if instr(l_mail_list_raw, 'BILL_TO') <> 0 then
        begin
          SELECT hcp.email_address
            INTO l_bill_contact_email
            FROM hz_cust_account_roles hcar, hz_contact_points hcp
           WHERE hcp.owner_table_name = 'HZ_PARTIES'
             AND hcp.contact_point_type = 'EMAIL'
             AND hcp.email_address IS NOT NULL
             ...
             AND hcp.primary_flag = 'Y'
             AND hcar.party_id = hcp.owner_table_id
             AND hcar.cust_account_role_id = p_bill_contact_id;
        exception
          when no_data_found then
            l_bill_contact_email := null;
        end;

        if l_bill_contact_email is not null then
          if instr(l_distribution_list, 'BILL_TO') <> 0 then
            for rec in (select regexp_substr(l_distribution_list,
                                             '[^|]+',
                                             1,
                                             level) emails
                        ...
                        connect by regexp_substr(l_distribution_list,
                                                 ...
                                                 1,
                                                 level) is not null) loop
              if rec.emails like '%' || l_bill_mail_suffix || '%' then
                l_distribution_list := replace(l_distribution_list,
                                               rec.emails,
                                               '');
                exit;
            ...
            if instr(l_distribution_list,
                     l_bill_contact_email || l_bill_mail_suffix) = 0 then
              l_distribution_list := replace(l_distribution_list,
                                             'BILL_TO',
                                             l_bill_contact_email ||
                                             l_bill_mail_suffix);
            else
              l_distribution_list := replace(l_distribution_list,
                                             'BILL_TO',
            ...
            if instr(l_distribution_list,
                     l_bill_contact_email || l_bill_mail_suffix) = 0 then
              l_distribution_list := l_bill_contact_email ||
                                     l_bill_mail_suffix || '|' ||
                                     l_distribution_list;
            end if;
          end if;
          ...
          l_delete            := 'Y';
          l_distribution_list := replace(l_distribution_list, 'BILL_TO', '');
        end if;
      else
        l_delete := 'Y';
      end if;
    ...
    l_delete := 'N';
    if p_sold_contact_id is not null then
      if instr(l_mail_list_raw, 'SOLD_TO') <> 0 then
        begin
          SELECT hcp.email_address
            INTO l_cust_contact_email
            FROM hz_cust_account_roles hcar, hz_contact_points hcp
           WHERE hcp.owner_table_name = 'HZ_PARTIES'
             AND hcp.contact_point_type = 'EMAIL'
             AND hcp.email_address IS NOT NULL
             ...
             AND hcp.primary_flag = 'Y'
             AND hcar.party_id = hcp.owner_table_id
             AND hcar.cust_account_role_id = p_sold_contact_id;
        exception
          when no_data_found then
            l_cust_contact_email := null;
        end;

        if l_cust_contact_email is not null then
          if instr(l_distribution_list, 'SOLD_TO') <> 0 then
            for rec in (select regexp_substr(l_distribution_list,
                                             '[^|]+',
                                             1,
                                             level) emails
                        ...
                        connect by regexp_substr(l_distribution_list,
                                                 ...
                                                 1,
                                                 level) is not null) loop
              if rec.emails like '%' || l_cust_mail_suffix || '%' then
                l_distribution_list := replace(l_distribution_list,
                                               rec.emails,
                                               '');
                exit;
            ...
            if instr(l_distribution_list,
                     l_cust_contact_email || l_cust_mail_suffix) = 0 then
              l_distribution_list := replace(l_distribution_list,
                                             'SOLD_TO',
            ...
            if instr(l_distribution_list,
                     l_cust_contact_email || l_cust_mail_suffix) = 0 then
              l_distribution_list := l_cust_contact_email ||
                                     l_cust_mail_suffix || '|' ||
                                     l_distribution_list;
            end if;
          end if;
          ...
          l_delete            := 'Y';
          l_distribution_list := replace(l_distribution_list, 'SOLD_TO', '');
        end if;
      else
        l_delete := 'Y';
      end if;
    ...
    l_distribution_list := trim(ltrim(regexp_replace(l_distribution_list || '|',
                                                     '(\|){2,}',
                                                     '|'),
                                      '|'));

    RETURN l_distribution_list;
  EXCEPTION
    WHEN OTHERS THEN
      RETURN NULL;
  END generate_ack_dist_list;
  ...
  function modifier_elibility_type(p_inventory_item_id IN NUMBER)
    return varchar2 is
    l_modifier_eligibility varchar2(40) := 'NEW';
  begin
    select mcb.segment1
      into l_modifier_eligibility
     ...
     where mic.inventory_item_id = p_inventory_item_id
       ...
       and mcs.CATEGORY_SET_NAME = 'Modifier Eligibility'
       and mic.category_id = mcb.category_id
       and mcb.structure_id = mcs.STRUCTURE_ID;

    return l_modifier_eligibility;
  exception
    when no_data_found then
      return 'NEW';
  end modifier_elibility_type;

  ----------------------------------------------------------------------------------------------------
  -- Ver   When         Who            Descr
  -- ----  -----------  -------------  ---------------------------------------------------------------
  -- 1.0   24/03/2020   Roman W.       CHG0047653 - Commercial Invoice - Change logic for Zero price for SP items
  --                                   CHG0047653 - removed /* AND ROWNUM = 1 */
  -- 1.1   19/05/2020   Roman W.       CHG0047959 - Commercial Invoice - Fix SP 50% price on zero price function
  ----------------------------------------------------------------------------------------------------
  function is_line_order_sp_zero(p_line_id number) return varchar2 is
    -----------------------
    -- Local Definition
    -----------------------
    ...
    SELECT 'Y'
     ...
     WHERE oola.unit_selling_price = 0
       AND xxinv_utils_pkg.get_category_segment('SEGMENT1',
                                                ...
                                                oola.INVENTORY_ITEM_ID) =
           'Customer Support' -- FROM Product Hierarchy
       AND nvl(oola.item_type_code, 'STANDARD') = 'STANDARD' -- CHG0047959
       AND oola.line_id = p_line_id;
    --AND ROWNUM = 1; -- rem by Roman W. 01/04/2020 CHG0047653

    return l_ret_value;
  exception
    when others then
      return 'N';
  end is_line_order_sp_zero;
  /*
  --------------------------------------------------------------------------------
  -- Ver   When        Who             Descr
  -- ----  ----------  --------------  -------------------------------------------
  ...
  --------------------------------------------------------------------------------
  function get_ship_to_country_code(p_line_id NUMBER) return varchar2 is
    --------------------------
    --   Local Definition
    --------------------------
  ...
  */
  ...
  -- 1.0   04/02/2021  Roman W.        CHG0049362 - ECO country excluding from notifications
  --------------------------------------------------------------------------------
  function get_ship_to_country_code(p_ship_to_org_id NUMBER) return varchar2 is
    --------------------------
    --   Local Definition
    --------------------------
    l_ret_value VARCHAr2(120);
  ...
  begin
    select hl.country
      ...
      from hz_cust_site_uses_all  hcs,
           ...
           hz_locations           hl
     where 1 = 1
       and hcs.site_use_id = p_ship_to_org_id
  ...
  function get_order_totals(p_header_id NUMBER,
                            p_converted_currency VARCHAR2 default NULL) return number IS
    ...
    l_charges number;
  begin
    select oh.transactional_curr_code, oh.pricing_date
      into l_order_currency, l_order_date
      from oe_order_headers_all oh
    ...
    l_order_total := l_subtotal + l_charges + l_tax;
    
    if p_converted_currency is null then
      return l_order_total;
    else
      if l_order_currency <> p_converted_currency then
        return l_order_total * gl_currency_api.get_closest_rate(x_from_currency => l_order_currency,
                                         x_to_currency => p_converted_currency,
                                         x_conversion_date => l_order_date,
