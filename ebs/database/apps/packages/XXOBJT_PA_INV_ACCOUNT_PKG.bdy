CREATE OR REPLACE PACKAGE BODY XXOBJT_PA_INV_ACCOUNT_PKG AS
  ...
  --  1.2  10/07/2020  Diptasurjya       CHG0048732 - Add procedure to fetch BU segment for project based REQ
  --------------------------------------------------------------------
  ...
  --------------------------------------------------------------------
  --  name:            get_bu_from_person_expense
  --  create by:       Diptasurjya Chatterjee
  --  Revision:        1.0
  --  creation date:   10/07/2020
  --------------------------------------------------------------------
  --  purpose :        procedure will get BU segment value from the default purchasing expense account
  --                   for the requestor on the requisition line
  --------------------------------------------------------------------
  --  ver  date        name                          desc
  --  1.0  10/07/2020  Diptasurjya Chatterjee        CHG0048732 - initial build
  --------------------------------------------------------------------    
  PROCEDURE get_bu_from_person_expense(itemtype IN  VARCHAR2,
			    itemkey  IN  VARCHAR2,
			    actid    IN  NUMBER,
			    funcmode IN  VARCHAR2,
			    result   OUT nocopy VARCHAR2)
  IS
	  ...
	  lv_bu_segment 	VARCHAR2(20);
  BEGIN

  IF (funcmode = 'RUN') THEN
  	ln_to_person_id := wf_engine.GetItemAttrNumber(itemtype, itemkey,'TO_PERSON_ID');
    
    begin
      select glc.segment9
        into lv_bu_segment
        from per_people_x pp, per_assignments_x pa, gl_code_combinations glc
       where pp.PERSON_ID = pa.PERSON_ID
         and pa.DEFAULT_CODE_COMB_ID = glc.code_combination_id
         and pp.PERSON_ID = ln_to_person_id;
    exception when others then
      lv_bu_segment := '000000';
    end;
    
	  wf_engine.SetItemAttrText(itemtype, itemkey, 'XXPO_PA_EXPENSE_BU', lv_bu_segment);
  ...
  END GET_BU_FROM_PERSON_EXPENSE;
