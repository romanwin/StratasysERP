CREATE OR REPLACE PACKAGE BODY xxobjt_custom_events_pkg AS
  ...
  -- 1.7  27.2.2019   yuval tal       INC0148958 - modify om_events/ar_events
  -- ---------------------------------------------------------------------------------
  ...
  EXCEPTION
    WHEN OTHERS THEN
      insert_error_event('XXOBJT_CUSTOM_EVENTS_PKG.INSERT_EVENT',
		 'Error occurred in insert_event for' ||
		 ' event_name ' || p_rec.event_name ||
		 ' event_table ' || p_rec.event_table ||
		 ' event_key ' || p_rec.event_key || ' : ' ||
		 SQLERRM);
  END insert_event;
  ...
  BEGIN
    ...
    ELSE
	 ...
	 nvl(fnd_date.canonical_to_date(p_date_from),
	      to_date('01011889', 'DDMMYYYY')) AND
	 nvl(fnd_date.canonical_to_date(p_date_to),
	     to_date('31124792', 'DDMMYYYY'));
    
      fnd_file.put_line(fnd_file.output,
		'Total Records Deleted: ' || SQL%ROWCOUNT);
    END IF;
    retcode := 0;
  EXCEPTION
    WHEN e_error THEN
  ...
  BEGIN
    ...
    l_date := xxobjt_general_utils_pkg.get_valueset_desc('XXOBJT_EVENT_TABLE_NAME',
				         p_source_name);
  ...
  --  1.2    27.2.19     yuval tal           INC0148958 - add >= to query
  -------------------------------------------------------------------------
  ...
  PROCEDURE om_events(errbuf            OUT VARCHAR2,
    ...
    CURSOR c_ord(c_last_query_date DATE) IS
      ...
      WHERE  t.last_update_date >= c_last_query_date --INC0148958
  ...
  BEGIN
    ...
    BEGIN
      ...
      END LOOP;
      COMMIT;
      fnd_flex_val_api.update_independent_vset_value(p_flex_value_set_name => 'XXOBJT_EVENT_TABLE_NAME',
				     p_flex_value          => l_rec.event_table,
				     p_description         => to_char(l_current_query_date,
							  'DDMMYYYY HH24MISS'),
				     x_storage_value       => l_out);
  ...
  --  ver    date          name             desc
  --  1.0    4.3.14     yuval tal           CUST776 CR 1215 intial build , Customer support SF-OA interfaces
  --  1.1    27.2.19    YUVAL TAL           INC0148958 add >= to cursor query 
  ---------------------------------------------------------------------
  PROCEDURE ar_events(errbuf            OUT VARCHAR2,
    ...
    CURSOR c_ar_trx(c_last_query_date DATE) IS
      ...
      WHERE  t.creation_date >= c_last_query_date --INC0148958
  ...
  PROCEDURE cs_events(errbuf            OUT VARCHAR2,
    ...
    CURSOR c_get_upg_instance_id(c_last_query_date    DATE,
	 ...
	 (xpi.creation_date +
	 (fnd_profile.value('XXCS_UPGRADE_SHIPP_DAYS'))) upgrade_date, -- Added on 3Oct2018 for #CHG0043859
  ...
  BEGIN
    ...
    FOR i IN c_get_upg_instance_id(l_last_query_date, l_current_query_date) LOOP
      ...
      */ -- Commented for #CHG0043859
      ...
      l_xxssys_event_rec.attribute4 := to_char(i.upgrade_date,
			           'DD-MON-YYYY');
    ...
    END LOOP;
    fnd_flex_val_api.update_independent_vset_value(p_flex_value_set_name => 'XXOBJT_EVENT_TABLE_NAME',
				   p_flex_value          => l_event_table /*#CHG0043859 l_rec.event_table*/,
				   p_description         => to_char(l_current_query_date,
							'DDMMYYYY HH24MISS'),
				   x_storage_value       => l_out);
    COMMIT;
